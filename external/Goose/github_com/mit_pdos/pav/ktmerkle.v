...
Definition signedLink := struct.decl [
  "epoch" :: epochTy;
  "prevLink" :: linkTy;
  "dig" :: slice.T byteT;
  "sig" :: slice.T byteT
...
Definition chainSepSome__encode: val :=
  rec: "chainSepSome__encode" "o" :=
    ...
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF chainSepSome "prevLink" "o"));;
...
Definition servSepLink__encode: val :=
  rec: "servSepLink__encode" "o" :=
    ...
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF servSepLink "link" "o"));;
    ![slice.T byteT] "b".

Definition signedLink__check: val :=
  rec: "signedLink__check" "o" "pk" :=
    let: "preLink" := chainSepSome__encode (struct.new chainSepSome [
      "epoch" ::= struct.loadF signedLink "epoch" "o";
      "prevLink" ::= struct.loadF signedLink "prevLink" "o";
      "data" ::= struct.loadF signedLink "dig" "o"
    ]) in
    let: "link" := cryptoffi.Hash "preLink" in
    let: "sepLink" := servSepLink__encode (struct.new servSepLink [
      "link" ::= "link"
    ]) in
    let: "ok" := cryptoffi.PublicKey__Verify "pk" "sepLink" (struct.loadF signedLink "sig" "o") in
    ("link", (~ "ok")).

Definition signedPut := struct.decl [
  "epoch" :: epochTy;
  "id" :: slice.T byteT;
  "val" :: slice.T byteT;
  "sig" :: slice.T byteT
...
Definition servSepPut__encode: val :=
  rec: "servSepPut__encode" "o" :=
    ...
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF servSepPut "id" "o"));;
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF servSepPut "val" "o"));;
    ![slice.T byteT] "b".

Definition signedPut__check: val :=
  rec: "signedPut__check" "o" "pk" :=
    let: "sepPut" := servSepPut__encode (struct.new servSepPut [
      "epoch" ::= struct.loadF signedPut "epoch" "o";
      "id" ::= struct.loadF signedPut "id" "o";
      "val" ::= struct.loadF signedPut "val" "o"
    ]) in
    let: "okPut" := cryptoffi.PublicKey__Verify "pk" "sepPut" (struct.loadF signedPut "sig" "o") in
    (~ "okPut").
...
Definition epochChain := struct.decl [
  "epochs" :: slice.T ptrT
].
...
Definition epochChain__put: val :=
  rec: "epochChain__put" "c" "tree" "sk" :=
    let: "chainLen" := slice.len (struct.loadF epochChain "epochs" "c") in
    ...
    struct.storeF epochChain "epochs" "c" (SliceAppend ptrT (struct.loadF epochChain "epochs" "c") "epoch");;
    #().
  ...
  rec: "server__getIdAt" "s" "id" "epoch" :=
    ...
    then
      ...
      struct.new servGetIdAtReply [
        ...
        "sig" ::= struct.loadF epochInfo "linkSig" "info";
...
Definition client__addLink: val :=
  rec: "client__addLink" "c" "epoch" "prevLink" "dig" "sig" :=
    ...
    then (slice.nil, errSome)
      ...
      let: "newSigLn" := struct.new signedLink [
        "epoch" ::= "epoch";
        "prevLink" ::= "prevLink";
        "dig" ::= "dig";
        "sig" ::= "sig"
      ...
      then
        let: "cachedSigLn" := struct.new signedLink [
          "epoch" ::= "epoch";
          "prevLink" ::= struct.loadF cliSigLink "prevLink" "cachedLink";
          "dig" ::= struct.loadF cliSigLink "dig" "cachedLink";
...
Definition servPutArg__encode: val :=
  rec: "servPutArg__encode" "o" :=
    let: "b" := ref_to (slice.T byteT) (NewSlice byteT #0) in
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF servPutArg "id" "o"));;
    ...
    then (slice.nil, "err")
      ...
      then (slice.nil, "err")
        ...
        then (slice.nil, "err")
          ...
          let: (("linkSig", "b"), "err") := marshalutil.ReadSlice1D (![slice.T byteT] "b") in
          (if: "err"
          then (slice.nil, "err")
          else
            let: (("putSig", "b"), "err") := marshalutil.ReadSlice1D (![slice.T byteT] "b") in
            (if: "err"
            then (slice.nil, "err")
            else
...
(* timeSeries__put from ktmerkle.go *)
...
Definition servGetIdAtArg__encode: val :=
  rec: "servGetIdAtArg__encode" "o" :=
    let: "b" := ref_to (slice.T byteT) (NewSlice byteT #0) in
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF servGetIdAtArg "id" "o"));;
  ...
  rec: "servGetIdAtReply__decode" "o" "b0" :=
    ...
    then (slice.nil, "err")
      ...
      then (slice.nil, "err")
        ...
        let: (("sig", "b"), "err") := marshalutil.ReadSlice1D (![slice.T byteT] "b") in
        (if: "err"
        then (slice.nil, "err")
        else
  ...
  rec: "adtrGetReply__decode" "o" "b0" :=
    ...
    then (slice.nil, "err")
      ...
      then (slice.nil, "err")
        ...
        let: (("servSig", "b"), "err") := marshalutil.ReadSlice1D (![slice.T byteT] "b") in
        (if: "err"
        then (slice.nil, "err")
        else
          let: (("adtrSig", "b"), "err") := marshalutil.ReadSlice1D (![slice.T byteT] "b") in
          (if: "err"
          then (slice.nil, "err")
          else
            let: (("error", "b"), "err") := marshalutil.ReadBool (![slice.T byteT] "b") in
        ...
        then (#0, slice.nil, struct.loadF adtrGetReply "error" "reply")
          ...
          then (#0, slice.nil, errSome)
            ...
            then (#0, slice.nil, errSome)
              ...
              then
                let: "adtrSigLn" := struct.new signedLink [
                  "epoch" ::= "lastEpoch";
                  "prevLink" ::= struct.loadF adtrGetReply "prevLink" "reply";
                  "dig" ::= struct.loadF adtrGetReply "dig" "reply";
                  "sig" ::= struct.loadF adtrGetReply "servSig" "reply"
                ] in
                let: "mySigLn" := struct.new signedLink [
                  "epoch" ::= "lastEpoch";
                  "prevLink" ::= struct.loadF cliSigLink "prevLink" "lastLink";
                  "dig" ::= struct.loadF cliSigLink "dig" "lastLink";
...
(* timeSeries__get from ktmerkle.go *)
...
Definition timeSeries__get: val :=
  rec: "timeSeries__get" "ts" "epoch" :=
    ...
    ForSlice (struct.t timeEntry) <> "te" (![timeSeries] "ts")
      ((if: (struct.get timeEntry "time" "te") ≤ "epoch"
      then
    ...
    then (slice.nil, slice.nil, struct.loadF servGetIdAtReply "error" "reply")
      ...
      then (slice.nil, slice.nil, "errMerkle")
        ...
        then ("linkEvid", slice.nil, "errLink")
          ...
          (if: "expProofTy" ≠ (struct.loadF servGetIdAtReply "proofTy" "reply")
          then (slice.nil, slice.nil, errSome)
            ...
            then
              (if: "isBoundary"
              then
                let: "sigLn" := struct.new signedLink [
                  "epoch" ::= "epoch";
                  "prevLink" ::= struct.loadF servGetIdAtReply "prevLink" "reply";
                  "dig" ::= struct.loadF servGetIdAtReply "dig" "reply";
                  "sig" ::= struct.loadF servGetIdAtReply "sig" "reply"
                ] in
                let: "sigPut" := struct.new signedPut [
                  "epoch" ::= "epoch";
                  "id" ::= struct.loadF client "id" "c";
                  "val" ::= "expVal";
                  "sig" ::= "putSig"
...
Definition client__selfCheck: val :=
  rec: "client__selfCheck" "c" :=
    ...
    then (#0, slice.nil, slice.nil, errSome)
      ...
      (if: ((![ptrT] "evidLink") ≠ #null) || ((![ptrT] "evidPut") ≠ #null)
...
Definition servPutArg__decode: val :=
  rec: "servPutArg__decode" "o" "b0" :=
    let: "b" := ref_to (slice.T byteT) "b0" in
    let: (("id", "b"), "err") := marshalutil.ReadSlice1D (![slice.T byteT] "b") in
    (if: "err"
    then (slice.nil, "err")
    else
      let: (("val", "b"), "err") := marshalutil.ReadSlice1D (![slice.T byteT] "b") in
...
Definition servGetIdAtReply__encode: val :=
  rec: "servGetIdAtReply__encode" "o" :=
    let: "b" := ref_to (slice.T byteT) (NewSlice byteT #0) in
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF servGetIdAtReply "prevLink" "o"));;
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF servGetIdAtReply "dig" "o"));;
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF servGetIdAtReply "sig" "o"));;
...
Definition servGetLinkReply__encode: val :=
  rec: "servGetLinkReply__encode" "o" :=
    let: "b" := ref_to (slice.T byteT) (NewSlice byteT #0) in
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF servGetLinkReply "prevLink" "o"));;
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF servGetLinkReply "dig" "o"));;
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF servGetLinkReply "sig" "o"));;
...
Definition server__start: val :=
  rec: "server__start" "s" "addr" :=
    ...
    MapInsert "handlers" rpcServUpdateEpoch (λ: "enc_args" "enc_reply",
      server__updateEpoch "s";;
      #()
      );;
    MapInsert "handlers" rpcServPut (λ: "enc_args" "enc_reply",
    ...
    MapInsert "handlers" rpcServGetIdAt (λ: "enc_args" "enc_reply",
    ...
    MapInsert "handlers" rpcServGetLink (λ: "enc_args" "enc_reply",
...
Definition adtrGetReply__encode: val :=
  rec: "adtrGetReply__encode" "o" :=
    let: "b" := ref_to (slice.T byteT) (NewSlice byteT #0) in
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF adtrGetReply "prevLink" "o"));;
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF adtrGetReply "dig" "o"));;
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF adtrGetReply "servSig" "o"));;
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF adtrGetReply "adtrSig" "o"));;
...
Definition auditor__start: val :=
  rec: "auditor__start" "a" "addr" :=
    let: "handlers" := NewMap uint64T ((slice.T byteT) -> ptrT -> unitT)%ht #() in
    MapInsert "handlers" rpcAdtrPut (λ: "enc_args" "enc_reply",
    ...
    MapInsert "handlers" rpcAdtrGet (λ: "enc_args" "enc_reply",
...
Definition adtrPutArg__encode: val :=
  rec: "adtrPutArg__encode" "o" :=
    let: "b" := ref_to (slice.T byteT) (NewSlice byteT #0) in
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF adtrPutArg "prevLink" "o"));;
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF adtrPutArg "dig" "o"));;
    "b" <-[slice.T byteT] (marshalutil.WriteSlice1D (![slice.T byteT] "b") (struct.loadF adtrPutArg "servSig" "o"));;
    ![slice.T byteT] "b".
