...
Definition EnterNewConfig: val :=
  rec: "EnterNewConfig" "configHosts" "servers" :=
    ...
    then
      ...
      let: "configCk" := config2.MakeClerk "configHosts" in
      let: ("epoch", "oldServers") := config2.Clerk__ReserveEpochAndGetConfig "configCk" in
      ...
      then
        ...
        then
          ...
          (if: (config2.Clerk__TryWriteConfig "configCk" "epoch" "servers") ≠ e.None
          then
...
Definition InitializeSystem: val :=
  rec: "InitializeSystem" "configHosts" "servers" :=
    let: "configCk" := config2.MakeClerk "configHosts" in
    config2.Clerk__TryWriteConfig "configCk" #0 "servers";;
