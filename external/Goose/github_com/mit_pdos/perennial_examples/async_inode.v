(* autogenerated from github.com/mit-pdos/perennial-examples/async_inode *)
From Perennial.goose_lang Require Import prelude.
From Perennial.goose_lang Require Import ffi.disk_prelude.

From Goose Require github_com.mit_pdos.perennial_examples.alloc.
From Goose Require github_com.tchajed.marshal.

Definition MaxBlocks : expr := #511.

Module Inode.
  Definition S := struct.decl [
    "d" :: disk.Disk;
    "m" :: lockRefT;
    "addr" :: uint64T;
    "addrs" :: slice.T uint64T;
    "buffered" :: slice.T disk.blockT
  ].
End Inode.

Definition Open: val :=
  rec: "Open" "d" "addr" :=
    let: "b" := disk.Read "addr" in
    let: "dec" := marshal.NewDec "b" in
    let: "numAddrs" := marshal.Dec__GetInt "dec" in
    let: "addrs" := marshal.Dec__GetInts "dec" "numAddrs" in
    struct.new Inode.S [
      "d" ::= "d";
      "m" ::= lock.new #();
      "addr" ::= "addr";
      "addrs" ::= "addrs";
      "buffered" ::= slice.nil
    ].

(* UsedBlocks returns the addresses allocated to the inode for the purposes
   of recovery. Assumes full ownership of the inode, so does not lock,
   and expects the caller to need only temporary access to the returned slice. *)
Definition Inode__UsedBlocks: val :=
  rec: "Inode__UsedBlocks" "i" :=
    struct.loadF Inode.S "addrs" "i".

Definition Inode__read: val :=
  rec: "Inode__read" "i" "off" :=
    (if: "off" ≥ slice.len (struct.loadF Inode.S "addrs" "i") + slice.len (struct.loadF Inode.S "buffered" "i")
    then slice.nil
    else
      (if: "off" < slice.len (struct.loadF Inode.S "addrs" "i")
      then
        let: "a" := SliceGet uint64T (struct.loadF Inode.S "addrs" "i") "off" in
        disk.Read "a"
      else SliceGet (slice.T byteT) (struct.loadF Inode.S "buffered" "i") ("off" - slice.len (struct.loadF Inode.S "addrs" "i")))).

Definition Inode__Read: val :=
  rec: "Inode__Read" "i" "off" :=
    lock.acquire (struct.loadF Inode.S "m" "i");;
    let: "b" := Inode__read "i" "off" in
    lock.release (struct.loadF Inode.S "m" "i");;
    "b".

Definition Inode__Size: val :=
  rec: "Inode__Size" "i" :=
    lock.acquire (struct.loadF Inode.S "m" "i");;
    let: "sz" := slice.len (struct.loadF Inode.S "addrs" "i") + slice.len (struct.loadF Inode.S "buffered" "i") in
    lock.release (struct.loadF Inode.S "m" "i");;
    "sz".

Definition Inode__mkHdr: val :=
  rec: "Inode__mkHdr" "i" :=
    let: "enc" := marshal.NewEnc disk.BlockSize in
    marshal.Enc__PutInt "enc" (slice.len (struct.loadF Inode.S "addrs" "i"));;
    marshal.Enc__PutInts "enc" (struct.loadF Inode.S "addrs" "i");;
    let: "hdr" := marshal.Enc__Finish "enc" in
    "hdr".

Definition reserveMany: val :=
  rec: "reserveMany" "allocator" "n" :=
    let: "failed" := ref_to boolT #false in
    let: "allocated" := ref (zero_val (slice.T uint64T)) in
    let: "i" := ref_to uint64T #0 in
    (for: (λ: <>, ![uint64T] "i" < "n"); (λ: <>, "i" <-[uint64T] ![uint64T] "i" + #1) := λ: <>,
      let: ("a", "ok") := alloc.Allocator__Reserve "allocator" in
      (if: ~ "ok"
      then
        "failed" <-[boolT] #true;;
        Break
      else "allocated" <-[slice.T uint64T] SliceAppend uint64T (![slice.T uint64T] "allocated") "a"));;
    (if: ![boolT] "failed"
    then
      ForSlice uint64T <> "a" (![slice.T uint64T] "allocated")
        (alloc.Allocator__Free "allocator" "a");;
      (slice.nil, #false)
    else (![slice.T uint64T] "allocated", #true)).

(* critical section for Flush

   assumes lock is held *)
Definition Inode__flush: val :=
  rec: "Inode__flush" "i" "allocator" :=
    let: ("addresses", "ok") := reserveMany "allocator" (slice.len (struct.loadF Inode.S "buffered" "i")) in
    (if: ~ "ok"
    then #false
    else
      ForSlice (slice.T byteT) "j" "buf" (struct.loadF Inode.S "buffered" "i")
        (let: "a" := SliceGet uint64T "addresses" "j" in
        disk.Write "a" "buf";;
        struct.storeF Inode.S "addrs" "i" (SliceAppend uint64T (struct.loadF Inode.S "addrs" "i") "a"));;
      let: "hdr" := Inode__mkHdr "i" in
      disk.Write (struct.loadF Inode.S "addr" "i") "hdr";;
      struct.storeF Inode.S "buffered" "i" slice.nil;;
      #true).

(* Flush persists all allocated data atomically

   returns false on allocator failure *)
Definition Inode__Flush: val :=
  rec: "Inode__Flush" "i" "allocator" :=
    lock.acquire (struct.loadF Inode.S "m" "i");;
    let: "ok" := Inode__flush "i" "allocator" in
    lock.release (struct.loadF Inode.S "m" "i");;
    "ok".

(* assumes lock is held *)
Definition Inode__append: val :=
  rec: "Inode__append" "i" "b" :=
    (if: slice.len (struct.loadF Inode.S "addrs" "i") + slice.len (struct.loadF Inode.S "buffered" "i") ≥ MaxBlocks
    then #false
    else
      struct.storeF Inode.S "buffered" "i" (SliceAppend (slice.T byteT) (struct.loadF Inode.S "buffered" "i") "b");;
      #true).

(* Append adds a block to the inode, without making it persistent.

   Returns false on failure (if the allocator or inode are out of space) *)
Definition Inode__Append: val :=
  rec: "Inode__Append" "i" "b" :=
    lock.acquire (struct.loadF Inode.S "m" "i");;
    let: "ok" := Inode__append "i" "b" in
    lock.release (struct.loadF Inode.S "m" "i");;
    "ok".
