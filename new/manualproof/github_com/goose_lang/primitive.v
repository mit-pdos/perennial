From New.code Require Export github_com.goose_lang.primitive.
...
Global Instance into_val_Mutex : IntoVal Mutex.t :=
  {|
    to_val_def := λ v, struct.val_aux primitive.Mutex [("state" ::= #v.(Mutex.state))]%struct
  |}.

Global Program Instance into_val_typed_Mutex : IntoValTyped Mutex.t primitive.Mutex :=
...
Global Instance into_val_struct_Mutex_state :
  IntoValStructField "state" primitive.Mutex Mutex.state.
...
Global Instance wp_struct_make_Mutex (state : bool) :
  PureWp True
    (struct.make #primitive.Mutex (alist_val [
      "state" ::= #state
    ]))%struct
    #(Mutex.mk state).
...
Global Instance Mutex_struct_split dq l (v : Mutex.t) :
  StructFieldsSplit dq l v (
      "Hstate" ∷ l ↦s[primitive.Mutex :: "state"]{dq} v.(Mutex.state)
    ).
Proof.
  rewrite /named.
