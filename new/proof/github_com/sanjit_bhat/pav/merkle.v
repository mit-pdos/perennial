...
Local Definition is_tree_hash_pre (recur : tree -d> list w8 -d> iPropO Σ)
    : tree -d> list w8 -d> iPropO Σ :=
  (λ t h,
    ∃ d,
...
Local Instance is_tree_hash_pre_contractive : Contractive is_tree_hash_pre.
Proof. solve_contractive. Qed.

Local Definition is_tree_hash_def : tree → list w8 → iProp Σ :=
  fixpoint is_tree_hash_pre.
Local Program Definition is_tree_hash := sealed is_tree_hash_def.
Local Definition is_tree_hash_unseal : is_tree_hash = _ := seal_eq _.

Local Lemma is_tree_hash_unfold t h :
  is_tree_hash t h ⊣⊢
    ∃ d,
    "#His_hash" ∷ cryptoffi.is_hash d h ∗
    "Hdecode" ∷ match decode_node d with
