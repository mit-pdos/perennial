...
Theorem wp_ReadInt tail s q x :
  {{{ is_pkg_init marshal ∗ own_slice s q (u64_le x ++ tail) }}}
    marshal @ "ReadInt" #s
  {{{ (s': slice.t), RET (#x, #s'); own_slice s' q tail }}}.
Proof.
  wp_start as "Hs".
  wp_auto.
  ...
  rewrite length_app u64_le_length in Hlen.
  iDestruct (own_slice_wf with "Hs") as %Hwf.
  wp_pure.
  { word. }
  wp_auto.
  iApply "HΦ".
  iDestruct (own_slice_split_all (W64 8) with "Hs") as "[Hs1 Hs2]".
  { word. }
  rewrite drop_app_ge; [ | len ].
  rewrite drop_eq_0; [ | len ].
  iFrame "Hs2".
Qed.

(*
Theorem wp_ReadInt32 tail s q (x: u32) :
  {{{ is_pkg_init marshal ∗ own_slice_small s byteT q (u32_le x ++ tail) }}}
    ReadInt32 (slice_val s)
  {{{ s', RET (#x, slice_val s'); own_slice_small s' byteT q tail }}}.
Proof.
  iIntros (Φ) "Hs HΦ". wp_auto.
...
Theorem wp_ReadBytes s q (len: u64) (head tail : list u8) :
  ...
  {{{ is_pkg_init marshal ∗ own_slice_small s byteT q (head ++ tail) }}}
...
Proof.
  iIntros (Hlen Φ) "Hs HΦ". wp_auto. wp_auto.
...
Proof.
  iIntros (Hown_val ϕ) "Hpre HΦ". iNamed "Hpre". wp_auto. wp_auto.
...
Proof.
  iIntros (Φ) "_ HΦ". wp_auto. wp_auto.
  wp_apply wp_ref_to. { val_ty. }
  iIntros (l) "Hl". wp_auto.
  wp_load.
  wp_if_destruct.
...
Local Theorem wp_reserve s (extra : u64) (vs : list u8) :
  {{{ is_pkg_init marshal ∗ own_slice s byteT (DfracOwn 1) vs }}}
...
Theorem wp_WriteBytes s (vs : list u8) data_sl q (data : list u8) :
  {{{ is_pkg_init marshal ∗ own_slice s byteT (DfracOwn 1) vs ∗ own_slice_small data_sl byteT q data }}}
...
Proof.
  iIntros (Φ) "[Hs Hdata] HΦ". wp_auto. wp_auto.
