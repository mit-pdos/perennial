...
From New.generatedproof Require Import github_com.mit_pdos.gokv.vrsm.replica.err_gk.
...
Definition name_map_def : gmap w32 go_string := list_to_map [
                                                ...
                                                ((W32 2), "OutOfOrder"%go);
                                                ((W32 3), "Timeout"%go);
                                                ((W32 4), "EmptyConfig"%go);
                                                ((W32 5), "NotLeader"%go);
                                                ((W32 6), "Sealed"%go);
                                                ((W32 7), "LeaseExpired"%go);
                                                ((W32 8), "Leased"%go)
                                              ].
Definition value_map_def : gmap go_string w32 := list_to_map [
                                                 ...
                                                 ("OutOfOrder"%go, (W32 2));
                                                 ("Timeout"%go, (W32 3));
                                                 ("EmptyConfig"%go, (W32 4));
                                                 ("NotLeader"%go, (W32 5));
                                                 ("Sealed"%go, (W32 6));
                                                 ("LeaseExpired"%go, (W32 7));
                                                 ("Leased"%go, (W32 8))
                                               ].

Definition is_initialized : iProp Σ :=
  ∃ name_map value_map,
  "HglobalName" ∷ (global_addr err_gk.Name) ↦□ name_map ∗
  "Hname" ∷ name_map ↦$□ name_map_def ∗
  "HglobalValue" ∷ (global_addr err_gk.Value) ↦□ value_map ∗
  "Hvalue" ∷ value_map ↦$□ value_map_def.

#[global] Instance : IsPkgInit err_gk := define_is_pkg_init is_initialized.
#[global] Instance : GetIsPkgInitWf err_gk := build_get_is_pkg_init_wf.

Lemma wp_initialize' get_is_pkg_init :
  get_is_pkg_init_prop err_gk get_is_pkg_init ->
  {{{ own_initializing get_is_pkg_init ∗ is_go_context ∗ □ is_pkg_defined err_gk }}}
    err_gk.initialize' #()
  {{{ RET #(); own_initializing get_is_pkg_init ∗ is_pkg_init err_gk }}}.
...
Definition own (args__v : err_gk.E.t) (args__c : I) (dq : dfrac) : iProp Σ :=
  "%Herr_eq" ∷ ⌜ args__v = to_tag args__c ⌝.

Lemma wp_Encode (args__t : err_gk.E.t) (args__c : I) (pre_sl : slice.t) (prefix : list u8) (dq : dfrac):
  {{{
        is_pkg_init err_gk ∗
