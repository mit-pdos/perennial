...
From New.code Require Import github_com.mit_pdos.gokv.memkv.kvop_gk.
From New.generatedproof Require Import github_com.mit_pdos.gokv.memkv.kvop_gk.
...
Definition name_map_def : gmap w32 go_string := list_to_map [
                                                ((W32 0), "KV_FRESHCID"%go);
                                                ((W32 1), "KV_PUT"%go);
                                                ((W32 2), "KV_GET"%go);
                                                ((W32 3), "KV_CONDITIONAL_PUT"%go);
                                                ((W32 4), "KV_INS_SHARD"%go);
                                                ((W32 5), "KV_MOV_SHARD"%go)
                                              ].
Definition value_map_def : gmap go_string w32 := list_to_map [
                                                 ("KV_FRESHCID"%go, (W32 0));
                                                 ("KV_PUT"%go, (W32 1));
                                                 ("KV_GET"%go, (W32 2));
                                                 ("KV_CONDITIONAL_PUT"%go, (W32 3));
                                                 ("KV_INS_SHARD"%go, (W32 4));
                                                 ("KV_MOV_SHARD"%go, (W32 5))
                                               ].

Definition is_initialized : iProp Σ :=
  ∃ name_map value_map,
  "HglobalName" ∷ (global_addr kvop_gk.Name) ↦□ name_map ∗
  "Hname" ∷ name_map ↦$□ name_map_def ∗
  "HglobalValue" ∷ (global_addr kvop_gk.Value) ↦□ value_map ∗
  "Hvalue" ∷ value_map ↦$□ value_map_def.

#[global] Instance : IsPkgInit kvop_gk := define_is_pkg_init is_initialized.
#[global] Instance : GetIsPkgInitWf kvop_gk := build_get_is_pkg_init_wf.

Lemma wp_initialize' get_is_pkg_init :
  get_is_pkg_init_prop kvop_gk get_is_pkg_init ->
  {{{ own_initializing get_is_pkg_init ∗ is_go_context ∗ □ is_pkg_defined kvop_gk }}}
    kvop_gk.initialize' #()
  {{{ RET #(); own_initializing get_is_pkg_init ∗ is_pkg_init kvop_gk }}}.
...
Definition own (args__v : kvop_gk.E.t) (args__c : I) (dq : dfrac) : iProp Σ :=
  "%Hkvop_eq" ∷ ⌜ args__v = to_tag args__c ⌝.

Lemma wp_Encode (args__t : kvop_gk.E.t) (args__c : I) (pre_sl : slice.t) (prefix : list u8) (dq : dfrac):
  {{{
        is_pkg_init kvop_gk ∗
    ...
    @! kvop_gk.Marshal #pre_sl #args__t
  {{{
