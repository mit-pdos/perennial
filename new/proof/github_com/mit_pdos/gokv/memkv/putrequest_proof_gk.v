...
From New.code Require Import github_com.mit_pdos.gokv.memkv.putrequest_gk.
From New.generatedproof Require Import github_com.mit_pdos.gokv.memkv.putrequest_gk.

Module putRequest_gk.
Section putRequest_gk.

Context `{hG: heapGS Σ, !ffi_semantics _ _}.
Context `{!globalsGS Σ} {go_ctx : GoContext}.

#[global] Instance : IsPkgInit putrequest_gk := define_is_pkg_init True%I.
...
Definition own (args__v: putrequest_gk.S.t) (args__c: C) (dq: dfrac) : iProp Σ :=
  "%Hown_key" ∷ ⌜ args__v.(putrequest_gk.S.Key') = args__c.(key') ⌝ ∗
  "Hown_value" ∷ own_slice args__v.(putrequest_gk.S.Value') dq args__c.(value') ∗
  "%Hown_value_len" ∷ ⌜ length args__c.(value') < 2^64 ⌝.

Lemma wp_Encode (args__t : putrequest_gk.S.t) (args__c : C) (pre_sl : slice.t) (prefix : list u8) (dq : dfrac):
  {{{
        is_pkg_init putrequest_gk ∗
        own args__t args__c dq ∗ 
        own_slice pre_sl (DfracOwn 1) prefix ∗
        own_slice_cap w8 pre_sl (DfracOwn 1)
  }}}
    @! putrequest_gk.Marshal #pre_sl #args__t
  {{{
...
Lemma wp_Decode (enc : list u8) (enc_sl : slice.t) (args__c : C) (suffix : list u8) (dq : dfrac):
  {{{
    ...
    @! putrequest_gk.Unmarshal #enc_sl
  {{{
...
Proof.
  ...
  destruct Henc as (Henc & Hlen_value ).
