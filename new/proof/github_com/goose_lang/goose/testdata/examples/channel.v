...
Lemma wp_BroadcastExample :
  ...
  {{{ RET #(); True }}}.
Proof using chan_idiomG0 chan_idiomG1 ext ffi ffi_interp0 ffi_semantics0 go_ctx hG Σ.
  wp_start. wp_auto.
  ...
  iMod (start_done_with_broadcast_notified done_ch γdone (sharedValue_ptr ↦□ W64 2)%I with "Hdone_is_chan Hdone_own")
  ...
  wp_apply (wp_fork with "[result1 result2 done]").
  {
    ...
    wp_auto_lc 1.
    wp_apply ((wp_done_receive_broadcast (V:=unit) γbc done_ch_1 ((sharedValue_ptr ↦□ W64 2)%I))  with "[$Hbc]").
    {
      iFrame "#".
    }
   ...
   wp_apply (wp_fork with "[result1 result2 done]").
  {
    rename done_ch into done_ch_1.
    wp_auto_lc 1.
    wp_apply ((wp_done_receive_broadcast (V:=unit) γbc done_ch_1  ((sharedValue_ptr ↦□ W64 2)%I))
    with "[$Hbc]").
    {
      iFrame "#".
    }
  ...
  wp_apply ((wp_done_close (V:=unit) γbc done_ch ( ((sharedValue_ptr ↦□ W64 2)%I))) with "[Hbc HBcastNotif]").
  {
    iFrame "#". iFrame.
  }
