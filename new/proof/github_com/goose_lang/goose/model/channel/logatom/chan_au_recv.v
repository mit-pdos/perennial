...
Lemma wp_TryReceive (ch: loc) (γ: chan_names) (P: iProp Σ) :
  ∀ Φ ,
  is_channel ch γ -∗
  ...
  P ∗ (P -∗ (rcv_au_slow ch γ (λ v ok, Φ (#true, #v, #ok)%V))) -∗
...
Proof.
  ...
  - (* Buffered channel *)
    ...
    iCombine "His_cap Hcap" gives %Heq; subst.
    wp_if_destruct.
    {
      ...
      subst s.
      iMod ((own_channel_halves_update _ _ _ _ (chan_rep.Buffered rest)) with "[$Hcap] Hown Hoc")
        as "[Hown1 Hown2]".
      ...
      (*            with "[$lock state slice_Hsl buffer Hown2  $Hlock]"). *)
    ...
    }
    {
      ...
      (* wp_call. *)
      (* wp_apply (wp_lock_unlock *)
      (*            with "[$lock state buffer slice slice_Hchanrepfrag $Hlock]"). *)
  ...
  - iNamed "phys". wp_auto_lc 5.
    ...
    (* wp_call. *)
    (* wp_apply (wp_lock_unlock *)
    (*            with "[$lock state v slice slice_buffer  offer1 Hpred1 Hchanrepfrag HPau HP  $Hlock]"). *)
    ...
    (*   wp_call. *)
    (*   wp_apply (wp_lock_unlock *)
    (*              with "[$lock state v slice slice_buffer Hchanrepfrag   Hp  H1   $Hlock]"). *)
    ...
    (*   iDestruct (own_channel_halves_update ch _ _ *)
    (*                (chan_rep.Idle) *)
    ...
    (*   wp_call. *)
    (*   wp_apply (wp_lock_unlock *)
    (*              with "[$lock state v slice slice_buffer H1   Hgv2 offer   $Hlock]"). *)
    (*   { unfold chan_inv_inner. iExists (Idle). iFrame. *)
    (*   } *)
    (*   unfold K. *)
  ...
  - (* Idle unbuffered channel  *)
    ...
    (* iDestruct "Hoc" as (cap') "(Hcap' & H1 & H2)". *)
    ...
    (*   with "[H1]" as "Hown1". *)
    (* { iFrame. iPureIntro. done. } *)
    (* iDestruct (own_channel_halves_update ch _ _  (chan_rep.SndPending v) *)
    ...
    (* wp_call. *)
    (* wp_apply (wp_lock_unlock *)
    (*            with "[$lock state v slice slice_buffer Hgv1 H2 Hpred Hoffer Hcont1 $Hlock]"). *)
  ...
  - iNamed "phys". wp_auto.
    ...
    (* wp_call. *)
    (* wp_apply (wp_lock_unlock *)
    (*            with "[$lock state v slice slice_buffer offer  $Hlock]"). *)
  ...
  - iNamed "phys".
    ...
    (*   wp_call. *)
    (*   wp_apply (wp_lock_unlock *)
    (*              with "[$lock state  slice slice_buffer offer H $Hlock]"). *)
...
Proof.
  ...
  wp_for. iNamed "Hau".
  wp_apply ((wp_TryReceive ch γ    (rcv_au_slow ch γ (λ (v : V) (ok : bool), Φ (v ::= # ok)%V) ∗ c_ptr ↦ ch ∗
...
Lemma wp_TryReceive_nb (ch: loc)  (γ: chan_names) (P: iProp Σ) :
  ∀ Φ ,
  is_channel ch γ -∗
  (* P is here to support helping within a select statement. This is because the internal choice of 
  which case's atomic update cannot be made prematurely if an offer isn't accepted, so we need to 
  store the whole collective conjunct of atomic updates in the channel mutex invariant for the 
  channel we are making an offer on. *)
  P ∗ (P -∗ (rcv_au_fast ch γ (λ v ok, Φ (#true, #v, #ok)%V))) -∗
  (* Keep resources for attempts at this and/or other ops *)
  (P -∗ (Φ (#false, #(default_val V), #true)%V)) -∗ 
...
Proof.
  ...
  - (* Buffered channel *)
    ...
    wp_if_destruct.
    {
      ...
      iDestruct (own_channel_agree with "Hown Hoc") as %Heq.
      subst s.
      iMod ((own_channel_halves_update _ _ _ _ (chan_rep.Buffered rest)) with "[$His_cap] Hown Hoc")
        as "[Hown1 Hown2]".
      { iPureIntro. simpl in *; lia. }
      iMod ("Hcont" with "Hown1") as "Hcont".
      iModIntro.
  ...
  - iNamed "phys".
    ...
    { iNamed "offer". iNamed "phys".
      ...
      wp_call.
      (* wp_apply (wp_lock_unlock *)
      (*            with "[$lock state  slice slice_buffer H offer $Hlock]"). *)
      (* { unfold chan_inv_inner.  iExists (Closed []). iFrame. *)
      (* } *)
      (* done. *)
      admit.
