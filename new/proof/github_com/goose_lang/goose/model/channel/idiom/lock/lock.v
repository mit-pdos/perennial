...
Definition is_lock_channel (γ : lock_channel_names) (ch : loc)
                            (R : iProp Σ) : iProp Σ :=
  "#Hchan" ∷ IsChan ch γ.(lchan_name) ∗
  "#Hinv" ∷ inv nroot (
    ∃ s locked,
      "Hch" ∷ OwnChan ch s γ.(lchan_name) ∗
...
Lemma is_lock_channel_IsChan γ ch R :
  is_lock_channel γ ch R ⊢ IsChan ch γ.(lchan_name).
Proof.
  iDestruct 1 as "[$ _]".
Qed.
...
Proof.
  ...
  }
  {
    ...
    iIntros "H".
    iDestruct (OwnChan_buffer_size with "H") as "%Hbad".
...
Lemma lock_channel_unlock_au γ ch (R : iProp Σ) :
  ...
  £1 -∗
  ▷ (∀ v, True -∗ Φ v true) -∗
  RecvAU ch γ.(lchan_name) (λ (v:V) true, Φ v true).
Proof.
