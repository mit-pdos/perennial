...
(** Future Channel
    ...
    ** How to Use
    ...
    1. Create a future channel with [start_future]
    2. For each producer, allocate a [Promise] with [future_alloc_promise],
    ...
    3. Each worker sends its result using [wp_future_fulfill], providing
       [Promise γ contract ∗ contract v], bundled as [Fulfill γ v]
...
(** [Promise γ contract] is a token representing a registered contract.
    The holder commits to eventually sending a value [v] satisfying [contract v].
    Internally, it holds half of a ghost map fragment. *)
Definition Promise (γ : future_names) (contract : V → iProp Σ) : iProp Σ :=
  ∃ (gn : gname),
    gn ↪[γ.(contracts_name)]{#1/2} contract.

(** [Fulfill γ v] bundles a [Promise] with evidence that the contract is
    satisfied. This is what gets transferred through the channel. *)
Definition Fulfill (γ : future_names) (v: V) : iProp Σ :=
  ∃ contract,
...
(** [is_future γ ch] is the persistent channel invariant. The channel
    carries [Fulfill] tokens — values bundled with their contract evidence. *)
...
Lemma start_future (ch : loc) (γ : chan_names) (s : chan_rep.t V) :
...
Lemma future_alloc_promise γ ch (contract : V → iProp Σ)
  ...
  is_future γ ch -∗
  Await γ pending ={⊤}=∗
  Promise γ contract ∗ Await γ (pending ++ [contract]).
  Proof using IntoVal0 IntoValTyped0 V chan_idiomG0 ext ffi ffi_interp0 ffi_semantics0
ghost_mapG0 hG inG0 t Σ.
  iIntros "#Hmf HAwait".
...
Proof.
  ...
  iApply (future_fulfill_au with "[$Hch $Hinv] [$Hlc1 $Hlc2 $Hlc3 $HFulfill]").
  done.
Qed.

(** The core receive lemma. Each receive:
    1. Pulls a [Fulfill γ v] from the channel invariant
...
Proof.
  ...
  {
    iIntros (v_rcv) "HFulfill HAwait".
    iDestruct "HFulfill" as (contract_f) "[HPromise Hcontract_v]".
    iDestruct "HPromise" as (gn_f) "Hfrag_f".
    iDestruct "HAwait" as (pending_map) "(Hauth & %Hperm & Hfrags)".
  ...
  - (* Buffered *)
    ...
    + iDestruct "Hinv_open" as "[HFulfill_v HFulfills]".
      ...
      iMod ("Hmatch" with "HFulfill_v HAwait")
