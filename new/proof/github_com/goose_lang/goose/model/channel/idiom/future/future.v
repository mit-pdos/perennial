...
Definition is_future (γ : future_names) (ch : loc) : iProp Σ :=
  ...
  inv nroot (
    ∃ (s : chan_rep.t V),
      ...
      match s with
      | chan_rep.Buffered msgs => [∗ list] v ∈ msgs, Fulfilled γ v
      | chan_rep.SndPending v => Fulfilled γ v
      | chan_rep.SndCommit v => Fulfilled γ v
...
Proof.
  ...
  iMod (inv_alloc nroot _ (
    ∃ s',
      "Hch" ∷ own_chan ch s' γ ∗
      match s' with
      | chan_rep.Buffered msgs => [∗ list] v ∈ msgs, Fulfilled γmf v
      | chan_rep.SndPending v => Fulfilled γmf v
      | chan_rep.SndCommit v => Fulfilled γmf v
      | chan_rep.Idle | chan_rep.RcvPending | chan_rep.RcvCommit => True
      | _ => False
      end
  )%I with "[Hoc]") as "#Hinv".
