...
Definition is_done (γ : done_names) (ch : loc) : iProp Σ :=
  ...
  inv nroot (
    ∃ (s : chan_rep.t V) (m : gmap nat gname) (m_bc : gmap nat gname) (Qs: list (iProp Σ)) (duplicableQs : list (iProp Σ)),
...
iMod (saved_prop_update (R ∗ Q) with "Hsp") as "Hsp".
  destruct s; try iDestruct "Hstate" as ">Hstate"; try done.
  {
    ...
    {
      iNext. iExists chan_rep.Idle. iExists (<[i := prop_gname]> m'). iExists m_bc'. iFrame. iExists []. iExists [].
      done.
    }
    {
...
with "[HQs_bc Hfrag Hprop]" as "HQs_bc_ext".
{
  ...
  rewrite big_sepL_singleton.
  iFrame.
  replace (length duplicableQs + 0) with (length duplicableQs) by done.
  iFrame.
}
...
iMod (saved_prop_update (R ∗ Q) with "Hsp") as "Hsp".
  destruct s; try iDestruct "Hstate" as ">Hstate"; try done.
  {
    ...
    {
      iNext. iExists chan_rep.Idle. iExists m'. iExists (<[i := prop_gname]> m_bc'). iFrame. iExists []. iExists [].
      done.
    }
    {
      iModIntro. 
...
replace (length duplicableQs + 0) with (length duplicableQs) by lia.
simpl. 
      iFrame "#".
    }
  }
...
Lemma wp_make_done `{ BoundedTypeSize t}:
  {{{ True }}}
    chan.make #t #(W64 0)
  {{{ (ch: loc) (γdone: done_names), RET #ch;
      is_done γdone ch ∗
      Notify γdone True
  }}}.
Proof.
  iIntros (Φ) "_ HΦ".
  rewrite -wp_fupd.
   wp_apply (chan.wp_make). { set_solver. }
  iIntros (ch γ) "(Hch & %Hcap_eq & Hown)".
  iMod ((start_done ch γ) with "[$Hch] [$Hown]") 
  as (γdone) "(#Hdone & HNotify)".
  iModIntro. iApply "HΦ". iFrame "∗#". 
Qed.

Lemma wp_make_done_notified Q `{ BoundedTypeSize t}:
  {{{ True }}}
    chan.make #t #(W64 0)
  {{{ (ch: loc) (γdone: done_names), RET #ch;
      is_done γdone ch ∗
...
Proof.
  ...
  rewrite -wp_fupd.
   wp_apply (chan.wp_make). { set_solver. }
  iIntros (ch γ) "(Hch & %Hcap_eq & Hown)".
  iMod ((start_done_with_notified ch γ Q) with "[$Hch] [$Hown]") 
  as (γdone) "(#Hdone & HNotify & HBroadcastNotified)".
  iModIntro. iApply "HΦ". iFrame "∗#". 
Qed.


Lemma wp_make_done_with_broadcast_notified (Q : iProp Σ) `{BoundedTypeSize t} `{!Persistent Q} :
  {{{ True }}}
    chan.make #t #(W64 0)
  {{{ (ch: loc) (γdone: done_names), RET #ch;
      is_done γdone ch ∗
...
Proof.
  ...
  rewrite -wp_fupd.
   wp_apply (chan.wp_make). { set_solver. }
  iIntros (ch γ) "(Hch & %Hcap_eq & Hown)".
  iMod ((start_done_with_broadcast_notified ch γ Q) with "[$Hch] [$Hown]") 
  as (γdone) "(#Hdone & HNotify & HBroadcastNotified)".
  iModIntro. iApply "HΦ". iFrame "∗#". 
Qed.
...
Proof.
  ...
  - iIntros "Hoc".
    ...
    {
      iNext. iExists (chan_rep.Closed []), m', m_bc'. iFrame "Hmap_half Hmap_bc_half".
    ...
    }
    {
      ...
      destruct drain.
      {
        ...
        - iDestruct "H4" as "[Hprop_half x]".
          ...
          {
            iCombine "Hprop_half" "Hn2" as "H". iModIntro. iExists (chan_rep.Closed []). iFrame. iExists Qs0. iSplitL ""; first done. iSplitL ""; first done. iFrame. iApply "H2".
  ...
  destruct s; try done.
  {
    ...
    }
    {
      ...
      destruct drain.
      {
        ...
        {
          destruct (lt_dec i (length duplicableQs0)) as [H|Hge]; [exact H|].
          have Hge' : i ≥ length duplicableQs0 by lia.
          rewrite (Hbound_bc i Hge') in Hlookup. discriminate.
        }
        destruct (lookup_lt_is_Some_2 duplicableQs0 i Hi_lt) as [x Hx].
...
{
  iModIntro. iExists (chan_rep.Closed []). iFrame. iExists  duplicableQs0. 
...
iApply "HphiQ".
  ...
  }
  {
    ...
    destruct drain.
    {
      ...
      {
        destruct (lt_dec i (length duplicableQs)) as [H|Hge]; [exact H|].
        have Hge' : i ≥ length duplicableQs by lia.
        rewrite (Hbound_bc i Hge') in Hlookup. discriminate.
      }
      destruct (lookup_lt_is_Some_2 duplicableQs i Hi_lt) as [x Hx].
      iDestruct (big_sepL_lookup_acc _ _ i x Hx with "Hrecv_bc") as "[H H2]".
      iNamed "H". iDestruct "H" as "(H3 & H4 & HQ)".
      iDestruct (ghost_map_elem_agree with "Hn1 H3") as %Heq.
      subst prop_gname0.
...
iDestruct "HQ" as "#HQ".
      iMod ("Hinv_close" with "[Hoc H2 Hmap Hmap_bc Hgm Hgm_bc H3 H4 HQ Hrecv]") as "Hc".
      {
        iModIntro. iExists (chan_rep.Closed []). iFrame.
...
Proof.
  ...
  destruct s; try done.
  {
    ...
    - iApply fupd_mask_intro; [solve_ndisj|iIntros "Hmask"].
      ...
      {
        destruct (lt_dec i0 (length duplicableQs0)) as [H|Hge]; [exact H|].
        have Hge' : i0 ≥ length duplicableQs0 by lia.
        rewrite (Hbound_bc i0 Hge') in Hlookup_bc. discriminate.
      }
      destruct (lookup_lt_is_Some_2 duplicableQs0 i0 Hj_lt) as [y Hy].
  ...
  - iApply fupd_mask_intro; [solve_ndisj|iIntros "Hmask"].
    ...
    {
      destruct (lt_dec i0 (length duplicableQs)) as [H|Hge]; [exact H|].
      have Hge' : i0 ≥ length duplicableQs by lia.
      rewrite (Hbound_bc i0 Hge') in Hlookup_bc. discriminate.
    }
    destruct (lookup_lt_is_Some_2 duplicableQs i0 Hj_lt) as [y Hy].
    iDestruct (big_sepL_lookup_acc _ _ i0 y Hy with "Hrecv_bc") as "[HR HR_rest]".
    iNamed "HR". iDestruct "HR" as "(HR_frag & HR_saved & HR_data)".
    iDestruct (ghost_map_elem_agree with "Hb1 HR_frag") as %Heq_R.
    subst prop_gname0.
