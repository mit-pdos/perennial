...
Lemma wp_siftDownCmpFunc (data: slice.t) (lo hi a b: w64) (cmp_code: func.t)
  ...
  {{{ is_pkg_init slices ∗
    ...
    @! slices.siftDownCmpFunc #Et #data #lo #hi #a #cmp_code
  {{{ (xs': list E), RET #();
      "Hxs" ∷ data ↦* xs' ∗
...
Proof using StrictWeakOrder0 RelDecision0.
  ...
  with "[Hxs1]" as "%Hlen". { iApply own_slice_len. iFrame. }
  ...
  - replace (sint.Z (word.add (word.mul (W64 2) root_val) (W64 1))) with (2 * (sint.Z root_val) + 1) in n by abstract(word).
    assert(sint.Z (word.add a root_val) = sint.Z a + sint.Z root_val) by abstract(word).
    assert(∃ x : E, xs' !! ((sint.nat a) + 2 * (sint.nat root_val) + 1)%nat  = Some x). 
    { apply list_lookup_lt. lia. } destruct H1 as [xls Hls].
    assert(Hindex_ls: sint.nat (word.add a (word.add (word.mul (W64 2) root_val) (W64 1))) = ((sint.nat a) + 2 * (sint.nat root_val) + 1)%nat) by abstract(word).

    assert(∃ x : E, xs' !! (sint.nat a + sint.nat root_val)%nat = Some x). { apply list_lookup_lt. lia. }
    assert(Hindex_rt: sint.nat (word.add a root_val) = (sint.nat a + sint.nat root_val)%nat ) by abstract(word).
    destruct H1 as [xrt Hrt]. 
    wp_if_destruct. 
    + (* right-chld within bound, check data[right-child] *)
      replace (sint.Z (word.add (word.add (word.mul (W64 2) root_val) (W64 1)) (W64 1))) with (2 * (sint.Z root_val) + 2) in l; try abstract(word).
      wp_pure; first lia. (* Read left-child *)
      wp_apply (wp_load_slice_elem with "[$Hxs1]") as "Hxs1"; 
      ...
      assert(Hindex_rs1: sint.nat (word.add (word.add a (word.add (word.mul (W64 2) root_val) (W64 1))) (W64 1)) = ((sint.nat a) + 2 * (sint.nat root_val) + 2)%nat) by abstract(word).
      assert(Hindex_rs2: sint.nat (word.add a (word.add (word.add (word.mul (W64 2) root_val) (W64 1)) (W64 1))) = ((sint.nat a) + 2 * (sint.nat root_val) + 2)%nat) by abstract(word).

      wp_pure; first lia. (* Read right-child *)
      wp_apply (wp_load_slice_elem with "[$Hxs1]") as "Hxs1"; try rewrite Hindex_rs1; eauto; try lia.

      wp_apply "Hcmp". iIntros (r) "%Hcmp_r". wp_auto. wp_if_destruct.

      * (*.right-child greater *)
        wp_pure; first lia. (* read the root *)
        wp_apply (wp_load_slice_elem with "[$Hxs1]") as "Hxs1"; try rewrite Hindex_rt; eauto; first lia.

        wp_pure; first lia. (* read the right child *)
        wp_apply (wp_load_slice_elem with "[$Hxs1]") as "Hxs1"; try rewrite Hindex_rs2; eauto; first lia.
        ...
        -- (* root is not greatest, swap and continue *)
           ...
           wp_pure; first lia. (* read the root *)
           wp_apply (wp_load_slice_elem with "[$Hxs1]") as "Hxs1"; try rewrite Hindex_rt; eauto; first lia.
           wp_pure; first lia. (* write to the root *)
           wp_apply (wp_store_slice_elem with"[$Hxs1]") as "Hxs1"; try (iPureIntro; lia). 
           wp_pure; first lia. (* write to the right child *)
           wp_apply (wp_store_slice_elem with"[$Hxs1]") as "Hxs1".
           ...
           { split; first (clear HeapParent Heap0 HSegSorted Houtside; abstract(word)). 
      ...
      * (* left-child greater *)
        ...
        wp_apply (wp_load_slice_elem with "[$Hxs1]") as "Hxs1"; try rewrite Hindex_rt; auto; first lia.

        wp_pure; first lia. (* read the right child *)
        wp_apply (wp_load_slice_elem with "[$Hxs1]") as "Hxs1"; try rewrite Hindex_ls; auto; first lia.

        wp_apply "Hcmp". iIntros (r2) "%Hcmp_r2". wp_auto. wp_if_destruct.
        -- (* root is not greatest, swap and continue *)
           wp_pure; first lia. (* read the left-child *) 
           wp_apply (wp_load_slice_elem with "[$Hxs1]") as "Hxs1"; try rewrite Hindex_ls; eauto; first lia.
           wp_pure; first lia. (* read the root *)
           wp_apply (wp_load_slice_elem with "[$Hxs1]") as "Hxs1"; try rewrite Hindex_rt; eauto; first lia.
           ...
           wp_pure; first lia. (* write to the left-child *)
           wp_apply (wp_store_slice_elem with"[$Hxs1]") as "Hxs1". 
    ...
    + (* right-child out of bound; only consider left-child *)
      wp_pure; first lia. (* read the root val *)
      wp_apply (wp_load_slice_elem with "[$Hxs1]") as "Hxs1"; try rewrite Hindex_rt; eauto; first lia.
      wp_pure; first lia. (* read the left-child *)
      wp_apply (wp_load_slice_elem with "[$Hxs1]") as "Hxs1"; try rewrite Hindex_ls; eauto; first lia.
...
Lemma wp_siftDownCmpFunc_Trivial (data: slice.t) (a b: w64) (cmp_code: func.t)
  ...
  {{{ is_pkg_init slices ∗
    ...
    @! slices.siftDownCmpFunc #Et #data #(W64 0) #(W64 0) #a #cmp_code
  {{{ RET #();
      "Hxs" ∷ data ↦* xs 
  }}}.
Proof using RelDecision0 StrictWeakOrder0.
  wp_start as "H". iNamed "H". unfold is_heap_seg in *. wp_auto.  
  iAssert( 
    "hi" ∷ hi_ptr ↦ (W64 0) ∗ 
    "cmp" ∷ cmp_ptr ↦ cmp_code ∗ 
...
Lemma wp_heapSortCmpFunc (data: slice.t) (a b: w64) (cmp_code: func.t)
  ...
  {{{ is_pkg_init slices ∗
    ...
    @! slices.heapSortCmpFunc #Et #data #a #b #cmp_code
  {{{ (xs': list E), RET #();
      data ↦* xs' ∗
...
(W64 1)))) with (sint.nat b - sint.nat a)%nat by word.
    ...
    + assert(length xs = length xs'0) by (apply Permutation_length; auto).
      iAssert(⌜length xs'0 = sint.nat data .(slice.len_f) ∧ 0 ≤ sint.Z data .(slice.len_f)⌝)%I
      with "[Hxs1]" as "%Hlen". { iApply own_slice_len. iFrame. }
      wp_if_destruct.
      * assert(Hindex_i: sint.nat (word.add a i_val) = (sint.nat a + sint.nat i_val)%nat) by word.
        ...
        wp_pure; first word.
        wp_apply (wp_store_slice_elem with "[$Hxs1]") as "Hxs1"; first word.
