...
Lemma wp_insertionSortCmpFunc (data: slice.t) (a b: w64) (cmp_code: func.t)
                       (xs: list E):
  {{{ is_pkg_init slices ∗
      "Hxs" ∷ data ↦* xs ∗
    ...
    @! slices.insertionSortCmpFunc #Et #data #a #b #cmp_code
  {{{ (xs': list E), RET #();
      data ↦* xs' ∗
...
Proof using StrictWeakOrder0 RelDecision0.
  ...
  - (* Loop Invariant for the inner loop *)
    ...
    )%I with "[a b cmp i j Hxs1]" as "HI2".
    ...
    + assert(length xs = length xs''). { apply Permutation_length. auto. } 
      ...
      (* read data[j] *)
      wp_pure; first lia.   
      wp_apply (wp_load_slice_elem with "[$Hxs2]") as "Hxs2"; eauto; try lia.
      (* read data[j-1] *)
      wp_pure; first word.
      wp_apply (wp_load_slice_elem with "[$Hxs2]") as "Hxs2"; eauto; try word.
      ...
      * (* read old data[j-1] *)
        ...
        (* write new data[j] *)
        wp_pure; first lia.
        wp_apply (wp_store_slice_elem with "[$Hxs2]") as "Hxs2"; first word.

        (* write new data[j-1] *)
        wp_pure; first word.
        wp_apply (wp_store_slice_elem with "[$Hxs2]") as "Hxs2". { rewrite length_insert. word. }
...
Proof using StrictWeakOrder0 RelDecision0.
  ...
  wp_for "HI1". wp_if_destruct. {
      ...
      wp_for "HI2". wp_if_destruct. {
        ...
        wp_apply (wp_load_slice_elem with "[$Hxs1]") as "Hxs"; eauto; try lia.
        ...
        wp_apply (wp_load_slice_elem with "[$Hxs]") as "Hxs"; eauto; try word.
    ...
    wp_pure; first lia.
    wp_apply (wp_load_slice_elem with "[$Hxs]") as "Hxs"; eauto; try lia.

    (* write new data[j] *)
    wp_pure; first lia.
    wp_apply (wp_store_slice_elem with "[$Hxs]") as "Hxs"; first word.

    (* write new data[j-1] *)
    wp_pure; first word.
    wp_apply (wp_store_slice_elem with "[$Hxs]") as "Hxs". { rewrite length_insert. word. }
    ...
    wp_bind. iApply ((wp_wand _ _ _ (fun v => ⌜ v = execute_val ⌝ ∗ HI)%I) with "[a b Hxs i j0 data cmp]"). {
      wp_if_destruct. {
        ...
        wp_if_destruct. {
          ...
          wp_pure; first lia.
          wp_apply (wp_load_slice_elem with "[$Hxs2]") as "Hxs"; eauto; try lia.
          ...
          wp_apply "Hcmp". iIntros (r) "%Hcmp_r". wp_auto. wp_if_destruct. {
            ...
            wp_apply (wp_load_slice_elem with "[$Hxs]") as "Hxs"; eauto; try lia.
            wp_pure; first lia.
            wp_apply (wp_store_slice_elem with "[$Hxs]") as "Hxs"; first (iPureIntro; word).

            (* write new data[j-1] *)
            wp_pure; first word.
            wp_apply (wp_store_slice_elem with "[$Hxs]") as "Hxs". { rewrite length_insert. iPureIntro. lia. }
      ...
      wp_if_destruct. {
        ...
        wp_for "HI2". wp_if_destruct. {
          ...
          wp_apply "Hcmp". iIntros (r) "%Hcmp_r". wp_auto. wp_if_destruct. {
            ...
            (* write new data[j-1] *)
            wp_pure; first word.
            wp_apply (wp_store_slice_elem with "[$Hxs]") as "Hxs". { iPureIntro; lia. }

            wp_pure; first word.
            wp_apply (wp_store_slice_elem with "[$Hxs]") as "Hxs". 
