...
Lemma wp_reverseRangeCmpFunc (data: slice.t) (a b: w64) (cmp_code: func.t)
  ...
  {{{ is_pkg_init slices ∗
    ...
    #(functions slices.reverseRangeCmpFunc [Et]) #data #a #b #cmp_code
  {{{ (xs': list E), RET #();
      data ↦* xs' ∗
...
Proof using W.
  ...
  - assert(length xs = length xs1). { apply Permutation_length. auto. }
    ...
    wp_apply (wp_load_slice_index with "[$Hxs]") as "Hxs"; auto; first lia.
    ...
    wp_apply (wp_store_slice_index with "[$Hxs]") as "Hxs"; first (iPureIntro; lia).
...
iApply ((wp_wand _ _ _ (fun v => ∃ xs' (new_a: w64),
    ...
    )%I) with "[a b Hxs limit wasPartitioned wasBalanced data cmp pivot]").
  {
    ...
    rewrite -> decide_True; last word.
    wp_apply (wp_load_slice_index with "[$Hxs]") as "Hxs"; eauto; first word.
