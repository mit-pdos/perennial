(* autogenerated from testing *)
From New.golang Require Import defn.
Module pkg_id.
Definition testing : go_string := "testing".

End pkg_id.
Export pkg_id.
Module testing.

Definition durationOrCountFlag {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.durationOrCountFlag"%go [].

#[global] Opaque durationOrCountFlag.

Definition InternalBenchmark {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.InternalBenchmark"%go [].

#[global] Opaque InternalBenchmark.

Definition B {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.B"%go [].

#[global] Opaque B.

Definition BenchmarkResult {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.BenchmarkResult"%go [].

#[global] Opaque BenchmarkResult.

Definition benchState {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.benchState"%go [].

#[global] Opaque benchState.

Definition PB {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.PB"%go [].

#[global] Opaque PB.

Definition discard {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.discard"%go [].

#[global] Opaque discard.

Definition CoverBlock {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.CoverBlock"%go [].

#[global] Opaque CoverBlock.

Definition Cover {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.Cover"%go [].

#[global] Opaque Cover.

Definition InternalExample {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.InternalExample"%go [].

#[global] Opaque InternalExample.

Definition InternalFuzzTarget {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.InternalFuzzTarget"%go [].

#[global] Opaque InternalFuzzTarget.

Definition F {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.F"%go [].

#[global] Opaque F.

Definition fuzzResult {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.fuzzResult"%go [].

#[global] Opaque fuzzResult.

Definition fuzzCrashError {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.fuzzCrashError"%go [].

#[global] Opaque fuzzCrashError.

Definition fuzzState {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.fuzzState"%go [].

#[global] Opaque fuzzState.

Definition fuzzMode {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.fuzzMode"%go [].

#[global] Opaque fuzzMode.

Definition matcher {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.matcher"%go [].

#[global] Opaque matcher.

Definition filterMatch {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.filterMatch"%go [].

#[global] Opaque filterMatch.

Definition simpleMatch {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.simpleMatch"%go [].

#[global] Opaque simpleMatch.

Definition alternationMatch {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.alternationMatch"%go [].

#[global] Opaque alternationMatch.

Definition chattyFlag {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.chattyFlag"%go [].

#[global] Opaque chattyFlag.

Definition chattyPrinter {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.chattyPrinter"%go [].

#[global] Opaque chattyPrinter.

Definition common {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.common"%go [].

#[global] Opaque common.

Definition indenter {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.indenter"%go [].

#[global] Opaque indenter.

Definition TB {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.TB"%go [].

#[global] Opaque TB.

Definition T {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.T"%go [].

#[global] Opaque T.

Definition outputWriter {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.outputWriter"%go [].

#[global] Opaque outputWriter.

Definition panicHandling {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.panicHandling"%go [].

#[global] Opaque panicHandling.

Definition InternalTest {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.InternalTest"%go [].

#[global] Opaque InternalTest.

Definition testState {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.testState"%go [].

#[global] Opaque testState.

Definition matchStringOnly {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.matchStringOnly"%go [].

#[global] Opaque matchStringOnly.

Definition M {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.M"%go [].

#[global] Opaque M.

Definition testDeps {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.testDeps"%go [].

#[global] Opaque testDeps.

Definition highPrecisionTime {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "testing.highPrecisionTime"%go [].

#[global] Opaque highPrecisionTime.

Axiom durationOrCountFlagⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom InternalBenchmarkⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom Bⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom BenchmarkResultⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom benchStateⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom PBⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom discardⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom CoverBlockⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom Coverⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom InternalExampleⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom InternalFuzzTargetⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom Fⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom corpusEntry : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom fuzzResultⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom fuzzCrashErrorⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom fuzzStateⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom fuzzModeⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom matcherⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom filterMatchⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom simpleMatchⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom alternationMatchⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom chattyFlagⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom chattyPrinterⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom commonⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom indenterⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom TBⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom Tⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom outputWriterⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom panicHandlingⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom InternalTestⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom testStateⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom matchStringOnlyⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom Mⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom testDepsⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom highPrecisionTimeⁱᵐᵖˡ : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, go.type.

Axiom maxBenchPredictIters : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Axiom loopPoisonTimer : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Axiom loopPoisonMask : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Axiom fuzzWorkerExitCode : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Axiom seedCorpusOnly : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Axiom fuzzCoordinator : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Axiom fuzzWorker : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Axiom marker : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Axiom maxStackLen : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Axiom indent : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Axiom normalPanic : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Axiom recoverAndReturnPanic : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Axiom parallelConflict : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Definition matchBenchmarks {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.matchBenchmarks"%go.

Definition benchmarkMemory {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.benchmarkMemory"%go.

Definition benchTime {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.benchTime"%go.

Axiom benchTime'init : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Definition benchmarkLock {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.benchmarkLock"%go.

Definition memStats {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.memStats"%go.

Definition labelsOnce {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.labelsOnce"%go.

Definition hideStdoutForTesting {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.hideStdoutForTesting"%go.

Axiom hideStdoutForTesting'init : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Definition matchFuzz {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.matchFuzz"%go.

Definition fuzzDuration {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.fuzzDuration"%go.

Definition minimizeDuration {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.minimizeDuration"%go.

Axiom minimizeDuration'init : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Definition fuzzCacheDir {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.fuzzCacheDir"%go.

Definition isFuzzWorker {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.isFuzzWorker"%go.

Definition corpusDir {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.corpusDir"%go.

Axiom corpusDir'init : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Definition supportedTypes {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.supportedTypes"%go.

Axiom supportedTypes'init : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Definition matchMutex {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.matchMutex"%go.

Definition cover {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.cover"%go.

Definition initRan {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.initRan"%go.

Definition parallelStart {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.parallelStart"%go.

Definition parallelStop {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.parallelStop"%go.

Definition short {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.short"%go.

Definition failFast {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.failFast"%go.

Definition outputDir {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.outputDir"%go.

Definition artifacts {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.artifacts"%go.

Definition chatty {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.chatty"%go.

Definition count {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.count"%go.

Definition coverProfile {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.coverProfile"%go.

Definition gocoverdir {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.gocoverdir"%go.

Definition matchList {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.matchList"%go.

Definition match' {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.match"%go.

Definition skip {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.skip"%go.

Definition memProfile {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.memProfile"%go.

Definition memProfileRate {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.memProfileRate"%go.

Definition cpuProfile {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.cpuProfile"%go.

Definition blockProfile {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.blockProfile"%go.

Definition blockProfileRate {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.blockProfileRate"%go.

Definition mutexProfile {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.mutexProfile"%go.

Definition mutexProfileFraction {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.mutexProfileFraction"%go.

Definition panicOnExit0 {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.panicOnExit0"%go.

Definition traceFile {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.traceFile"%go.

Definition timeout {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.timeout"%go.

Definition cpuListStr {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.cpuListStr"%go.

Definition parallel {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.parallel"%go.

Definition shuffle {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.shuffle"%go.

Definition testlog {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.testlog"%go.

Definition fullPath {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.fullPath"%go.

Definition haveExamples {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.haveExamples"%go.

Definition cpuList {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.cpuList"%go.

Definition testlogFile {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.testlogFile"%go.

Definition artifactDir {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.artifactDir"%go.

Definition numFailed {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.numFailed"%go.

Definition running {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.running"%go.

Definition testBinary {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.testBinary"%go.

Axiom testBinary'init : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Definition errNilPanicOrGoexit {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.errNilPanicOrGoexit"%go.

Axiom errNilPanicOrGoexit'init : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Definition errMain {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.errMain"%go.

Axiom errMain'init : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Definition testingTesting {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.testingTesting"%go.

Definition realStderr {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.realStderr"%go.

Definition AllocsPerRun {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.AllocsPerRun"%go.

Definition initBenchmarkFlags {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.initBenchmarkFlags"%go.

Definition predictN {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.predictN"%go.

Definition prettyPrint {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.prettyPrint"%go.

Definition benchmarkName {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.benchmarkName"%go.

Definition RunBenchmarks {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.RunBenchmarks"%go.

Definition runBenchmarks {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.runBenchmarks"%go.

Definition Benchmark {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.Benchmark"%go.

Definition RegisterCover {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.RegisterCover"%go.

Definition RunExamples {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.RunExamples"%go.

Definition runExamples {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.runExamples"%go.

Definition initFuzzFlags {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.initFuzzFlags"%go.

Definition runFuzzTests {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.runFuzzTests"%go.

Definition runFuzzing {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.runFuzzing"%go.

Definition fRunner {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.fRunner"%go.

Definition allMatcher {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.allMatcher"%go.

Definition newMatcher {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.newMatcher"%go.

Definition splitRegexp {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.splitRegexp"%go.

Definition parseSubtestNumber {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.parseSubtestNumber"%go.

Definition rewrite {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.rewrite"%go.

Definition isSpace {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.isSpace"%go.

Definition registerCover {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.registerCover"%go.

Definition coverReport {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.coverReport"%go.

Definition Coverage {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.Coverage"%go.

Definition runExample {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.runExample"%go.

Definition Init {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.Init"%go.

Definition newChattyPrinter {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.newChattyPrinter"%go.

Definition Short {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.Short"%go.

Definition Testing {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.Testing"%go.

Definition CoverMode {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.CoverMode"%go.

Definition Verbose {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.Verbose"%go.

Definition fmtDuration {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.fmtDuration"%go.

Definition hashString {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.hashString"%go.

Definition removeSymbolsExcept {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.removeSymbolsExcept"%go.

Definition removeAll {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.removeAll"%go.

Definition callerName {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.callerName"%go.

Definition pcToName {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.pcToName"%go.

Definition checkParallel {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.checkParallel"%go.

Definition tRunner {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.tRunner"%go.

Definition testingSynctestTest {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.testingSynctestTest"%go.

Definition newTestState {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.newTestState"%go.

Definition Main {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.Main"%go.

Definition MainStart {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.MainStart"%go.

Definition listTests {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.listTests"%go.

Definition RunTests {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.RunTests"%go.

Definition runTests {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.runTests"%go.

Definition toOutputDir {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.toOutputDir"%go.

Definition runningList {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.runningList"%go.

Definition parseCpuList {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.parseCpuList"%go.

Definition shouldFailFast {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.shouldFailFast"%go.

Definition isWindowsRetryable {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.isWindowsRetryable"%go.

Definition highPrecisionTimeNow {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.highPrecisionTimeNow"%go.

Definition highPrecisionTimeSince {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "testing.highPrecisionTimeSince"%go.

#[global] Instance info' : PkgInfo pkg_id.testing :=
{|
  pkg_imported_pkgs := []
|}.

Axiom _'init : ∀ {ext : ffi_syntax} {go_gctx : GoGlobalContext}, val.

Definition initialize' {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: <>,
    package.init pkg_id.testing (λ: <>,
      exception_do (do:  (benchTime'init #());;;
      do:  (hideStdoutForTesting'init #());;;
      do:  (minimizeDuration'init #());;;
      do:  (corpusDir'init #());;;
      do:  (_'init #());;;
      do:  (supportedTypes'init #());;;
      do:  (testBinary'init #());;;
      do:  (_'init #());;;
      do:  (_'init #());;;
      do:  (errNilPanicOrGoexit'init #());;;
      do:  (errMain'init #()))
      ).

Module durationOrCountFlag.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End durationOrCountFlag.

Class durationOrCountFlag_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] durationOrCountFlag_type_repr  :: go.TypeReprUnderlying durationOrCountFlagⁱᵐᵖˡ durationOrCountFlag.t;
  #[global] durationOrCountFlag_underlying :: (durationOrCountFlag) <u (durationOrCountFlagⁱᵐᵖˡ);
  #[global] durationOrCountFlagⁱᵐᵖˡ_underlying :: (durationOrCountFlagⁱᵐᵖˡ) ↓u (durationOrCountFlagⁱᵐᵖˡ);
}.

Module InternalBenchmark.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End InternalBenchmark.

Class InternalBenchmark_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] InternalBenchmark_type_repr  :: go.TypeReprUnderlying InternalBenchmarkⁱᵐᵖˡ InternalBenchmark.t;
  #[global] InternalBenchmark_underlying :: (InternalBenchmark) <u (InternalBenchmarkⁱᵐᵖˡ);
  #[global] InternalBenchmarkⁱᵐᵖˡ_underlying :: (InternalBenchmarkⁱᵐᵖˡ) ↓u (InternalBenchmarkⁱᵐᵖˡ);
}.

Module B.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End B.

Class B_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] B_type_repr  :: go.TypeReprUnderlying Bⁱᵐᵖˡ B.t;
  #[global] B_underlying :: (B) <u (Bⁱᵐᵖˡ);
  #[global] Bⁱᵐᵖˡ_underlying :: (Bⁱᵐᵖˡ) ↓u (Bⁱᵐᵖˡ);
}.

Module BenchmarkResult.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End BenchmarkResult.

Class BenchmarkResult_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] BenchmarkResult_type_repr  :: go.TypeReprUnderlying BenchmarkResultⁱᵐᵖˡ BenchmarkResult.t;
  #[global] BenchmarkResult_underlying :: (BenchmarkResult) <u (BenchmarkResultⁱᵐᵖˡ);
  #[global] BenchmarkResultⁱᵐᵖˡ_underlying :: (BenchmarkResultⁱᵐᵖˡ) ↓u (BenchmarkResultⁱᵐᵖˡ);
}.

Module benchState.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End benchState.

Class benchState_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] benchState_type_repr  :: go.TypeReprUnderlying benchStateⁱᵐᵖˡ benchState.t;
  #[global] benchState_underlying :: (benchState) <u (benchStateⁱᵐᵖˡ);
  #[global] benchStateⁱᵐᵖˡ_underlying :: (benchStateⁱᵐᵖˡ) ↓u (benchStateⁱᵐᵖˡ);
}.

Module PB.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End PB.

Class PB_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] PB_type_repr  :: go.TypeReprUnderlying PBⁱᵐᵖˡ PB.t;
  #[global] PB_underlying :: (PB) <u (PBⁱᵐᵖˡ);
  #[global] PBⁱᵐᵖˡ_underlying :: (PBⁱᵐᵖˡ) ↓u (PBⁱᵐᵖˡ);
}.

Module discard.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End discard.

Class discard_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] discard_type_repr  :: go.TypeReprUnderlying discardⁱᵐᵖˡ discard.t;
  #[global] discard_underlying :: (discard) <u (discardⁱᵐᵖˡ);
  #[global] discardⁱᵐᵖˡ_underlying :: (discardⁱᵐᵖˡ) ↓u (discardⁱᵐᵖˡ);
}.

Module CoverBlock.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End CoverBlock.

Class CoverBlock_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] CoverBlock_type_repr  :: go.TypeReprUnderlying CoverBlockⁱᵐᵖˡ CoverBlock.t;
  #[global] CoverBlock_underlying :: (CoverBlock) <u (CoverBlockⁱᵐᵖˡ);
  #[global] CoverBlockⁱᵐᵖˡ_underlying :: (CoverBlockⁱᵐᵖˡ) ↓u (CoverBlockⁱᵐᵖˡ);
}.

Module Cover.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End Cover.

Class Cover_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] Cover_type_repr  :: go.TypeReprUnderlying Coverⁱᵐᵖˡ Cover.t;
  #[global] Cover_underlying :: (Cover) <u (Coverⁱᵐᵖˡ);
  #[global] Coverⁱᵐᵖˡ_underlying :: (Coverⁱᵐᵖˡ) ↓u (Coverⁱᵐᵖˡ);
}.

Module InternalExample.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End InternalExample.

Class InternalExample_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] InternalExample_type_repr  :: go.TypeReprUnderlying InternalExampleⁱᵐᵖˡ InternalExample.t;
  #[global] InternalExample_underlying :: (InternalExample) <u (InternalExampleⁱᵐᵖˡ);
  #[global] InternalExampleⁱᵐᵖˡ_underlying :: (InternalExampleⁱᵐᵖˡ) ↓u (InternalExampleⁱᵐᵖˡ);
}.

Module InternalFuzzTarget.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End InternalFuzzTarget.

Class InternalFuzzTarget_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] InternalFuzzTarget_type_repr  :: go.TypeReprUnderlying InternalFuzzTargetⁱᵐᵖˡ InternalFuzzTarget.t;
  #[global] InternalFuzzTarget_underlying :: (InternalFuzzTarget) <u (InternalFuzzTargetⁱᵐᵖˡ);
  #[global] InternalFuzzTargetⁱᵐᵖˡ_underlying :: (InternalFuzzTargetⁱᵐᵖˡ) ↓u (InternalFuzzTargetⁱᵐᵖˡ);
}.

Module F.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End F.

Class F_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] F_type_repr  :: go.TypeReprUnderlying Fⁱᵐᵖˡ F.t;
  #[global] F_underlying :: (F) <u (Fⁱᵐᵖˡ);
  #[global] Fⁱᵐᵖˡ_underlying :: (Fⁱᵐᵖˡ) ↓u (Fⁱᵐᵖˡ);
}.

Module fuzzResult.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End fuzzResult.

Class fuzzResult_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] fuzzResult_type_repr  :: go.TypeReprUnderlying fuzzResultⁱᵐᵖˡ fuzzResult.t;
  #[global] fuzzResult_underlying :: (fuzzResult) <u (fuzzResultⁱᵐᵖˡ);
  #[global] fuzzResultⁱᵐᵖˡ_underlying :: (fuzzResultⁱᵐᵖˡ) ↓u (fuzzResultⁱᵐᵖˡ);
}.

Module fuzzCrashError.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End fuzzCrashError.

Class fuzzCrashError_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] fuzzCrashError_type_repr  :: go.TypeReprUnderlying fuzzCrashErrorⁱᵐᵖˡ fuzzCrashError.t;
  #[global] fuzzCrashError_underlying :: (fuzzCrashError) <u (fuzzCrashErrorⁱᵐᵖˡ);
  #[global] fuzzCrashErrorⁱᵐᵖˡ_underlying :: (fuzzCrashErrorⁱᵐᵖˡ) ↓u (fuzzCrashErrorⁱᵐᵖˡ);
}.

Module fuzzState.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End fuzzState.

Class fuzzState_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] fuzzState_type_repr  :: go.TypeReprUnderlying fuzzStateⁱᵐᵖˡ fuzzState.t;
  #[global] fuzzState_underlying :: (fuzzState) <u (fuzzStateⁱᵐᵖˡ);
  #[global] fuzzStateⁱᵐᵖˡ_underlying :: (fuzzStateⁱᵐᵖˡ) ↓u (fuzzStateⁱᵐᵖˡ);
}.

Module fuzzMode.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End fuzzMode.

Class fuzzMode_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] fuzzMode_type_repr  :: go.TypeReprUnderlying fuzzModeⁱᵐᵖˡ fuzzMode.t;
  #[global] fuzzMode_underlying :: (fuzzMode) <u (fuzzModeⁱᵐᵖˡ);
  #[global] fuzzModeⁱᵐᵖˡ_underlying :: (fuzzModeⁱᵐᵖˡ) ↓u (fuzzModeⁱᵐᵖˡ);
}.

Module matcher.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End matcher.

Class matcher_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] matcher_type_repr  :: go.TypeReprUnderlying matcherⁱᵐᵖˡ matcher.t;
  #[global] matcher_underlying :: (matcher) <u (matcherⁱᵐᵖˡ);
  #[global] matcherⁱᵐᵖˡ_underlying :: (matcherⁱᵐᵖˡ) ↓u (matcherⁱᵐᵖˡ);
}.

Module filterMatch.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End filterMatch.

Class filterMatch_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] filterMatch_type_repr  :: go.TypeReprUnderlying filterMatchⁱᵐᵖˡ filterMatch.t;
  #[global] filterMatch_underlying :: (filterMatch) <u (filterMatchⁱᵐᵖˡ);
  #[global] filterMatchⁱᵐᵖˡ_underlying :: (filterMatchⁱᵐᵖˡ) ↓u (filterMatchⁱᵐᵖˡ);
}.

Module simpleMatch.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End simpleMatch.

Class simpleMatch_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] simpleMatch_type_repr  :: go.TypeReprUnderlying simpleMatchⁱᵐᵖˡ simpleMatch.t;
  #[global] simpleMatch_underlying :: (simpleMatch) <u (simpleMatchⁱᵐᵖˡ);
  #[global] simpleMatchⁱᵐᵖˡ_underlying :: (simpleMatchⁱᵐᵖˡ) ↓u (simpleMatchⁱᵐᵖˡ);
}.

Module alternationMatch.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End alternationMatch.

Class alternationMatch_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] alternationMatch_type_repr  :: go.TypeReprUnderlying alternationMatchⁱᵐᵖˡ alternationMatch.t;
  #[global] alternationMatch_underlying :: (alternationMatch) <u (alternationMatchⁱᵐᵖˡ);
  #[global] alternationMatchⁱᵐᵖˡ_underlying :: (alternationMatchⁱᵐᵖˡ) ↓u (alternationMatchⁱᵐᵖˡ);
}.

Module chattyFlag.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End chattyFlag.

Class chattyFlag_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] chattyFlag_type_repr  :: go.TypeReprUnderlying chattyFlagⁱᵐᵖˡ chattyFlag.t;
  #[global] chattyFlag_underlying :: (chattyFlag) <u (chattyFlagⁱᵐᵖˡ);
  #[global] chattyFlagⁱᵐᵖˡ_underlying :: (chattyFlagⁱᵐᵖˡ) ↓u (chattyFlagⁱᵐᵖˡ);
}.

Module chattyPrinter.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End chattyPrinter.

Class chattyPrinter_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] chattyPrinter_type_repr  :: go.TypeReprUnderlying chattyPrinterⁱᵐᵖˡ chattyPrinter.t;
  #[global] chattyPrinter_underlying :: (chattyPrinter) <u (chattyPrinterⁱᵐᵖˡ);
  #[global] chattyPrinterⁱᵐᵖˡ_underlying :: (chattyPrinterⁱᵐᵖˡ) ↓u (chattyPrinterⁱᵐᵖˡ);
}.

Module common.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End common.

Class common_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] common_type_repr  :: go.TypeReprUnderlying commonⁱᵐᵖˡ common.t;
  #[global] common_underlying :: (common) <u (commonⁱᵐᵖˡ);
  #[global] commonⁱᵐᵖˡ_underlying :: (commonⁱᵐᵖˡ) ↓u (commonⁱᵐᵖˡ);
}.

Module indenter.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End indenter.

Class indenter_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] indenter_type_repr  :: go.TypeReprUnderlying indenterⁱᵐᵖˡ indenter.t;
  #[global] indenter_underlying :: (indenter) <u (indenterⁱᵐᵖˡ);
  #[global] indenterⁱᵐᵖˡ_underlying :: (indenterⁱᵐᵖˡ) ↓u (indenterⁱᵐᵖˡ);
}.

Module TB.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End TB.

Class TB_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] TB_type_repr  :: go.TypeReprUnderlying TBⁱᵐᵖˡ TB.t;
  #[global] TB_underlying :: (TB) <u (TBⁱᵐᵖˡ);
  #[global] TBⁱᵐᵖˡ_underlying :: (TBⁱᵐᵖˡ) ↓u (TBⁱᵐᵖˡ);
}.

Module T.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End T.

Class T_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] T_type_repr  :: go.TypeReprUnderlying Tⁱᵐᵖˡ T.t;
  #[global] T_underlying :: (T) <u (Tⁱᵐᵖˡ);
  #[global] Tⁱᵐᵖˡ_underlying :: (Tⁱᵐᵖˡ) ↓u (Tⁱᵐᵖˡ);
}.

Module outputWriter.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End outputWriter.

Class outputWriter_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] outputWriter_type_repr  :: go.TypeReprUnderlying outputWriterⁱᵐᵖˡ outputWriter.t;
  #[global] outputWriter_underlying :: (outputWriter) <u (outputWriterⁱᵐᵖˡ);
  #[global] outputWriterⁱᵐᵖˡ_underlying :: (outputWriterⁱᵐᵖˡ) ↓u (outputWriterⁱᵐᵖˡ);
}.

Module panicHandling.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End panicHandling.

Class panicHandling_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] panicHandling_type_repr  :: go.TypeReprUnderlying panicHandlingⁱᵐᵖˡ panicHandling.t;
  #[global] panicHandling_underlying :: (panicHandling) <u (panicHandlingⁱᵐᵖˡ);
  #[global] panicHandlingⁱᵐᵖˡ_underlying :: (panicHandlingⁱᵐᵖˡ) ↓u (panicHandlingⁱᵐᵖˡ);
}.

Module InternalTest.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End InternalTest.

Class InternalTest_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] InternalTest_type_repr  :: go.TypeReprUnderlying InternalTestⁱᵐᵖˡ InternalTest.t;
  #[global] InternalTest_underlying :: (InternalTest) <u (InternalTestⁱᵐᵖˡ);
  #[global] InternalTestⁱᵐᵖˡ_underlying :: (InternalTestⁱᵐᵖˡ) ↓u (InternalTestⁱᵐᵖˡ);
}.

Module testState.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End testState.

Class testState_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] testState_type_repr  :: go.TypeReprUnderlying testStateⁱᵐᵖˡ testState.t;
  #[global] testState_underlying :: (testState) <u (testStateⁱᵐᵖˡ);
  #[global] testStateⁱᵐᵖˡ_underlying :: (testStateⁱᵐᵖˡ) ↓u (testStateⁱᵐᵖˡ);
}.

Module matchStringOnly.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End matchStringOnly.

Class matchStringOnly_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] matchStringOnly_type_repr  :: go.TypeReprUnderlying matchStringOnlyⁱᵐᵖˡ matchStringOnly.t;
  #[global] matchStringOnly_underlying :: (matchStringOnly) <u (matchStringOnlyⁱᵐᵖˡ);
  #[global] matchStringOnlyⁱᵐᵖˡ_underlying :: (matchStringOnlyⁱᵐᵖˡ) ↓u (matchStringOnlyⁱᵐᵖˡ);
}.

Module M.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End M.

Class M_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] M_type_repr  :: go.TypeReprUnderlying Mⁱᵐᵖˡ M.t;
  #[global] M_underlying :: (M) <u (Mⁱᵐᵖˡ);
  #[global] Mⁱᵐᵖˡ_underlying :: (Mⁱᵐᵖˡ) ↓u (Mⁱᵐᵖˡ);
}.

Module testDeps.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End testDeps.

Class testDeps_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] testDeps_type_repr  :: go.TypeReprUnderlying testDepsⁱᵐᵖˡ testDeps.t;
  #[global] testDeps_underlying :: (testDeps) <u (testDepsⁱᵐᵖˡ);
  #[global] testDepsⁱᵐᵖˡ_underlying :: (testDepsⁱᵐᵖˡ) ↓u (testDepsⁱᵐᵖˡ);
}.

Module highPrecisionTime.
Section def.
Context {ext : ffi_syntax} {go_gctx : GoGlobalContext}.
Axiom t : Type.
Axiom zero_val : ZeroVal t.
#[global] Existing Instance zero_val.
End def.
End highPrecisionTime.

Class highPrecisionTime_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] highPrecisionTime_type_repr  :: go.TypeReprUnderlying highPrecisionTimeⁱᵐᵖˡ highPrecisionTime.t;
  #[global] highPrecisionTime_underlying :: (highPrecisionTime) <u (highPrecisionTimeⁱᵐᵖˡ);
  #[global] highPrecisionTimeⁱᵐᵖˡ_underlying :: (highPrecisionTimeⁱᵐᵖˡ) ↓u (highPrecisionTimeⁱᵐᵖˡ);
}.

Class Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  #[global] durationOrCountFlag_instance :: durationOrCountFlag_Assumptions;
  #[global] InternalBenchmark_instance :: InternalBenchmark_Assumptions;
  #[global] B_instance :: B_Assumptions;
  #[global] BenchmarkResult_instance :: BenchmarkResult_Assumptions;
  #[global] benchState_instance :: benchState_Assumptions;
  #[global] PB_instance :: PB_Assumptions;
  #[global] discard_instance :: discard_Assumptions;
  #[global] CoverBlock_instance :: CoverBlock_Assumptions;
  #[global] Cover_instance :: Cover_Assumptions;
  #[global] InternalExample_instance :: InternalExample_Assumptions;
  #[global] InternalFuzzTarget_instance :: InternalFuzzTarget_Assumptions;
  #[global] F_instance :: F_Assumptions;
  #[global] fuzzResult_instance :: fuzzResult_Assumptions;
  #[global] fuzzCrashError_instance :: fuzzCrashError_Assumptions;
  #[global] fuzzState_instance :: fuzzState_Assumptions;
  #[global] fuzzMode_instance :: fuzzMode_Assumptions;
  #[global] matcher_instance :: matcher_Assumptions;
  #[global] filterMatch_instance :: filterMatch_Assumptions;
  #[global] simpleMatch_instance :: simpleMatch_Assumptions;
  #[global] alternationMatch_instance :: alternationMatch_Assumptions;
  #[global] chattyFlag_instance :: chattyFlag_Assumptions;
  #[global] chattyPrinter_instance :: chattyPrinter_Assumptions;
  #[global] common_instance :: common_Assumptions;
  #[global] indenter_instance :: indenter_Assumptions;
  #[global] TB_instance :: TB_Assumptions;
  #[global] T_instance :: T_Assumptions;
  #[global] outputWriter_instance :: outputWriter_Assumptions;
  #[global] panicHandling_instance :: panicHandling_Assumptions;
  #[global] InternalTest_instance :: InternalTest_Assumptions;
  #[global] testState_instance :: testState_Assumptions;
  #[global] matchStringOnly_instance :: matchStringOnly_Assumptions;
  #[global] M_instance :: M_Assumptions;
  #[global] testDeps_instance :: testDeps_Assumptions;
  #[global] highPrecisionTime_instance :: highPrecisionTime_Assumptions;
}.
End testing.
