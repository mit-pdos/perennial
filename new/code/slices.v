...
Definition SortFuncⁱᵐᵖˡ : val :=
  λ: "S" "E" "x" "cmp",
    ...
    let: "$a1" := #(W64 0) in
    let: "$a2" := (![#intT] "n") in
    let: "$a3" := (let: "$a0" := (s_to_w64 (![#intT] "n")) in
    (func_call #bits.Len) "$a0") in
    let: "$a4" := (![#funcT] "cmp") in
...
#[global] Instance info' : PkgInfo slices.slices :=
  {|
    ...
    pkg_msets := msets';
    pkg_imported_pkgs := [code.math.bits.bits];
  |}.
...
Definition initialize' : val :=
  λ: <>,
    package.init #slices.slices (λ: <>,
      ...
      do:  (package.alloc slices.slices #()))
      ).
