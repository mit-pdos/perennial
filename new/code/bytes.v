...
Definition initialize' {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: <>,
    package.init pkg_id.bytes (λ: <>,
      ...
      do:  (go.GlobalAlloc errUnreadByte go.error #());;;
      do:  (go.GlobalAlloc errNegativeRead go.error #());;;
      do:  (go.GlobalAlloc ErrTooLarge go.error #());;;
      do:  (errors.initialize' #());;;
      let: "$r0" := (let: "$a0" := #"bytes.Buffer: too large"%go in
      (FuncResolve errors.New [] #()) "$a0") in
      do:  ((GlobalVarAddr ErrTooLarge #()) <-[go.error] "$r0");;;
      let: "$r0" := (let: "$a0" := #"bytes.Buffer: reader returned negative count from Read"%go in
      (FuncResolve errors.New [] #()) "$a0") in
      do:  ((GlobalVarAddr errNegativeRead #()) <-[go.error] "$r0");;;
      let: "$r0" := (let: "$a0" := #"bytes.Buffer: UnreadByte: previous operation was not a successful read"%go in
      (FuncResolve errors.New [] #()) "$a0") in
      do:  ((GlobalVarAddr errUnreadByte #()) <-[go.error] "$r0");;;
