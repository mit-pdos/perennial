...
Definition Time__nsecⁱᵐᵖˡ : val :=
  λ: "t" <>,
    ...
    return: (u_to_w32 ((![uint64T] (struct.field_ref ptrT #"wall"%go (![ptrT] "t"))) `and` #(W64 nsecMask)))).
...
Definition Time__secⁱᵐᵖˡ : val :=
  λ: "t" <>,
    exception_do (let: "t" := (mem.alloc "t") in
    (if: ((![uint64T] (struct.field_ref ptrT #"wall"%go (![ptrT] "t"))) `and` #(W64 hasMonotonic)) ≠ #(W64 0)
    then return: (wallToInternal + (u_to_w64 (((![uint64T] (struct.field_ref ptrT #"wall"%go (![ptrT] "t"))) ≪ #(W64 1)) ≫ #(W64 (nsecShift + 1)))))
    else do:  #());;;
    return: (![int64T] (struct.field_ref ptrT #"ext"%go (![ptrT] "t")))).
