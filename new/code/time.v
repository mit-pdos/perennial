...
{|
  pkg_imported_pkgs := [code.errors.pkg_id.errors]
|}.
...
Definition initialize' {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: <>,
    package.init pkg_id.time (λ: <>,
      exception_do (do:  (go.GlobalAlloc platformZoneSources (go.SliceType go.string) #());;;
      do:  (go.GlobalAlloc errBadData go.error #());;;
      do:  (go.GlobalAlloc errLocation go.error #());;;
      do:  (go.GlobalAlloc Local (go.PointerType Location) #());;;
      do:  (go.GlobalAlloc utcLoc Location #());;;
      do:  (go.GlobalAlloc UTC (go.PointerType Location) #());;;
      do:  (go.GlobalAlloc startNano go.int64 #());;;
      do:  (go.GlobalAlloc unitMap (go.MapType go.string go.uint64) #());;;
      do:  (go.GlobalAlloc errLeadingInt go.error #());;;
      do:  (go.GlobalAlloc errBad go.error #());;;
      do:  (go.GlobalAlloc errAtoi go.error #());;;
      do:  (go.GlobalAlloc longMonthNames (go.SliceType go.string) #());;;
      do:  (go.GlobalAlloc shortMonthNames (go.SliceType go.string) #());;;
      do:  (go.GlobalAlloc shortDayNames (go.SliceType go.string) #());;;
      do:  (go.GlobalAlloc longDayNames (go.SliceType go.string) #());;;
      do:  (go.GlobalAlloc std0x (go.ArrayType 6 go.int) #());;;
      do:  (errors.initialize' #());;;
      let: "$r0" := (let: "$v0" := (Convert go.untyped_int go.int stdZeroMonth) in
      let: "$v1" := (Convert go.untyped_int go.int stdZeroDay) in
      let: "$v2" := (Convert go.untyped_int go.int stdZeroHour12) in
      let: "$v3" := (Convert go.untyped_int go.int stdZeroMinute) in
      let: "$v4" := (Convert go.untyped_int go.int stdZeroSecond) in
      let: "$v5" := (Convert go.untyped_int go.int stdYear) in
      CompositeLiteral (go.ArrayType 6 go.int) (LiteralValue [KeyedElement None (ElementExpression go.int "$v0"); KeyedElement None (ElementExpression go.int "$v1"); KeyedElement None (ElementExpression go.int "$v2"); KeyedElement None (ElementExpression go.int "$v3"); KeyedElement None (ElementExpression go.int "$v4"); KeyedElement None (ElementExpression go.int "$v5")])) in
      do:  ((GlobalVarAddr std0x #()) <-[go.ArrayType 6 go.int] "$r0");;;
      let: "$r0" := (let: "$v0" := #"Sunday"%go in
      let: "$v1" := #"Monday"%go in
      let: "$v2" := #"Tuesday"%go in
      let: "$v3" := #"Wednesday"%go in
      let: "$v4" := #"Thursday"%go in
      let: "$v5" := #"Friday"%go in
      let: "$v6" := #"Saturday"%go in
      ...
      do:  ((GlobalVarAddr longDayNames #()) <-[go.SliceType go.string] "$r0");;;
      let: "$r0" := (let: "$v0" := #"Sun"%go in
      let: "$v1" := #"Mon"%go in
      let: "$v2" := #"Tue"%go in
      let: "$v3" := #"Wed"%go in
      let: "$v4" := #"Thu"%go in
      ...
      let: "$v6" := #"Sat"%go in
      CompositeLiteral (go.SliceType go.string) (LiteralValue [KeyedElement None (ElementExpression go.string "$v0"); KeyedElement None (ElementExpression go.string "$v1"); KeyedElement None (ElementExpression go.string "$v2"); KeyedElement None (ElementExpression go.string "$v3"); KeyedElement None (ElementExpression go.string "$v4"); KeyedElement None (ElementExpression go.string "$v5"); KeyedElement None (ElementExpression go.string "$v6")])) in
      do:  ((GlobalVarAddr shortDayNames #()) <-[go.SliceType go.string] "$r0");;;
      let: "$r0" := (let: "$v0" := #"Jan"%go in
      let: "$v1" := #"Feb"%go in
      let: "$v2" := #"Mar"%go in
      let: "$v3" := #"Apr"%go in
      let: "$v4" := #"May"%go in
      ...
      do:  ((GlobalVarAddr shortMonthNames #()) <-[go.SliceType go.string] "$r0");;;
      let: "$r0" := (let: "$v0" := #"January"%go in
      let: "$v1" := #"February"%go in
      let: "$v2" := #"March"%go in
      let: "$v3" := #"April"%go in
      let: "$v4" := #"May"%go in
      let: "$v5" := #"June"%go in
      let: "$v6" := #"July"%go in
      let: "$v7" := #"August"%go in
      let: "$v8" := #"September"%go in
      let: "$v9" := #"October"%go in
      let: "$v10" := #"November"%go in
      let: "$v11" := #"December"%go in
      CompositeLiteral (go.SliceType go.string) (LiteralValue [KeyedElement None (ElementExpression go.string "$v0"); KeyedElement None (ElementExpression go.string "$v1"); KeyedElement None (ElementExpression go.string "$v2"); KeyedElement None (ElementExpression go.string "$v3"); KeyedElement None (ElementExpression go.string "$v4"); KeyedElement None (ElementExpression go.string "$v5"); KeyedElement None (ElementExpression go.string "$v6"); KeyedElement None (ElementExpression go.string "$v7"); KeyedElement None (ElementExpression go.string "$v8"); KeyedElement None (ElementExpression go.string "$v9"); KeyedElement None (ElementExpression go.string "$v10"); KeyedElement None (ElementExpression go.string "$v11")])) in
      do:  ((GlobalVarAddr longMonthNames #()) <-[go.SliceType go.string] "$r0");;;
      let: "$r0" := (let: "$a0" := #"time: invalid number"%go in
      (FuncResolve errors.New [] #()) "$a0") in
      do:  ((GlobalVarAddr errAtoi #()) <-[go.error] "$r0");;;
      let: "$r0" := (let: "$a0" := #"bad value for field"%go in
      (FuncResolve errors.New [] #()) "$a0") in
      do:  ((GlobalVarAddr errBad #()) <-[go.error] "$r0");;;
      let: "$r0" := (let: "$a0" := #"time: bad [0-9]*"%go in
      (FuncResolve errors.New [] #()) "$a0") in
      do:  ((GlobalVarAddr errLeadingInt #()) <-[go.error] "$r0");;;
      let: "$r0" := (let: "$k0" := #"ns"%go in
      let: "$v1" := (Convert Duration go.uint64 Nanosecond) in
      let: "$k2" := #"us"%go in
      let: "$v3" := (Convert Duration go.uint64 Microsecond) in
      let: "$k4" := #"µs"%go in
      let: "$v5" := (Convert Duration go.uint64 Microsecond) in
      let: "$k6" := #"μs"%go in
      let: "$v7" := (Convert Duration go.uint64 Microsecond) in
      let: "$k8" := #"ms"%go in
      let: "$v9" := (Convert Duration go.uint64 Millisecond) in
      let: "$k10" := #"s"%go in
      let: "$v11" := (Convert Duration go.uint64 Second) in
      let: "$k12" := #"m"%go in
      let: "$v13" := (Convert Duration go.uint64 Minute) in
      let: "$k14" := #"h"%go in
      let: "$v15" := (Convert Duration go.uint64 Hour) in
      CompositeLiteral (go.MapType go.string go.uint64) (LiteralValue [KeyedElement (Some (KeyExpression go.string "$k0")) (ElementExpression go.uint64 "$v1"); KeyedElement (Some (KeyExpression go.string "$k2")) (ElementExpression go.uint64 "$v3"); KeyedElement (Some (KeyExpression go.string "$k4")) (ElementExpression go.uint64 "$v5"); KeyedElement (Some (KeyExpression go.string "$k6")) (ElementExpression go.uint64 "$v7"); KeyedElement (Some (KeyExpression go.string "$k8")) (ElementExpression go.uint64 "$v9"); KeyedElement (Some (KeyExpression go.string "$k10")) (ElementExpression go.uint64 "$v11"); KeyedElement (Some (KeyExpression go.string "$k12")) (ElementExpression go.uint64 "$v13"); KeyedElement (Some (KeyExpression go.string "$k14")) (ElementExpression go.uint64 "$v15")])) in
      do:  ((GlobalVarAddr unitMap #()) <-[go.MapType go.string go.uint64] "$r0");;;
      do:  (asynctimerchan'init #());;;
      let: "$r0" := (((FuncResolve runtimeNano [] #()) #()) -⟨go.int64⟩ #(W64 1)) in
      do:  ((GlobalVarAddr startNano #()) <-[go.int64] "$r0");;;
      let: "$r0" := (let: "$v0" := #"UTC"%go in
      CompositeLiteral Location (LiteralValue [KeyedElement (Some (KeyField "name"%go)) (ElementExpression go.string "$v0")])) in
      do:  ((GlobalVarAddr utcLoc #()) <-[Location] "$r0");;;
      let: "$r0" := (GlobalVarAddr utcLoc #()) in
      do:  ((GlobalVarAddr UTC #()) <-[go.PointerType Location] "$r0");;;
      let: "$r0" := (GlobalVarAddr localLoc #()) in
      do:  ((GlobalVarAddr Local #()) <-[go.PointerType Location] "$r0");;;
      let: "$r0" := (let: "$a0" := #"time: invalid location name"%go in
      (FuncResolve errors.New [] #()) "$a0") in
      do:  ((GlobalVarAddr errLocation #()) <-[go.error] "$r0");;;
      let: "$r0" := (let: "$a0" := #"malformed time zone information"%go in
      (FuncResolve errors.New [] #()) "$a0") in
      do:  ((GlobalVarAddr errBadData #()) <-[go.error] "$r0");;;
      let: "$r0" := (let: "$v0" := #"/usr/share/zoneinfo/"%go in
      let: "$v1" := #"/usr/share/lib/zoneinfo/"%go in
      let: "$v2" := #"/usr/lib/locale/TZ/"%go in
      let: "$v3" := #"/etc/zoneinfo"%go in
      ...
      do:  ((GlobalVarAddr platformZoneSources #()) <-[go.SliceType go.string] "$r0"))
      ).
...
Definition Timer'fds_unsealed {ext : ffi_syntax} {go_gctx : GoGlobalContext} : list go.field_decl := [
  (go.FieldDecl "C"%go (go.ChannelType go.recvonly Time));
  (go.FieldDecl "initTimer"%go go.bool)
].
Program Definition Timer'fds {ext : ffi_syntax} {go_gctx : GoGlobalContext} := sealed (Timer'fds_unsealed).
Global Instance equals_unfold_Timer {ext : ffi_syntax} {go_gctx : GoGlobalContext} : Timer'fds =→ Timer'fds_unsealed.
Proof. rewrite /Timer'fds seal_eq //. Qed.

Definition Timerⁱᵐᵖˡ {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.StructType (Timer'fds).
...
{
  #[global] Month_type_repr  :: go.TypeReprUnderlying Monthⁱᵐᵖˡ Month.t;
  #[global] Month_underlying :: (Month) <u (Monthⁱᵐᵖˡ);
  #[global] Monthⁱᵐᵖˡ_underlying :: (Monthⁱᵐᵖˡ) ↓u (Monthⁱᵐᵖˡ);
}.
...
{
  #[global] Weekday_type_repr  :: go.TypeReprUnderlying Weekdayⁱᵐᵖˡ Weekday.t;
  #[global] Weekday_underlying :: (Weekday) <u (Weekdayⁱᵐᵖˡ);
  #[global] Weekdayⁱᵐᵖˡ_underlying :: (Weekdayⁱᵐᵖˡ) ↓u (Weekdayⁱᵐᵖˡ);
}.
...
{
  #[global] absSeconds_type_repr  :: go.TypeReprUnderlying absSecondsⁱᵐᵖˡ absSeconds.t;
  #[global] absSeconds_underlying :: (absSeconds) <u (absSecondsⁱᵐᵖˡ);
  #[global] absSecondsⁱᵐᵖˡ_underlying :: (absSecondsⁱᵐᵖˡ) ↓u (absSecondsⁱᵐᵖˡ);
}.
...
{
  #[global] absDays_type_repr  :: go.TypeReprUnderlying absDaysⁱᵐᵖˡ absDays.t;
  #[global] absDays_underlying :: (absDays) <u (absDaysⁱᵐᵖˡ);
  #[global] absDaysⁱᵐᵖˡ_underlying :: (absDaysⁱᵐᵖˡ) ↓u (absDaysⁱᵐᵖˡ);
}.
...
{
  #[global] absCentury_type_repr  :: go.TypeReprUnderlying absCenturyⁱᵐᵖˡ absCentury.t;
  #[global] absCentury_underlying :: (absCentury) <u (absCenturyⁱᵐᵖˡ);
  #[global] absCenturyⁱᵐᵖˡ_underlying :: (absCenturyⁱᵐᵖˡ) ↓u (absCenturyⁱᵐᵖˡ);
}.
...
{
  #[global] absCyear_type_repr  :: go.TypeReprUnderlying absCyearⁱᵐᵖˡ absCyear.t;
  #[global] absCyear_underlying :: (absCyear) <u (absCyearⁱᵐᵖˡ);
  #[global] absCyearⁱᵐᵖˡ_underlying :: (absCyearⁱᵐᵖˡ) ↓u (absCyearⁱᵐᵖˡ);
}.
...
{
  #[global] absYday_type_repr  :: go.TypeReprUnderlying absYdayⁱᵐᵖˡ absYday.t;
  #[global] absYday_underlying :: (absYday) <u (absYdayⁱᵐᵖˡ);
  #[global] absYdayⁱᵐᵖˡ_underlying :: (absYdayⁱᵐᵖˡ) ↓u (absYdayⁱᵐᵖˡ);
}.
...
{
  #[global] absMonth_type_repr  :: go.TypeReprUnderlying absMonthⁱᵐᵖˡ absMonth.t;
  #[global] absMonth_underlying :: (absMonth) <u (absMonthⁱᵐᵖˡ);
  #[global] absMonthⁱᵐᵖˡ_underlying :: (absMonthⁱᵐᵖˡ) ↓u (absMonthⁱᵐᵖˡ);
}.
...
{
  #[global] absLeap_type_repr  :: go.TypeReprUnderlying absLeapⁱᵐᵖˡ absLeap.t;
  #[global] absLeap_underlying :: (absLeap) <u (absLeapⁱᵐᵖˡ);
  #[global] absLeapⁱᵐᵖˡ_underlying :: (absLeapⁱᵐᵖˡ) ↓u (absLeapⁱᵐᵖˡ);
}.
...
{
  #[global] absJanFeb_type_repr  :: go.TypeReprUnderlying absJanFebⁱᵐᵖˡ absJanFeb.t;
  #[global] absJanFeb_underlying :: (absJanFeb) <u (absJanFebⁱᵐᵖˡ);
  #[global] absJanFebⁱᵐᵖˡ_underlying :: (absJanFebⁱᵐᵖˡ) ↓u (absJanFebⁱᵐᵖˡ);
}.
...
{
  #[global] Location_type_repr  :: go.TypeReprUnderlying Locationⁱᵐᵖˡ Location.t;
  #[global] Location_underlying :: (Location) <u (Locationⁱᵐᵖˡ);
  #[global] Locationⁱᵐᵖˡ_underlying :: (Locationⁱᵐᵖˡ) ↓u (Locationⁱᵐᵖˡ);
}.
...
{
  #[global] zone_type_repr  :: go.TypeReprUnderlying zoneⁱᵐᵖˡ zone.t;
  #[global] zone_underlying :: (zone) <u (zoneⁱᵐᵖˡ);
  #[global] zoneⁱᵐᵖˡ_underlying :: (zoneⁱᵐᵖˡ) ↓u (zoneⁱᵐᵖˡ);
}.
...
{
  #[global] zoneTrans_type_repr  :: go.TypeReprUnderlying zoneTransⁱᵐᵖˡ zoneTrans.t;
  #[global] zoneTrans_underlying :: (zoneTrans) <u (zoneTransⁱᵐᵖˡ);
  #[global] zoneTransⁱᵐᵖˡ_underlying :: (zoneTransⁱᵐᵖˡ) ↓u (zoneTransⁱᵐᵖˡ);
}.
...
{
  #[global] ruleKind_type_repr  :: go.TypeReprUnderlying ruleKindⁱᵐᵖˡ ruleKind.t;
  #[global] ruleKind_underlying :: (ruleKind) <u (ruleKindⁱᵐᵖˡ);
  #[global] ruleKindⁱᵐᵖˡ_underlying :: (ruleKindⁱᵐᵖˡ) ↓u (ruleKindⁱᵐᵖˡ);
}.
...
{
  #[global] rule_type_repr  :: go.TypeReprUnderlying ruleⁱᵐᵖˡ rule.t;
  #[global] rule_underlying :: (rule) <u (ruleⁱᵐᵖˡ);
  #[global] ruleⁱᵐᵖˡ_underlying :: (ruleⁱᵐᵖˡ) ↓u (ruleⁱᵐᵖˡ);
}.
...
{
  #[global] fileSizeError_type_repr  :: go.TypeReprUnderlying fileSizeErrorⁱᵐᵖˡ fileSizeError.t;
  #[global] fileSizeError_underlying :: (fileSizeError) <u (fileSizeErrorⁱᵐᵖˡ);
  #[global] fileSizeErrorⁱᵐᵖˡ_underlying :: (fileSizeErrorⁱᵐᵖˡ) ↓u (fileSizeErrorⁱᵐᵖˡ);
}.
...
{
  #[global] dataIO_type_repr  :: go.TypeReprUnderlying dataIOⁱᵐᵖˡ dataIO.t;
  #[global] dataIO_underlying :: (dataIO) <u (dataIOⁱᵐᵖˡ);
  #[global] dataIOⁱᵐᵖˡ_underlying :: (dataIOⁱᵐᵖˡ) ↓u (dataIOⁱᵐᵖˡ);
}.
