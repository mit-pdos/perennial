...
Definition Index__GetTuple : val :=
  rec: "Index__GetTuple" "idx" "key" :=
    ...
    let: ("$ret0", "$ret1") := (map.get (![#(mapT stringT ptrT)] (struct.field_ref #Index #"tplm"%go (![#ptrT] "idx"))) (![#stringT] "key")) in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
    do:  ("tpl" <-[#ptrT] "$r0");;;
    do:  ("ok" <-[#boolT] "$r1");;;
    ...
    do:  (map.insert (![#(mapT stringT ptrT)] (struct.field_ref #Index #"tplm"%go (![#ptrT] "idx"))) (![#stringT] "key") "$r0");;;
    do:  ((method_call #sync #"Mutex'ptr" #"Unlock" (![#ptrT] (struct.field_ref #Index #"mu"%go (![#ptrT] "idx")))) #());;;
    return: (![#ptrT] "tplnew")).
...
Definition MkIndex : val :=
  rec: "MkIndex" <> :=
    ...
    do:  ((struct.field_ref #Index #"tplm"%go (![#ptrT] "idx")) <-[#(mapT stringT ptrT)] "$r0");;;
    return: (![#ptrT] "idx")).
...
Definition initialize' : val :=
  rec: "initialize'" <> :=
    globals.package_init index.index (λ: <>,
