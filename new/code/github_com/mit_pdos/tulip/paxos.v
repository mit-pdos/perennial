...
Definition Paxos__nominate : val :=
  rec: "Paxos__nominate" "px" <> :=
    ...
    do:  ((struct.field_ref #Paxos #"respp"%go (![#ptrT] "px")) <-[#(mapT uint64T boolT)] "$r0");;;
    let: "$r0" := #true in
    do:  (map.insert (![#(mapT uint64T boolT)] (struct.field_ref #Paxos #"respp"%go (![#ptrT] "px"))) (![#uint64T] (struct.field_ref #Paxos #"nidme"%go (![#ptrT] "px"))) "$r0");;;
...
Definition Paxos__collect : val :=
  rec: "Paxos__collect" "px" "nid" "term" "ents" :=
    ...
    let: ("$ret0", "$ret1") := (map.get (![#(mapT uint64T boolT)] (struct.field_ref #Paxos #"respp"%go (![#ptrT] "px"))) (![#uint64T] "nid")) in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
    do:  "$r0";;;
    do:  ("recved" <-[#boolT] "$r1");;;
    ...
    then
      let: "$r0" := #true in
      do:  (map.insert (![#(mapT uint64T boolT)] (struct.field_ref #Paxos #"respp"%go (![#ptrT] "px"))) (![#uint64T] "nid") "$r0");;;
      return: (#())
    else do:  #());;;
...
Definition Paxos__ascend : val :=
  rec: "Paxos__ascend" "px" <> :=
    exception_do (let: "px" := (mem.alloc "px") in
    (if: (~ (let: "$a0" := (s_to_w64 (let: "$a0" := (![#(mapT uint64T boolT)] (struct.field_ref #Paxos #"respp"%go (![#ptrT] "px"))) in
    map.len "$a0")) in
    ...
    do:  ((struct.field_ref #Paxos #"lsnpeers"%go (![#ptrT] "px")) <-[#(mapT uint64T uint64T)] "$r0");;;
...
Definition Paxos__obtain : val :=
  rec: "Paxos__obtain" "px" "nid" :=
    ...
    let: ("$ret0", "$ret1") := (map.get (![#(mapT uint64T uint64T)] (struct.field_ref #Paxos #"lsnpeers"%go (![#ptrT] "px"))) (![#uint64T] "nid")) in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
    do:  ("lsne" <-[#uint64T] "$r0");;;
    do:  ("ok" <-[#boolT] "$r1");;;
...
Definition Paxos__forward : val :=
  rec: "Paxos__forward" "px" "nid" "lsn" :=
    ...
    then
      ...
      do:  (map.insert (![#(mapT uint64T uint64T)] (struct.field_ref #Paxos #"lsnpeers"%go (![#ptrT] "px"))) (![#uint64T] "nid") "$r0");;;
      return: (#true)
    else do:  #());;;
    return: (#false)).
...
Definition Paxos__push : val :=
  rec: "Paxos__push" "px" <> :=
    exception_do (let: "px" := (mem.alloc "px") in
    (if: (~ (let: "$a0" := ((s_to_w64 (let: "$a0" := (![#(mapT uint64T uint64T)] (struct.field_ref #Paxos #"lsnpeers"%go (![#ptrT] "px"))) in
    map.len "$a0")) + #(W64 1)) in
    (method_call #paxos.paxos #"Paxos'ptr" #"cquorum" (![#ptrT] "px")) "$a0"))
    ...
    do:  ("lsns" <-[#sliceT] "$r0");;;
    let: "$range" := (![#(mapT uint64T uint64T)] (struct.field_ref #Paxos #"lsnpeers"%go (![#ptrT] "px"))) in
    (let: "lsn" := (mem.alloc (type.zero_val #uint64T)) in
...
Definition Paxos__Serve : val :=
  rec: "Paxos__Serve" "px" <> :=
    ...
    let: "$r0" := (Fst (map.get (![#(mapT uint64T uint64T)] (struct.field_ref #Paxos #"addrm"%go (![#ptrT] "px"))) (![#uint64T] (struct.field_ref #Paxos #"nidme"%go (![#ptrT] "px"))))) in
...
Definition Paxos__GetConnection : val :=
  rec: "Paxos__GetConnection" "px" "nid" :=
    ...
    let: "ok" := (mem.alloc (type.zero_val #boolT)) in
    let: "conn" := (mem.alloc (type.zero_val #grove_ffi.Connection)) in
    let: ("$ret0", "$ret1") := (map.get (![#(mapT uint64T grove_ffi.Connection)] (struct.field_ref #Paxos #"conns"%go (![#ptrT] "px"))) (![#uint64T] "nid")) in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
    do:  ("conn" <-[#grove_ffi.Connection] "$r0");;;
    do:  ("ok" <-[#boolT] "$r1");;;
...
Definition Paxos__Connect : val :=
  rec: "Paxos__Connect" "px" "nid" :=
    ...
    let: "$r0" := (Fst (map.get (![#(mapT uint64T uint64T)] (struct.field_ref #Paxos #"addrm"%go (![#ptrT] "px"))) (![#uint64T] "nid"))) in
    do:  ("addr" <-[#uint64T] "$r0");;;
    ...
    then
      ...
      do:  (map.insert (![#(mapT uint64T grove_ffi.Connection)] (struct.field_ref #Paxos #"conns"%go (![#ptrT] "px"))) (![#uint64T] "nid") "$r0");;;
      do:  ((method_call #sync #"Mutex'ptr" #"Unlock" (![#ptrT] (struct.field_ref #Paxos #"mu"%go (![#ptrT] "px")))) #());;;
      return: (#true)
    else do:  #());;;
    return: (#false)).
...
Definition Paxos__DumpState : val :=
  rec: "Paxos__DumpState" "px" <> :=
    ...
    then
      ...
      "%go in
      let: "$a1" := ((let: "$sl0" := (interface.make #""%go #"uint64"%go (s_to_w64 (let: "$a0" := (![#(mapT uint64T boolT)] (struct.field_ref #Paxos #"respp"%go (![#ptrT] "px"))) in
      map.len "$a0"))) in
      slice.literal #interfaceT ["$sl0"])) in
      (func_call #fmt.fmt #"Printf"%go) "$a0" "$a1")
    else do:  #());;;
...
Definition mkPaxos : val :=
  rec: "mkPaxos" "nidme" "termc" "terml" "lsnc" "log" "addrm" "fname" :=
    ...
    struct.make #Paxos [{
      ...
      "respp" ::= type.zero_val #(mapT uint64T boolT);
      "lsnpeers" ::= type.zero_val #(mapT uint64T uint64T);
      "conns" ::= "$conns"
    }])) in
    do:  ("px" <-[#ptrT] "$r0");;;
    return: (![#ptrT] "px")).
...
Definition Start : val :=
  rec: "Start" "nidme" "addrm" "fname" :=
    ...
    let: "addrm" := (mem.alloc "addrm") in
    let: "nidme" := (mem.alloc "nidme") in
    do:  (let: "$a0" := (#(W64 1) < (s_to_w64 (let: "$a0" := (![#(mapT uint64T uint64T)] "addrm") in
    map.len "$a0"))) in
    (func_call #primitive.primitive #"Assume"%go) "$a0");;;
    let: "ok" := (mem.alloc (type.zero_val #boolT)) in
    let: ("$ret0", "$ret1") := (map.get (![#(mapT uint64T uint64T)] "addrm") (![#uint64T] "nidme")) in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
    do:  "$r0";;;
    do:  ("ok" <-[#boolT] "$r1");;;
    ...
    let: "$a5" := (![#(mapT uint64T uint64T)] "addrm") in
