...
Definition CountBoolMap : val :=
  rec: "CountBoolMap" "m" "b" :=
    ...
    let: "$r0" := #(W64 0) in
    do:  ("n" <-[#uint64T] "$r0");;;
    let: "$range" := (![#(mapT uint64T boolT)] "m") in
...
Definition DecodeKVMap : val :=
  rec: "DecodeKVMap" "bs" :=
    ...
    let: "$r0" := (![#sliceT] "bs1") in
    do:  ("data" <-[#sliceT] "$r0");;;
    let: "m" := (mem.alloc (type.zero_val #(mapT stringT tulip.Value))) in
    let: "$r0" := (map.make #stringT #tulip.Value) in
    do:  ("m" <-[#(mapT stringT tulip.Value)] "$r0");;;
    let: "i" := (mem.alloc (type.zero_val #uint64T)) in
    let: "$r0" := #(W64 0) in
    do:  ("i" <-[#uint64T] "$r0");;;
    (for: (λ: <>, (![#uint64T] "i") < (![#uint64T] "n")); (λ: <>, Skip) := λ: <>,
      ...
      do:  (map.insert (![#(mapT stringT tulip.Value)] "m") (![#stringT] (struct.field_ref #tulip.WriteEntry #"Key"%go "x")) "$r0");;;
      let: "$r0" := (![#sliceT] "bsloop") in
      do:  ("data" <-[#sliceT] "$r0");;;
      do:  ("i" <-[#uint64T] ((![#uint64T] "i") + #(W64 1))));;;
    return: (![#(mapT stringT tulip.Value)] "m", ![#sliceT] "data")).
