...
Definition S {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "github.com/mit-pdos/gokv/vrsm/apps/vkv/putargs_gk.S"%go [].

Definition Marshal {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "github.com/mit-pdos/gokv/vrsm/apps/vkv/putargs_gk.Marshal"%go.

Definition Unmarshal {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "github.com/mit-pdos/gokv/vrsm/apps/vkv/putargs_gk.Unmarshal"%go.
...
Definition Marshalⁱᵐᵖˡ {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: "enc" "p",
    ...
    do:  (let: "$a0" := (![go.string] (StructFieldRef S "Val"%go "p")) in
    (FuncResolve primitive.AssumeNoStringOverflow [] #()) "$a0");;;
    let: "$r0" := (let: "$a0" := (![go.SliceType go.byte] "enc") in
    let: "$a1" := (Convert go.string (go.SliceType go.byte) (![go.string] (StructFieldRef S "Val"%go "p"))) in
    (FuncResolve marshal.WriteLenPrefixedBytes [] #()) "$a0" "$a1") in
    do:  ("enc" <-[go.SliceType go.byte] "$r0");;;
...
Definition Unmarshalⁱᵐᵖˡ {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: "s",
    ...
    (FuncResolve std.BytesClone [] #()) "$a0")) in
    do:  ("val" <-[go.string] "$r0");;;
    return: (CompositeLiteral S (LiteralValue [KeyedElement (Some (KeyField "Key"%go)) (ElementExpression go.string (![go.string] "key")); KeyedElement (Some (KeyField "Val"%go)) (ElementExpression go.string (![go.string] "val"))]), ![go.SliceType go.byte] "s")).

#[global] Instance info' : PkgInfo pkg_id.putargs_gk :=
{|
...
Definition initialize' {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: <>,
    package.init pkg_id.putargs_gk (λ: <>,
