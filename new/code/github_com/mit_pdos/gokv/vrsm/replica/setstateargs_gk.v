...
Module S. Definition id : go_string := "github.com/mit-pdos/gokv/vrsm/replica/setstateargs_gk.S"%go. End S.
...
Definition Marshalⁱᵐᵖˡ : val :=
  λ: "enc" "s",
    ...
    do:  ("enc" <-[#sliceT] "$r0");;;
    let: "$r0" := (let: "$a0" := (![#sliceT] "enc") in
    let: "$a1" := (![#uint64T] (struct.field_ref #S #"NextIndex"%go "s")) in
    (func_call #marshal.WriteInt) "$a0" "$a1") in
    do:  ("enc" <-[#sliceT] "$r0");;;
    let: "$r0" := (let: "$a0" := (![#sliceT] "enc") in
    let: "$a1" := (![#uint64T] (struct.field_ref #S #"CommittedNextIndex"%go "s")) in
    (func_call #marshal.WriteInt) "$a0" "$a1") in
    do:  ("enc" <-[#sliceT] "$r0");;;
    let: "$r0" := (let: "$a0" := (![#sliceT] "enc") in
    let: "$a1" := (s_to_w64 (let: "$a0" := (![#sliceT] (struct.field_ref #S #"State"%go "s")) in
    slice.len "$a0")) in
    (func_call #marshal.WriteInt) "$a0" "$a1") in
    do:  ("enc" <-[#sliceT] "$r0");;;
    let: "$r0" := (let: "$a0" := (![#sliceT] "enc") in
    let: "$a1" := (![#sliceT] (struct.field_ref #S #"State"%go "s")) in
    (func_call #marshal.WriteBytes) "$a0" "$a1") in
    do:  ("enc" <-[#sliceT] "$r0");;;
    return: (![#sliceT] "enc")).

Definition Unmarshal : go_string := "github.com/mit-pdos/gokv/vrsm/replica/setstateargs_gk.Unmarshal"%go.
...
#[global] Instance info' : PkgInfo setstateargs_gk.setstateargs_gk :=
  {|
    pkg_vars := vars';
...
Definition initialize' : val :=
  λ: <>,
    package.init #setstateargs_gk.setstateargs_gk (λ: <>,
      exception_do (do:  (marshal.initialize' #());;;
      do:  (std.initialize' #());;;
      do:  (package.alloc setstateargs_gk.setstateargs_gk #()))
      ).
