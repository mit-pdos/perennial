...
Definition AppendOnlyFileⁱᵈ : go_string := "github.com/mit-pdos/gokv/aof.AppendOnlyFile"%go.
...
Definition CreateAppendOnlyFileⁱᵐᵖˡ : val :=
  λ: "fname",
    ...
    let: "$r0" := (let: "$a0" := (interface.make #(ptrTⁱᵈ sync.Mutexⁱᵈ) (![#ptrT] (struct.field_ref #AppendOnlyFile #"mu"%go (![#ptrT] "a")))) in
    ...
    let: "$go" := (λ: <>,
      exception_do (do:  ((method_call #(ptrTⁱᵈ sync.Mutexⁱᵈ) #"Lock"%go (![#ptrT] (struct.field_ref #AppendOnlyFile #"mu"%go (![#ptrT] "a")))) #());;;
      (for: (λ: <>, #true); (λ: <>, #()) := λ: <>,
        ...
        then
          do:  ((method_call #(ptrTⁱᵈ sync.Condⁱᵈ) #"Wait"%go (![#ptrT] (struct.field_ref #AppendOnlyFile #"lengthCond"%go (![#ptrT] "a")))) #());;;
          continue: #()
        else do:  #());;;
        ...
        then
          ...
          do:  ((method_call #(ptrTⁱᵈ sync.Condⁱᵈ) #"Broadcast"%go (![#ptrT] (struct.field_ref #AppendOnlyFile #"durableCond"%go (![#ptrT] "a")))) #());;;
          let: "$r0" := #true in
          do:  ((struct.field_ref #AppendOnlyFile #"closed"%go (![#ptrT] "a")) <-[#boolT] "$r0");;;
          do:  ((method_call #(ptrTⁱᵈ sync.Condⁱᵈ) #"Broadcast"%go (![#ptrT] (struct.field_ref #AppendOnlyFile #"closedCond"%go (![#ptrT] "a")))) #());;;
          do:  ((method_call #(ptrTⁱᵈ sync.Mutexⁱᵈ) #"Unlock"%go (![#ptrT] (struct.field_ref #AppendOnlyFile #"mu"%go (![#ptrT] "a")))) #());;;
          break: #()
        else do:  #());;;
        ...
        let: "$a1" := (![#sliceT] "l") in
        (func_call #grove_ffi.FileAppend) "$a0" "$a1");;;
        do:  ((method_call #(ptrTⁱᵈ sync.Mutexⁱᵈ) #"Lock"%go (![#ptrT] (struct.field_ref #AppendOnlyFile #"mu"%go (![#ptrT] "a")))) #());;;
        let: "$r0" := (![#uint64T] "newLength") in
        do:  ((struct.field_ref #AppendOnlyFile #"durableLength"%go (![#ptrT] "a")) <-[#uint64T] "$r0");;;
        do:  ((method_call #(ptrTⁱᵈ sync.Condⁱᵈ) #"Broadcast"%go (![#ptrT] "cond")) #());;;
        continue: #());;;
      return: #())
      ) in
    do:  (Fork ("$go" #()));;;
...
Definition AppendOnlyFile__Closeⁱᵐᵖˡ : val :=
  λ: "a" <>,
    ...
    do:  ((method_call #(ptrTⁱᵈ sync.Condⁱᵈ) #"Signal"%go (![#ptrT] (struct.field_ref #AppendOnlyFile #"lengthCond"%go (![#ptrT] "a")))) #());;;
    (for: (λ: <>, (~ (![#boolT] (struct.field_ref #AppendOnlyFile #"closed"%go (![#ptrT] "a"))))); (λ: <>, #()) := λ: <>,
      do:  ((method_call #(ptrTⁱᵈ sync.Condⁱᵈ) #"Wait"%go (![#ptrT] (struct.field_ref #AppendOnlyFile #"closedCond"%go (![#ptrT] "a")))) #()));;;
    do:  ((method_call #(ptrTⁱᵈ sync.Mutexⁱᵈ) #"Unlock"%go (![#ptrT] (struct.field_ref #AppendOnlyFile #"mu"%go (![#ptrT] "a")))) #());;;
    return: #()).
...
Definition AppendOnlyFile__WaitAppendⁱᵐᵖˡ : val :=
  λ: "a" "length",
    ...
    (for: (λ: <>, (![#uint64T] (struct.field_ref #AppendOnlyFile #"durableLength"%go (![#ptrT] "a"))) < (![#uint64T] "length")); (λ: <>, #()) := λ: <>,
      do:  ((method_call #(ptrTⁱᵈ sync.Condⁱᵈ) #"Wait"%go (![#ptrT] "cond")) #()));;;
    do:  ((method_call #(ptrTⁱᵈ sync.Mutexⁱᵈ) #"Unlock"%go (![#ptrT] (struct.field_ref #AppendOnlyFile #"mu"%go (![#ptrT] "a")))) #());;;
    return: #()).
