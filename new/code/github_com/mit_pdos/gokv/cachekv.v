...
Module cacheValue. Definition id : go_string := "github.com/mit-pdos/gokv/cachekv.cacheValue"%go. End cacheValue.
...
Definition msets' : list (go_string * (list (go_string * val))) := [(cacheValue.id, []); (ptrT.id cacheValue.id, []); (CacheKv.id, []); (ptrT.id CacheKv.id, [("Get"%go, CacheKv__Getⁱᵐᵖˡ); ("GetAndCache"%go, CacheKv__GetAndCacheⁱᵐᵖˡ); ("Put"%go, CacheKv__Putⁱᵐᵖˡ)])].
...
#[global] Instance info' : PkgInfo cachekv.cachekv :=
  {|
    ...
    pkg_msets := msets';
    pkg_imported_pkgs := [code.sync.sync; code.github_com.mit_pdos.gokv.grove_ffi.grove_ffi; code.github_com.mit_pdos.gokv.kv.kv; code.github_com.tchajed.marshal.marshal];
  |}.
