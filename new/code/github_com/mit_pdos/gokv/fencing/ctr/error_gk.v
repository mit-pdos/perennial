...
Module E. Definition id : go_string := "github.com/mit-pdos/gokv/fencing/ctr/error_gk.E"%go. End E.
...
Definition EStale : expr := #(W32 1).

Definition Marshal : go_string := "github.com/mit-pdos/gokv/fencing/ctr/error_gk.Marshal"%go.
...
Definition Unmarshal : go_string := "github.com/mit-pdos/gokv/fencing/ctr/error_gk.Unmarshal"%go.
...
Definition Name : go_string := "github.com/mit-pdos/gokv/fencing/ctr/error_gk.Name"%go.

Definition Value : go_string := "github.com/mit-pdos/gokv/fencing/ctr/error_gk.Value"%go.
...
Definition initialize' : val :=
  λ: <>,
    package.init #error_gk.error_gk (λ: <>,
      ...
      let: "$v1" := #"EStale"%go in
      let: "$k1" := #(W32 1) in
      map.literal #uint32T #stringT [map.kv_entry "$k0" "$v0"; map.kv_entry "$k1" "$v1"])) in
      do:  ((globals.get #Name) <-[type.mapT #uint32T #stringT] "$r0");;;
      let: "$r0" := ((let: "$v0" := #(W32 0) in
      let: "$k0" := #"ENone"%go in
      let: "$v1" := #(W32 1) in
      let: "$k1" := #"EStale"%go in
      map.literal #stringT #uint32T [map.kv_entry "$k0" "$v0"; map.kv_entry "$k1" "$v1"])) in
