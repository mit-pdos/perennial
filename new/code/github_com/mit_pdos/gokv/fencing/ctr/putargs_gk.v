...
Module S. Definition id : go_string := "github.com/mit-pdos/gokv/fencing/ctr/putargs_gk.S"%go. End S.
...
Definition Marshalⁱᵐᵖˡ : val :=
  λ: "enc" "p",
    ...
    do:  ("enc" <-[#sliceT] "$r0");;;
    let: "$r0" := (let: "$a0" := (![#sliceT] "enc") in
    let: "$a1" := (![#uint64T] (struct.field_ref #S #"V"%go "p")) in
    (func_call #marshal.WriteInt) "$a0" "$a1") in
    do:  ("enc" <-[#sliceT] "$r0");;;
    return: (![#sliceT] "enc")).

Definition Unmarshal : go_string := "github.com/mit-pdos/gokv/fencing/ctr/putargs_gk.Unmarshal"%go.
