...
Module S. Definition id : go_string := "github.com/mit-pdos/gokv/tutorial/objectstore/dir/finishwrite_gk.S"%go. End S.
...
Definition Marshalⁱᵐᵖˡ : val :=
  λ: "enc" "f",
    ...
    let: "$a1" := (![#uint64T] (struct.field_ref #S #"WriteId"%go "f")) in
    (func_call #marshal.WriteInt) "$a0" "$a1") in
    do:  ("enc" <-[#sliceT] "$r0");;;
    do:  (let: "$a0" := (![#stringT] (struct.field_ref #S #"Keyname"%go "f")) in
    (func_call #primitive.AssumeNoStringOverflow) "$a0");;;
    let: "$r0" := (let: "$a0" := (![#sliceT] "enc") in
    let: "$a1" := (string.to_bytes (![#stringT] (struct.field_ref #S #"Keyname"%go "f"))) in
    (func_call #marshal.WriteLenPrefixedBytes) "$a0" "$a1") in
    do:  ("enc" <-[#sliceT] "$r0");;;
    return: (![#sliceT] "enc")).

Definition Unmarshal : go_string := "github.com/mit-pdos/gokv/tutorial/objectstore/dir/finishwrite_gk.Unmarshal"%go.
...
Definition Unmarshalⁱᵐᵖˡ : val :=
  λ: "s",
    ...
    do:  ("s" <-[#sliceT] "$r1");;;
    let: "keynameBytes" := (mem.alloc (type.zero_val #sliceT)) in
    let: ("$ret0", "$ret1") := (let: "$a0" := (![#sliceT] "s") in
    (func_call #marshal.ReadLenPrefixedBytes) "$a0") in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
    do:  ("keynameBytes" <-[#sliceT] "$r0");;;
    do:  ("s" <-[#sliceT] "$r1");;;
    let: "keyname" := (mem.alloc (type.zero_val #stringT)) in
    let: "$r0" := (string.from_bytes (let: "$a0" := (![#sliceT] "keynameBytes") in
    (func_call #std.BytesClone) "$a0")) in
    do:  ("keyname" <-[#stringT] "$r0");;;
    return: (let: "$WriteId" := (![#uint64T] "writeId") in
     let: "$Keyname" := (![#stringT] "keyname") in
     struct.make #S [{
       "WriteId" ::= "$WriteId";
       "Keyname" ::= "$Keyname"
     }], ![#sliceT] "s")).
...
#[global] Instance info' : PkgInfo finishwrite_gk.finishwrite_gk :=
  {|
    pkg_vars := vars';
...
Definition initialize' : val :=
  λ: <>,
    package.init #finishwrite_gk.finishwrite_gk (λ: <>,
      exception_do (do:  (marshal.initialize' #());;;
      do:  (std.initialize' #());;;
      do:  (primitive.initialize' #());;;
      do:  (package.alloc finishwrite_gk.finishwrite_gk #()))
      ).
