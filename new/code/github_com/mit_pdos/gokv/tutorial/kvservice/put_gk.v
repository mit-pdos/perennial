...
Definition S {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "github.com/mit-pdos/gokv/tutorial/kvservice/put_gk.S"%go [].

Definition Marshal {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "github.com/mit-pdos/gokv/tutorial/kvservice/put_gk.Marshal"%go.

Definition Unmarshal {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "github.com/mit-pdos/gokv/tutorial/kvservice/put_gk.Unmarshal"%go.
...
Definition Marshalⁱᵐᵖˡ {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: "enc" "p",
    ...
    let: "$a1" := (![go.uint64] (StructFieldRef S "OpId"%go "p")) in
    (FuncResolve marshal.WriteInt [] #()) "$a0" "$a1") in
    do:  ("enc" <-[go.SliceType go.byte] "$r0");;;
    do:  (let: "$a0" := (![go.string] (StructFieldRef S "Key"%go "p")) in
    (FuncResolve primitive.AssumeNoStringOverflow [] #()) "$a0");;;
    let: "$r0" := (let: "$a0" := (![go.SliceType go.byte] "enc") in
    let: "$a1" := (Convert go.string (go.SliceType go.byte) (![go.string] (StructFieldRef S "Key"%go "p"))) in
    (FuncResolve marshal.WriteLenPrefixedBytes [] #()) "$a0" "$a1") in
    do:  ("enc" <-[go.SliceType go.byte] "$r0");;;
    do:  (let: "$a0" := (![go.string] (StructFieldRef S "Value"%go "p")) in
    (FuncResolve primitive.AssumeNoStringOverflow [] #()) "$a0");;;
    let: "$r0" := (let: "$a0" := (![go.SliceType go.byte] "enc") in
    let: "$a1" := (Convert go.string (go.SliceType go.byte) (![go.string] (StructFieldRef S "Value"%go "p"))) in
    (FuncResolve marshal.WriteLenPrefixedBytes [] #()) "$a0" "$a1") in
    do:  ("enc" <-[go.SliceType go.byte] "$r0");;;
...
Definition Unmarshalⁱᵐᵖˡ {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: "s",
    ...
    let: "$r0" := (Convert (go.SliceType go.byte) go.string (let: "$a0" := (![go.SliceType go.byte] "valueBytes") in
    (FuncResolve std.BytesClone [] #()) "$a0")) in
    do:  ("value" <-[go.string] "$r0");;;
    return: (CompositeLiteral S (LiteralValue [KeyedElement (Some (KeyField "OpId"%go)) (ElementExpression go.uint64 (![go.uint64] "opId")); KeyedElement (Some (KeyField "Key"%go)) (ElementExpression go.string (![go.string] "key")); KeyedElement (Some (KeyField "Value"%go)) (ElementExpression go.string (![go.string] "value"))]), ![go.SliceType go.byte] "s")).

#[global] Instance info' : PkgInfo pkg_id.put_gk :=
{|
