...
Definition S {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "github.com/mit-pdos/gokv/tutorial/kvservice/get_gk.S"%go [].

Definition Marshal {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "github.com/mit-pdos/gokv/tutorial/kvservice/get_gk.Marshal"%go.

Definition Unmarshal {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "github.com/mit-pdos/gokv/tutorial/kvservice/get_gk.Unmarshal"%go.
...
Definition Marshalⁱᵐᵖˡ {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: "enc" "g",
    ...
    let: "$a1" := (![go.uint64] (StructFieldRef S "OpId"%go "g")) in
    (FuncResolve marshal.WriteInt [] #()) "$a0" "$a1") in
    do:  ("enc" <-[go.SliceType go.byte] "$r0");;;
    do:  (let: "$a0" := (![go.string] (StructFieldRef S "Key"%go "g")) in
    (FuncResolve primitive.AssumeNoStringOverflow [] #()) "$a0");;;
    let: "$r0" := (let: "$a0" := (![go.SliceType go.byte] "enc") in
    let: "$a1" := (Convert go.string (go.SliceType go.byte) (![go.string] (StructFieldRef S "Key"%go "g"))) in
...
Definition Unmarshalⁱᵐᵖˡ {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: "s",
    ...
    do:  ("key" <-[go.string] "$r0");;;
    return: (CompositeLiteral S (LiteralValue [KeyedElement (Some (KeyField "OpId"%go)) (ElementExpression go.uint64 (![go.uint64] "opId")); KeyedElement (Some (KeyField "Key"%go)) (ElementExpression go.string (![go.string] "key"))]), ![go.SliceType go.byte] "s")).

#[global] Instance info' : PkgInfo pkg_id.get_gk :=
{|
