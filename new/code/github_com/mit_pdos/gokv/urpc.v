...
Definition Client__CallCompleteⁱᵐᵖˡ {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: "cl" "cb" "reply" "timeout_ms",
    ...
    then
      do:  (let: "$a0" := (![go.PointerType sync.Cond] (StructFieldRef Callback "cond"%go (![go.PointerType Callback] "cb"))) in
      let: "$a1" := (![go.uint64] "timeout_ms") in
      (FuncResolve primitive.WaitTimeout [] #()) "$a0" "$a1")
    else do:  #());;;
...
{|
  pkg_imported_pkgs := [code.log.pkg_id.log; code.sync.pkg_id.sync; code.github_com.goose_lang.primitive.pkg_id.primitive; code.github_com.goose_lang.std.pkg_id.std; code.github_com.mit_pdos.gokv.grove_ffi.pkg_id.grove_ffi; code.github_com.tchajed.marshal.pkg_id.marshal]
|}.
