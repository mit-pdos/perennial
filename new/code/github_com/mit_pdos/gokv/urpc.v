  ...
  rec: "Server__readThread'" "srv" "conn" :=
    ...
    (for: (λ: <>, #true); (λ: <>, Skip) := λ: <>,
      ...
      let: "$go" := (λ: <>,
        ...
        (method_call Server'ptr "rpcHandle" #() (![ptrT] "srv")) "$a0" "$a1" "$a2" "$a3"))
        ) in
      do:  (Fork ("$go" #()));;;
      continue: #())).
  ...
  rec: "Server__Serve'" "srv" "host" :=
    ...
    let: "$go" := (λ: <>,
      exception_do ((for: (λ: <>, #true); (λ: <>, Skip) := λ: <>,
        ...
        let: "$go" := (λ: <>,
          exception_do (do:  (let: "$a0" := (![grove_ffi.Connection] "conn") in
          (method_call Server'ptr "readThread" #() (![ptrT] "srv")) "$a0"))
          ) in
        do:  (Fork ("$go" #()))))
      ) in
    do:  (Fork ("$go" #()))).
...
Definition TryMakeClient' : val :=
  rec: "TryMakeClient'" "host_name" :=
    ...
    do:  ("cl" <-[ptrT] "$r0");;;
    let: "$go" := (λ: <>,
      exception_do (do:  ((method_call Client'ptr "replyThread" #() (![ptrT] "cl")) #()))
      ) in
    do:  (Fork ("$go" #()));;;
...
Definition Client__Call' : val :=
  rec: "Client__Call'" "cl" "rpcid" "args" "reply" "timeout_ms" :=
    ...
    let: "$a1" := (![sliceT] "args") in
    (method_call Client'ptr "CallStart" #() (![ptrT] "cl")) "$a0" "$a1") in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
    do:  ("cb" <-[ptrT] "$r0");;;
    do:  ("err" <-[uint64T] "$r1");;;
    ...
    return: (let: "$a0" := (![ptrT] "cb") in
     ...
     (method_call Client'ptr "CallComplete" #() (![ptrT] "cl")) "$a0" "$a1" "$a2")).
