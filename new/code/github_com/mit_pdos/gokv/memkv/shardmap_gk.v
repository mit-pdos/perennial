(* autogenerated from github.com/mit-pdos/gokv/memkv/shardmap_gk *)
Require Export New.code.github_com.tchajed.marshal.
From New.golang Require Import defn.
Module pkg_id.
Definition shardmap_gk : go_string := "github.com/mit-pdos/gokv/memkv/shardmap_gk".

End pkg_id.
Export pkg_id.
Module shardmap_gk.

Definition S {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "github.com/mit-pdos/gokv/memkv/shardmap_gk.S"%go [].

Definition Marshal {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "github.com/mit-pdos/gokv/memkv/shardmap_gk.Marshal"%go.

Definition Unmarshal {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "github.com/mit-pdos/gokv/memkv/shardmap_gk.Unmarshal"%go.

(* go: shardmap_gk.go:16:6 *)
Definition Marshalⁱᵐᵖˡ {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: "enc" "s",
    ...
    let: "$a1" := (Convert go.int go.uint64 (let: "$a0" := (![go.SliceType go.uint64] (StructFieldRef S "Shards"%go "s")) in
    (FuncResolve go.len [go.SliceType go.uint64] #()) "$a0")) in
    (FuncResolve marshal.WriteInt [] #()) "$a0" "$a1") in
    do:  ("enc" <-[go.SliceType go.byte] "$r0");;;
    let: "$r0" := (let: "$a0" := (![go.SliceType go.byte] "enc") in
    let: "$a1" := (![go.SliceType go.uint64] (StructFieldRef S "Shards"%go "s")) in
...
(* go: shardmap_gk.go:24:6 *)
Definition Unmarshalⁱᵐᵖˡ {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: "s",
    exception_do (let: "s" := (GoAlloc (go.SliceType go.byte) "s") in
    let: "shardsLen" := (GoAlloc go.uint64 (GoZeroVal go.uint64 #())) in
    let: ("$ret0", "$ret1") := (let: "$a0" := (![go.SliceType go.byte] "s") in
    (FuncResolve marshal.ReadInt [] #()) "$a0") in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
    do:  ("shardsLen" <-[go.uint64] "$r0");;;
    do:  ("s" <-[go.SliceType go.byte] "$r1");;;
    let: "shards" := (GoAlloc (go.SliceType go.uint64) (GoZeroVal (go.SliceType go.uint64) #())) in
    let: ("$ret0", "$ret1") := (let: "$a0" := (![go.SliceType go.byte] "s") in
    let: "$a1" := (![go.uint64] "shardsLen") in
    let: "$a2" := (FuncResolve marshal.ReadInt [] #()) in
    (FuncResolve marshal.ReadSlice [go.uint64] #()) "$a0" "$a1" "$a2") in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
    do:  ("shards" <-[go.SliceType go.uint64] "$r0");;;
    do:  ("s" <-[go.SliceType go.byte] "$r1");;;
    return: (CompositeLiteral S (LiteralValue [KeyedElement (Some (KeyField "Shards"%go)) (ElementExpression (go.SliceType go.uint64) (![go.SliceType go.uint64] "shards"))]), ![go.SliceType go.byte] "s")).

#[global] Instance info' : PkgInfo pkg_id.shardmap_gk :=
{|
...
Definition initialize' {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: <>,
    package.init pkg_id.shardmap_gk (λ: <>,
...
Definition S'fds_unsealed {ext : ffi_syntax} {go_gctx : GoGlobalContext} : list go.field_decl := [
  (go.FieldDecl "Shards"%go (go.SliceType go.uint64))
].
...
Class S_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  ...
  #[global] S_get_Shards (x : S.t) :: ⟦StructFieldGet (Sⁱᵐᵖˡ) "Shards", #x⟧ ⤳[under] #x.(S.Shards');
  #[global] S_set_Shards (x : S.t) y :: ⟦StructFieldSet (Sⁱᵐᵖˡ) "Shards", (#x, #y)⟧ ⤳[under] #(x <|S.Shards' := y|>);
}.
