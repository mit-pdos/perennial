...
Definition S {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "github.com/mit-pdos/gokv/cachekv/cachevalue_gk.S"%go [].

Definition Marshal {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "github.com/mit-pdos/gokv/cachekv/cachevalue_gk.Marshal"%go.

Definition Unmarshal {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "github.com/mit-pdos/gokv/cachekv/cachevalue_gk.Unmarshal"%go.
...
Definition Marshalⁱᵐᵖˡ {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: "enc" "c",
    ...
    do:  (let: "$a0" := (![go.string] (StructFieldRef S "V"%go "c")) in
    (FuncResolve primitive.AssumeNoStringOverflow [] #()) "$a0");;;
    let: "$r0" := (let: "$a0" := (![go.SliceType go.byte] "enc") in
    let: "$a1" := (Convert go.string (go.SliceType go.byte) (![go.string] (StructFieldRef S "V"%go "c"))) in
    (FuncResolve marshal.WriteLenPrefixedBytes [] #()) "$a0" "$a1") in
    do:  ("enc" <-[go.SliceType go.byte] "$r0");;;
    let: "$r0" := (let: "$a0" := (![go.SliceType go.byte] "enc") in
    let: "$a1" := (![go.uint64] (StructFieldRef S "L"%go "c")) in
...
Definition Unmarshalⁱᵐᵖˡ {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: "s",
    ...
    let: "vBytes" := (GoAlloc (go.SliceType go.byte) (GoZeroVal (go.SliceType go.byte) #())) in
    let: ("$ret0", "$ret1") := (let: "$a0" := (![go.SliceType go.byte] "s") in
    (FuncResolve marshal.ReadLenPrefixedBytes [] #()) "$a0") in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
    do:  ("vBytes" <-[go.SliceType go.byte] "$r0");;;
    do:  ("s" <-[go.SliceType go.byte] "$r1");;;
    let: "v" := (GoAlloc go.string (GoZeroVal go.string #())) in
    let: "$r0" := (Convert (go.SliceType go.byte) go.string (let: "$a0" := (![go.SliceType go.byte] "vBytes") in
    ...
    do:  ("l" <-[go.uint64] "$r0");;;
    do:  ("s" <-[go.SliceType go.byte] "$r1");;;
    return: (CompositeLiteral S (LiteralValue [KeyedElement (Some (KeyField "V"%go)) (ElementExpression go.string (![go.string] "v")); KeyedElement (Some (KeyField "L"%go)) (ElementExpression go.uint64 (![go.uint64] "l"))]), ![go.SliceType go.byte] "s")).

#[global] Instance info' : PkgInfo pkg_id.cachevalue_gk :=
{|
...
Definition initialize' {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: <>,
    package.init pkg_id.cachevalue_gk (λ: <>,
...
Definition S'fds_unsealed {ext : ffi_syntax} {go_gctx : GoGlobalContext} : list go.field_decl := [
  (go.FieldDecl "V"%go go.string);
  (go.FieldDecl "L"%go go.uint64)
].
...
Class S_Assumptions {ext : ffi_syntax} `{!GoGlobalContext} `{!GoLocalContext} `{!GoSemanticsFunctions} : Prop :=
{
  ...
  #[global] S_get_L (x : S.t) :: ⟦StructFieldGet (Sⁱᵐᵖˡ) "L", #x⟧ ⤳[under] #x.(S.L');
  #[global] S_set_L (x : S.t) y :: ⟦StructFieldSet (Sⁱᵐᵖˡ) "L", (#x, #y)⟧ ⤳[under] #(x <|S.L' := y|>);
}.
