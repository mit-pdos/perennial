...
Definition LockClerk__Lockⁱᵐᵖˡ : val :=
  λ: "ck" "key",
    ...
    let: "$a1" := #""%go in
    let: "$a2" := #"1"%go in
    (interface.get #"ConditionalPut"%go (![kv.KvCput] (struct.field_ref ptrT #"kv"%go (![ptrT] "ck")))) "$a0" "$a1" "$a2") ≠ #"ok"%go); (λ: <>, #()) := λ: <>,
      do:  #());;;
    return: #()).
...
Definition LockClerk__Unlockⁱᵐᵖˡ : val :=
  λ: "ck" "key",
    ...
    let: "$a1" := #""%go in
    (interface.get #"Put"%go (![kv.KvCput] (struct.field_ref ptrT #"kv"%go (![ptrT] "ck")))) "$a0" "$a1");;;
    return: #()).
...
#[global] Instance info' : PkgInfo lockservice.lockservice :=
  {|
    ...
    pkg_msets := msets';
    pkg_imported_pkgs := [code.github_com.mit_pdos.gokv.kv.kv];
  |}.
