...
Definition sliceOps : val :=
  rec: "sliceOps" <> :=
    ...
    do:  ("v4" <-[ptrT] "$r0");;;
    return: ((((((![uint64T] "v1") + (![uint64T] (slice.elem_ref uint64T (![sliceT uint64T] "v2") #0))) + (![uint64T] (slice.elem_ref uint64T (![sliceT uint64T] "v3") #1))) + (![uint64T] (![ptrT] "v4"))) + (let: "$a0" := (![sliceT uint64T] "x") in
