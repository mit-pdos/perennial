...
Definition basicTypeSwitch : val :=
  rec: "basicTypeSwitch" "x" :=
    ...
    let: "$x_ok" := (interface.checked_type_assert #intT "$y" (#""%go, #"int"%go)) in
    (if: Snd "$x_ok"
    then return: (#(W64 1))
    else
      let: "$x_ok" := (interface.checked_type_assert #stringT "$y" (#""%go, #"string"%go)) in
      (if: Snd "$x_ok"
      then return: (#(W64 2))
      else do:  #()));;;
    return: (#(W64 0))).
