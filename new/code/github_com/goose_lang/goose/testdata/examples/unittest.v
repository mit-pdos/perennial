...
Definition ifJoinDemo : go_string := "github.com/goose-lang/goose/testdata/examples/unittest.ifJoinDemo"%go.

(* go: repeat_vars.go:3:6 *)
Definition ifJoinDemoⁱᵐᵖˡ : val :=
  λ: "arg1" "arg2",
    exception_do (let: "arg2" := (mem.alloc "arg2") in
    let: "arg1" := (mem.alloc "arg1") in
    let: "arr" := (mem.alloc (type.zero_val #sliceT)) in
    let: "$r0" := #slice.nil in
    do:  ("arr" <-[#sliceT] "$r0");;;
    (if: ![#boolT] "arg1"
    then
      let: "$r0" := (let: "$a0" := (![#sliceT] "arr") in
...
Definition repeatLocalVars : go_string := "github.com/goose-lang/goose/testdata/examples/unittest.repeatLocalVars"%go.

(* go: repeat_vars.go:13:6 *)
Definition repeatLocalVarsⁱᵐᵖˡ : val :=
  λ: <>,
    exception_do (let: "g" := (mem.alloc (type.zero_val #intT)) in
    let: "$r0" := #(W64 0) in
    do:  ("g" <-[#intT] "$r0");;;
    ...
    (if: (![#intT] "g") ≠ #(W64 3)
    then
      do:  (let: "$a0" := (interface.make #stringT.id #"failure"%go) in
      Panic "$a0")
    else do:  #());;;
    return: #()).
