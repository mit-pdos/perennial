...
Definition Enc__consumeⁱᵐᵖˡ : val :=
  λ: "e" "n",
    ...
    let: "$r0" := (let: "$s" := (![sliceT] (struct.field_ref ptrT #"p"%go (![ptrT] "e"))) in
    slice.slice byteT "$s" #(W64 0) (![uint64T] "n")) in
    do:  ("b" <-[sliceT] "$r0");;;
    let: "$r0" := (let: "$s" := (![sliceT] (struct.field_ref ptrT #"p"%go (![ptrT] "e"))) in
    slice.slice byteT "$s" (![uint64T] "n") (slice.len "$s")) in
    do:  ((struct.field_ref ptrT #"p"%go (![ptrT] "e")) <-[sliceT] "$r0");;;
    return: (![sliceT] "b")).
...
Definition Dec__consumeⁱᵐᵖˡ : val :=
  λ: "d" "n",
    ...
    let: "n" := (mem.alloc "n") in
    let: "b" := (mem.alloc (type.zero_val sliceT)) in
    let: "$r0" := (let: "$s" := (![sliceT] (struct.field_ref ptrT #"p"%go (![ptrT] "d"))) in
    slice.slice byteT "$s" #(W64 0) (![uint64T] "n")) in
    do:  ("b" <-[sliceT] "$r0");;;
    let: "$r0" := (let: "$s" := (![sliceT] (struct.field_ref ptrT #"p"%go (![ptrT] "d"))) in
    slice.slice byteT "$s" (![uint64T] "n") (slice.len "$s")) in
    do:  ((struct.field_ref ptrT #"p"%go (![ptrT] "d")) <-[sliceT] "$r0");;;
    return: (![sliceT] "b")).
...
Definition Editor__AdvanceReturnⁱᵐᵖˡ : val :=
  λ: "e" "next",
    ...
    let: "$r0" := (![uint64T] (struct.field_ref ptrT #"next_val"%go (![ptrT] "e"))) in
    do:  ("tmp" <-[uint64T] "$r0");;;
    let: "$r0" := (![uint64T] "tmp") in
    do:  ((slice.elem_ref uint64T (![sliceT] (struct.field_ref ptrT #"s"%go (![ptrT] "e"))) #(W64 0)) <-[uint64T] "$r0");;;
    let: "$r0" := (![uint64T] "next") in
    do:  ((struct.field_ref ptrT #"next_val"%go (![ptrT] "e")) <-[uint64T] "$r0");;;
    let: "$r0" := (let: "$s" := (![sliceT] (struct.field_ref ptrT #"s"%go (![ptrT] "e"))) in
    slice.slice uint64T "$s" #(W64 1) (slice.len "$s")) in
    do:  ((struct.field_ref ptrT #"s"%go (![ptrT] "e")) <-[sliceT] "$r0");;;
    return: (![uint64T] "tmp")).
...
Definition testForLoopWaitⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    do:  ("ls" <-[LoopStruct] "$r0");;;
    do:  (let: "$a0" := #(W64 3) in
    (method_call #(ptrT.id LoopStruct.id) #"forLoopWait"%go "ls") "$a0");;;
...
Definition CheckTrueⁱᵐᵖˡ : val :=
  λ: "b",
    ...
    do:  ((struct.field_ref ptrT #"tc"%go (![ptrT] "b")) <-[uint64T] ((![uint64T] (struct.field_ref ptrT #"tc"%go (![ptrT] "b"))) + #(W64 1)));;;
    return: (![boolT] (struct.field_ref ptrT #"t"%go (![ptrT] "b")))).
...
Definition CheckFalseⁱᵐᵖˡ : val :=
  λ: "b",
    exception_do (let: "b" := (mem.alloc "b") in
    do:  ((struct.field_ref ptrT #"fc"%go (![ptrT] "b")) <-[uint64T] ((![uint64T] (struct.field_ref ptrT #"fc"%go (![ptrT] "b"))) + #(W64 1)));;;
    return: (![boolT] (struct.field_ref ptrT #"f"%go (![ptrT] "b")))).
...
Definition testShortcircuitAndTFⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    return: (((![uint64T] (struct.field_ref ptrT #"tc"%go (![ptrT] "b"))) = #(W64 1)) && ((![uint64T] (struct.field_ref ptrT #"fc"%go (![ptrT] "b"))) = #(W64 1)))).
...
Definition testShortcircuitAndFTⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    return: (((![uint64T] (struct.field_ref ptrT #"tc"%go (![ptrT] "b"))) = #(W64 0)) && ((![uint64T] (struct.field_ref ptrT #"fc"%go (![ptrT] "b"))) = #(W64 1)))).
...
Definition testShortcircuitOrTFⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    (func_call #CheckTrue) "$a0") || (let: "$a0" := (![ptrT] "b") in
    (func_call #CheckFalse) "$a0")
    then return: (((![uint64T] (struct.field_ref ptrT #"tc"%go (![ptrT] "b"))) = #(W64 1)) && ((![uint64T] (struct.field_ref ptrT #"fc"%go (![ptrT] "b"))) = #(W64 0)))
    else do:  #());;;
    return: (#false)).
...
Definition testShortcircuitOrFTⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    (func_call #CheckFalse) "$a0") || (let: "$a0" := (![ptrT] "b") in
    (func_call #CheckTrue) "$a0")
    then return: (((![uint64T] (struct.field_ref ptrT #"tc"%go (![ptrT] "b"))) = #(W64 1)) && ((![uint64T] (struct.field_ref ptrT #"fc"%go (![ptrT] "b"))) = #(W64 1)))
    else do:  #());;;
    return: (#false)).
...
Definition ArrayEditor__Advanceⁱᵐᵖˡ : val :=
  λ: "ae" "arr" "next",
    ...
    do:  ((slice.elem_ref uint64T (![sliceT] "arr") #(W64 0)) <-[uint64T] ((![uint64T] (slice.elem_ref uint64T (![sliceT] "arr") #(W64 0))) + #(W64 1)));;;
    let: "$r0" := (![uint64T] (struct.field_ref ptrT #"next_val"%go (![ptrT] "ae"))) in
    do:  ((slice.elem_ref uint64T (![sliceT] (struct.field_ref ptrT #"s"%go (![ptrT] "ae"))) #(W64 0)) <-[uint64T] "$r0");;;
    let: "$r0" := (![uint64T] "next") in
    do:  ((struct.field_ref ptrT #"next_val"%go (![ptrT] "ae")) <-[uint64T] "$r0");;;
    let: "$r0" := (let: "$s" := (![sliceT] (struct.field_ref ptrT #"s"%go (![ptrT] "ae"))) in
    slice.slice uint64T "$s" #(W64 1) (slice.len "$s")) in
    do:  ((struct.field_ref ptrT #"s"%go (![ptrT] "ae")) <-[sliceT] "$r0");;;
    return: #()).
...
Definition Bar__mutateⁱᵐᵖˡ : val :=
  λ: "bar" <>,
    ...
    let: "$r0" := #(W64 2) in
    do:  ((struct.field_ref ptrT #"a"%go (![ptrT] "bar")) <-[uint64T] "$r0");;;
    let: "$r0" := #(W64 3) in
    do:  ((struct.field_ref ptrT #"b"%go (![ptrT] "bar")) <-[uint64T] "$r0");;;
    return: #()).
...
Definition Foo__mutateBarⁱᵐᵖˡ : val :=
  λ: "foo" <>,
    ...
    do:  ((method_call #(ptrT.id Bar.id) #"mutate"%go (struct.field_ref ptrT #"bar"%go (![ptrT] "foo"))) #());;;
    return: #()).
...
Definition S__readAⁱᵐᵖˡ : val :=
  λ: "s" <>,
    exception_do (let: "s" := (mem.alloc "s") in
    return: (![uint64T] (struct.field_ref ptrT #"a"%go (![ptrT] "s")))).

(* go: structs.go:26:13 *)
Definition S__readBⁱᵐᵖˡ : val :=
  λ: "s" <>,
    exception_do (let: "s" := (mem.alloc "s") in
    return: (![TwoInts] (struct.field_ref ptrT #"b"%go (![ptrT] "s")))).
...
Definition S__updateBValXⁱᵐᵖˡ : val :=
  λ: "s" "i",
    ...
    let: "i" := (mem.alloc "i") in
    let: "$r0" := (![uint64T] "i") in
    do:  ((struct.field_ref TwoInts #"x"%go (struct.field_ref ptrT #"b"%go (![ptrT] "s"))) <-[uint64T] "$r0");;;
    return: #()).
...
Definition S__negateCⁱᵐᵖˡ : val :=
  λ: "s" <>,
    exception_do (let: "s" := (mem.alloc "s") in
    let: "$r0" := (~ (![boolT] (struct.field_ref ptrT #"c"%go (![ptrT] "s")))) in
    do:  ((struct.field_ref ptrT #"c"%go (![ptrT] "s")) <-[boolT] "$r0");;;
    return: #()).
...
Definition testStructUpdatesⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    let: "$r0" := ((![boolT] "ok") && ((![boolT] (struct.field_ref ptrT #"c"%go (![ptrT] "ns"))) = #false)) in
    do:  ("ok" <-[boolT] "$r0");;;
    let: "$r0" := #(W64 3) in
    ...
    let: "$r0" := (struct.field_ref ptrT #"b"%go (![ptrT] "ns")) in
    do:  ("b3" <-[ptrT] "$r0");;;
    let: "$r0" := ((![boolT] "ok") && ((![uint64T] (struct.field_ref ptrT #"x"%go (![ptrT] "b3"))) = #(W64 1))) in
    do:  ("ok" <-[boolT] "$r0");;;
    do:  (let: "$a0" := #(W64 4) in
...
Definition testNestedStructUpdatesⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    do:  ("ns" <-[ptrT] "$r0");;;
    let: "$r0" := #(W64 5) in
    do:  ((struct.field_ref TwoInts #"x"%go (struct.field_ref ptrT #"b"%go (![ptrT] "ns"))) <-[uint64T] "$r0");;;
    let: "$r0" := ((![boolT] "ok") && ((![uint64T] (struct.field_ref TwoInts #"x"%go (struct.field_ref ptrT #"b"%go (![ptrT] "ns")))) = #(W64 5))) in
    do:  ("ok" <-[boolT] "$r0");;;
    let: "$r0" := ((func_call #NewS) #()) in
    do:  ("ns" <-[ptrT] "$r0");;;
    ...
    do:  ("p" <-[ptrT] "$r0");;;
    let: "$r0" := #(W64 5) in
    do:  ((struct.field_ref ptrT #"x"%go (![ptrT] "p")) <-[uint64T] "$r0");;;
    let: "$r0" := ((![boolT] "ok") && ((![uint64T] (struct.field_ref TwoInts #"x"%go (struct.field_ref ptrT #"b"%go (![ptrT] "ns")))) = #(W64 5))) in
    do:  ("ok" <-[boolT] "$r0");;;
    let: "$r0" := ((func_call #NewS) #()) in
    do:  ("ns" <-[ptrT] "$r0");;;
    ...
    let: "$r0" := #(W64 5) in
    do:  ((struct.field_ref TwoInts #"x"%go (struct.field_ref ptrT #"b"%go (![ptrT] "ns"))) <-[uint64T] "$r0");;;
    let: "$r0" := ((![boolT] "ok") && ((![uint64T] (struct.field_ref ptrT #"x"%go (![ptrT] "p"))) = #(W64 5))) in
    do:  ("ok" <-[boolT] "$r0");;;
    return: (![boolT] "ok")).
...
Definition testStoreInStructPointerVarⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    do:  ("p" <-[ptrT] "$r0");;;
    let: "$r0" := #(W64 5) in
    do:  ((struct.field_ref ptrT #"i"%go (![ptrT] "p")) <-[uint64T] "$r0");;;
    return: ((![uint64T] (struct.field_ref ptrT #"i"%go (![ptrT] "p"))) = #(W64 5))).
...
Definition testStructFieldFuncⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    do:  ((struct.field_ref ptrT #"fn"%go (![ptrT] "a")) <-[funcT] "$r0");;;
    return: ((let: "$a0" := #(W64 10) in
     (![funcT] (struct.field_ref ptrT #"fn"%go (![ptrT] "a"))) "$a0") = #(W64 20))).
...
Definition disabled_testWalⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    do:  ("lg" <-[Log] "$r0");;;
    (if: (method_call #(ptrT.id Log.id) #"BeginTxn"%go "lg") #()
    then
      do:  (let: "$a0" := #(W64 2) in
      let: "$a1" := (let: "$a0" := #(W64 11) in
      (func_call #intToBlock) "$a0") in
      (method_call #(ptrT.id Log.id) #"Write"%go "lg") "$a0" "$a1")
    else do:  #());;;
    let: "$r0" := ((![boolT] "ok") && ((let: "$a0" := (let: "$a0" := #(W64 2) in
    (method_call #(ptrT.id Log.id) #"Read"%go "lg") "$a0") in
    ...
    do:  ("ok" <-[boolT] "$r0");;;
    do:  ((method_call #(ptrT.id Log.id) #"Commit"%go "lg") #());;;
    let: "$r0" := ((![boolT] "ok") && ((let: "$a0" := (let: "$a0" := #(W64 0) in
    (interface.get #"Read"%go (![disk.Disk] (struct.field_ref Log #"d"%go "lg"))) "$a0") in
    (func_call #blockToInt) "$a0") = #(W64 1))) in
    do:  ("ok" <-[boolT] "$r0");;;
    do:  ((method_call #(ptrT.id Log.id) #"Apply"%go "lg") #());;;
