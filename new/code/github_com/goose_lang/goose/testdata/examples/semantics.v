...
Definition findKey : val :=
  rec: "findKey" "m" :=
    ...
    let: "found" := (alloc (zero_val uint64T)) in
    let: "$r0" := #(W64 0) in
    do:  ("found" <-[#uint64T] "$r0");;;
...
Definition freeRange : val :=
  rec: "freeRange" "sz" :=
    ...
    let: "m" := (alloc (zero_val (mapT uint64T unit))) in
...
Definition testAllocateDistinct : val :=
  rec: "testAllocateDistinct" <> :=
    ...
    do:  ("a1" <-[#uint64T] "$r0");;;
    do:  "$r1";;;
    let: "a2" := (alloc (zero_val uint64T)) in
    let: ("$ret0", "$ret1") := (let: "$a0" := (![#(mapT uint64T unit)] "free") in
    (func_call #semantics.semantics #"allocate"%go) "$a0") in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
...
Definition testAllocateFull : val :=
  rec: "testAllocateFull" <> :=
    ...
    (func_call #semantics.semantics #"freeRange"%go) "$a0") in
    do:  ("free" <-[#(mapT uint64T unit)] "$r0");;;
    let: "ok1" := (alloc (zero_val boolT)) in
    let: ("$ret0", "$ret1") := (let: "$a0" := (![#(mapT uint64T unit)] "free") in
    (func_call #semantics.semantics #"allocate"%go) "$a0") in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
    do:  "$r0";;;
    do:  ("ok1" <-[#boolT] "$r1");;;
    let: "ok2" := (alloc (zero_val boolT)) in
    let: ("$ret0", "$ret1") := (let: "$a0" := (![#(mapT uint64T unit)] "free") in
    (func_call #semantics.semantics #"allocate"%go) "$a0") in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
    do:  "$r0";;;
    do:  ("ok2" <-[#boolT] "$r1");;;
    let: "ok3" := (alloc (zero_val boolT)) in
    let: ("$ret0", "$ret1") := (let: "$a0" := (![#(mapT uint64T unit)] "free") in
    (func_call #semantics.semantics #"allocate"%go) "$a0") in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
...
Definition testExplicitBlockStmt : val :=
  rec: "testExplicitBlockStmt" <> :=
    exception_do (let: "x" := (alloc (zero_val intT)) in
    let: "$r0" := #(W64 10) in
    do:  ("x" <-[#intT] "$r0");;;
...
Definition testMinUint64 : val :=
  rec: "testMinUint64" <> :=
    exception_do (let: "x" := (alloc (zero_val uint64T)) in
    let: "$r0" := #(W64 10) in
    do:  ("x" <-[#uint64T] "$r0");;;
...
Definition adder : val :=
  rec: "adder" <> :=
    exception_do (let: "sum" := (alloc (zero_val uint64T)) in
    let: "$r0" := #(W64 0) in
    do:  ("sum" <-[#uint64T] "$r0");;;
    return: ((λ: "x",
...
Definition testClosureBasic : val :=
  rec: "testClosureBasic" <> :=
    exception_do (let: "pos" := (alloc (zero_val funcT)) in
    let: "$r0" := ((func_call #semantics.semantics #"adder"%go) #()) in
    do:  ("pos" <-[#funcT] "$r0");;;
    let: "doub" := (alloc (zero_val funcT)) in
    let: "$r0" := ((func_call #semantics.semantics #"adder"%go) #()) in
    do:  ("doub" <-[#funcT] "$r0");;;
    (let: "i" := (alloc (zero_val uint64T)) in
    let: "$r0" := #(W64 0) in
...
Definition testCompareAll : val :=
  rec: "testCompareAll" <> :=
    ...
    let: "$r0" := #true in
    do:  ("ok" <-[#boolT] "$r0");;;
    let: "nok" := (alloc (zero_val boolT)) in
    let: "$r0" := #false in
    do:  ("nok" <-[#boolT] "$r0");;;
...
Definition testCompareGT : val :=
  rec: "testCompareGT" <> :=
    ...
    let: "$r0" := #(W64 4) in
    do:  ("x" <-[#uint64T] "$r0");;;
    let: "y" := (alloc (zero_val uint64T)) in
    let: "$r0" := #(W64 5) in
    do:  ("y" <-[#uint64T] "$r0");;;
    let: "ok" := (alloc (zero_val boolT)) in
    let: "$r0" := #true in
...
Definition stringToByteSlice : val :=
  rec: "stringToByteSlice" "s" :=
    ...
    let: "p" := (alloc (zero_val sliceT)) in
...
Definition testByteSliceToString : val :=
  rec: "testByteSliceToString" <> :=
    exception_do (let: "x" := (alloc (zero_val sliceT)) in
...
Definition testCopySimple : val :=
  rec: "testCopySimple" <> :=
    ...
    let: "y" := (alloc (zero_val sliceT)) in
    let: "$r0" := (slice.make2 #byteT #(W64 10)) in
    do:  ("y" <-[#sliceT] "$r0");;;
...
Definition testCopyShorterDst : val :=
  rec: "testCopyShorterDst" <> :=
    ...
    let: "$r0" := (slice.make2 #byteT #(W64 10)) in
    do:  ("y" <-[#sliceT] "$r0");;;
    let: "n" := (alloc (zero_val uint64T)) in
...
Definition deferSimple : val :=
  rec: "deferSimple" <> :=
    with_defer: (let: "x" := (alloc (zero_val ptrT)) in
...
Definition roundtripEncDec32 : val :=
  rec: "roundtripEncDec32" "x" :=
    exception_do (let: "x" := (alloc "x") in
    let: "r" := (alloc (zero_val sliceT)) in
...
Definition Editor__AdvanceReturn : val :=
  rec: "Editor__AdvanceReturn" "e" "next" :=
    ...
    let: "tmp" := (alloc (zero_val uint64T)) in
...
Definition failing_testFunctionOrdering : val :=
  rec: "failing_testFunctionOrdering" <> :=
    exception_do (let: "arr" := (alloc (zero_val sliceT)) in
    let: "$r0" := (slice.make2 #uint64T #(W64 5)) in
    do:  ("arr" <-[#sliceT] "$r0");;;
    let: "e1" := (alloc (zero_val Editor)) in
    ...
    do:  ("e1" <-[#Editor] "$r0");;;
    let: "e2" := (alloc (zero_val Editor)) in
    let: "$r0" := (let: "$s" := (let: "$s" := (![#sliceT] "arr") in
    slice.slice #uint64T "$s" #(W64 0) (slice.len "$s")) in
...
Definition testU64ToU32 : val :=
  rec: "testU64ToU32" <> :=
    ...
    let: "$r0" := #(W64 1230) in
    do:  ("x" <-[#uint64T] "$r0");;;
    let: "y" := (alloc (zero_val uint32T)) in
    let: "$r0" := #(W32 1230) in
    do:  ("y" <-[#uint32T] "$r0");;;
...
Definition testU32Len : val :=
  rec: "testU32Len" <> :=
    exception_do (let: "s" := (alloc (zero_val sliceT)) in
...
Definition testBasicInterface : val :=
  rec: "testBasicInterface" <> :=
    exception_do (let: "s" := (alloc (zero_val SquareStruct)) in
...
Definition testAssignInterface : val :=
  rec: "testAssignInterface" <> :=
    ...
    do:  ("s" <-[#SquareStruct] "$r0");;;
    let: "area" := (alloc (zero_val uint64T)) in
...
Definition testMultipleInterface : val :=
  rec: "testMultipleInterface" <> :=
    ...
    do:  ("s" <-[#SquareStruct] "$r0");;;
    let: "square1" := (alloc (zero_val uint64T)) in
    let: "$r0" := (let: "$a0" := (interface.make #semantics.semantics #"SquareStruct" (![#SquareStruct] "s")) in
    (func_call #semantics.semantics #"measureArea"%go) "$a0") in
    do:  ("square1" <-[#uint64T] "$r0");;;
    let: "square2" := (alloc (zero_val uint64T)) in
    let: "$r0" := (let: "$a0" := (interface.make #semantics.semantics #"SquareStruct" (![#SquareStruct] "s")) in
    (func_call #semantics.semantics #"measureArea"%go) "$a0") in
...
Definition testsUseLocks : val :=
  rec: "testsUseLocks" <> :=
    exception_do (let: "m" := (alloc (zero_val ptrT)) in
...
Definition standardForLoop : val :=
  rec: "standardForLoop" "s" :=
    exception_do (let: "s" := (alloc "s") in
    let: "sumPtr" := (alloc (zero_val ptrT)) in
    let: "$r0" := (alloc (zero_val uint64T)) in
    do:  ("sumPtr" <-[#ptrT] "$r0");;;
    (let: "i" := (alloc (zero_val uint64T)) in
    let: "$r0" := #(W64 0) in
...
Definition LoopStruct__forLoopWait : val :=
  rec: "LoopStruct__forLoopWait" "ls" "i" :=
    ...
    let: "i" := (alloc "i") in
    (for: (λ: <>, #true); (λ: <>, Skip) := λ: <>,
      let: "nxt" := (alloc (zero_val ptrT)) in
...
Definition testForLoopWait : val :=
  rec: "testForLoopWait" <> :=
    exception_do (let: "ls" := (alloc (zero_val LoopStruct)) in
    let: "$r0" := (let: "$loopNext" := (alloc (zero_val uint64T)) in
...
Definition testBreakFromLoopWithContinue : val :=
  rec: "testBreakFromLoopWithContinue" <> :=
    exception_do (let: "i" := (alloc (zero_val uint64T)) in
    let: "$r0" := #(W64 0) in
    do:  ("i" <-[#uint64T] "$r0");;;
    (for: (λ: <>, #true); (λ: <>, Skip) := λ: <>,
      (if: #true
...
Definition testNestedLoops : val :=
  rec: "testNestedLoops" <> :=
    exception_do (let: "ok1" := (alloc (zero_val boolT)) in
    let: "$r0" := #false in
    do:  ("ok1" <-[#boolT] "$r0");;;
    let: "ok2" := (alloc (zero_val boolT)) in
    let: "$r0" := #false in
    ...
    do:  ("i" <-[#uint64T] "$r0");;;
    (for: (λ: <>, #true); (λ: <>, Skip) := λ: <>,
      (let: "j" := (alloc (zero_val uint64T)) in
      let: "$r0" := #(W64 0) in
      do:  ("j" <-[#uint64T] "$r0");;;
      (for: (λ: <>, #true); (λ: <>, Skip) := λ: <>,
        (if: (![#uint64T] "j") > #(W64 5)
...
Definition testIterateMap : val :=
  rec: "testIterateMap" <> :=
    ...
    let: "$r0" := #true in
    do:  ("ok" <-[#boolT] "$r0");;;
    let: "m" := (alloc (zero_val (mapT uint64T uint64T))) in
...
Definition testAssignThree : val :=
  rec: "testAssignThree" <> :=
    ...
    let: "$r0" := #(W64 10) in
    do:  ("x" <-[#uint64T] "$r0");;;
    let: "y" := (alloc (zero_val boolT)) in
    let: "$r0" := #false in
    do:  ("y" <-[#boolT] "$r0");;;
    let: "z" := (alloc (zero_val uint32T)) in
    let: "$r0" := #(W32 15) in
    do:  ("z" <-[#uint32T] "$r0");;;
...
Definition testReturnTwo : val :=
  rec: "testReturnTwo" <> :=
    exception_do (let: "y" := (alloc (zero_val uint64T)) in
    let: "x" := (alloc (zero_val uint64T)) in
...
Definition testReturnThree : val :=
  rec: "testReturnThree" <> :=
    exception_do (let: "z" := (alloc (zero_val uint32T)) in
    let: "y" := (alloc (zero_val boolT)) in
    let: "x" := (alloc (zero_val uint64T)) in
...
Definition testReturnFour : val :=
  rec: "testReturnFour" <> :=
    exception_do (let: "w" := (alloc (zero_val uint64T)) in
    let: "z" := (alloc (zero_val uint32T)) in
    let: "y" := (alloc (zero_val boolT)) in
    let: "x" := (alloc (zero_val uint64T)) in
...
Definition testComparePointerToNil : val :=
  rec: "testComparePointerToNil" <> :=
    exception_do (let: "s" := (alloc (zero_val ptrT)) in
    let: "$r0" := (alloc (zero_val uint64T)) in
    do:  ("s" <-[#ptrT] "$r0");;;
    return: ((![#ptrT] "s") ≠ #null)).
...
Definition testDivisionPrecedence : val :=
  rec: "testDivisionPrecedence" <> :=
    exception_do (let: "blockSize" := (alloc (zero_val uint64T)) in
    let: "$r0" := #(W64 4096) in
    do:  ("blockSize" <-[#uint64T] "$r0");;;
    let: "hdrmeta" := (alloc (zero_val uint64T)) in
    let: "$r0" := #(W64 8) in
    do:  ("hdrmeta" <-[#uint64T] "$r0");;;
    let: "hdraddrs" := (alloc (zero_val uint64T)) in
...
Definition testModPrecedence : val :=
  rec: "testModPrecedence" <> :=
    exception_do (let: "x1" := (alloc (zero_val intT)) in
    let: "$r0" := #(W64 (513 + (12 `rem` 8))) in
    do:  ("x1" <-[#intT] "$r0");;;
    let: "x2" := (alloc (zero_val intT)) in
    let: "$r0" := #(W64 ((513 + 12) `rem` 8)) in
    do:  ("x2" <-[#intT] "$r0");;;
...
Definition testBitAddAnd : val :=
  rec: "testBitAddAnd" <> :=
    exception_do (let: "tid" := (alloc (zero_val uint64T)) in
    let: "$r0" := #(W64 17) in
    do:  ("tid" <-[#uint64T] "$r0");;;
    let: "n" := (alloc (zero_val uint64T)) in
    let: "$r0" := #(W64 16) in
...
Definition testShortcircuitAndTF : val :=
  rec: "testShortcircuitAndTF" <> :=
    exception_do (let: "b" := (alloc (zero_val ptrT)) in
...
Definition testOverwriteArray : val :=
  rec: "testOverwriteArray" <> :=
    ...
    let: "$r0" := (slice.make2 #uint64T #(W64 4)) in
    do:  ("arr" <-[#sliceT] "$r0");;;
    let: "ae1" := (alloc (zero_val ptrT)) in
    ...
    do:  ("ae1" <-[#ptrT] "$r0");;;
    let: "ae2" := (alloc (zero_val ptrT)) in
    let: "$r0" := (alloc (let: "$s" := (let: "$s" := (![#sliceT] "arr") in
    slice.slice #uint64T "$s" #(W64 1) (slice.len "$s")) in
...
Definition testSliceLiteral : val :=
  rec: "testSliceLiteral" <> :=
    exception_do (let: "bytes" := (alloc (zero_val sliceT)) in
    ...
    do:  ("ok" <-[#boolT] "$r0");;;
    let: "ints" := (alloc (zero_val sliceT)) in
...
Definition testSliceAppend : val :=
  rec: "testSliceAppend" <> :=
    ...
    let: "$r0" := #true in
    do:  ("ok" <-[#boolT] "$r0");;;
    let: "bytes" := (alloc (zero_val sliceT)) in
    let: "$r0" := (slice.make2 #byteT #(W64 0)) in
    do:  ("bytes" <-[#sliceT] "$r0");;;
    ...
    do:  ("bytes" <-[#sliceT] "$r0");;;
    let: "newBytes" := (alloc (zero_val sliceT)) in
...
Definition testFooBarMutation : val :=
  rec: "testFooBarMutation" <> :=
    exception_do (let: "x" := (alloc (zero_val Foo)) in
...
Definition testStructUpdates : val :=
  rec: "testStructUpdates" <> :=
    ...
    do:  ("ok" <-[#boolT] "$r0");;;
    let: "b1" := (alloc (zero_val TwoInts)) in
...
Definition testStructConstructions : val :=
  rec: "testStructConstructions" <> :=
    ...
    let: "$r0" := #true in
    do:  ("ok" <-[#boolT] "$r0");;;
    let: "p1" := (alloc (zero_val ptrT)) in
    let: "p2" := (alloc (zero_val TwoInts)) in
    let: "p3" := (alloc (zero_val TwoInts)) in
    ...
    do:  ("p3" <-[#TwoInts] "$r0");;;
    let: "p4" := (alloc (zero_val TwoInts)) in
    ...
    do:  ("ok" <-[#boolT] "$r0");;;
    let: "$r0" := (alloc (zero_val TwoInts)) in
    do:  ("p1" <-[#ptrT] "$r0");;;
...
Definition testIncompleteStruct : val :=
  rec: "testIncompleteStruct" <> :=
    ...
    let: "$r0" := #true in
    do:  ("ok" <-[#boolT] "$r0");;;
    let: "p1" := (alloc (zero_val TwoInts)) in
    let: "$r0" := (let: "$x" := #(W64 0) in
    struct.make TwoInts [{
      "x" ::= "$x";
      "y" ::= zero_val uint64T
...
Definition testStoreInStructVar : val :=
  rec: "testStoreInStructVar" <> :=
    exception_do (let: "p" := (alloc (zero_val StructWrap)) in
...
Definition testStoreInStructPointerVar : val :=
  rec: "testStoreInStructPointerVar" <> :=
    exception_do (let: "p" := (alloc (zero_val ptrT)) in
    let: "$r0" := (alloc (zero_val StructWrap)) in
    do:  ("p" <-[#ptrT] "$r0");;;
    let: "$r0" := #(W64 5) in
...
Definition testStructFieldFunc : val :=
  rec: "testStructFieldFunc" <> :=
    exception_do (let: "a" := (alloc (zero_val ptrT)) in
    let: "$r0" := (alloc (zero_val StructWithFunc)) in
    do:  ("a" <-[#ptrT] "$r0");;;
    let: "$r0" := (λ: "arg",
...
Definition testSwitchConversion : val :=
  rec: "testSwitchConversion" <> :=
    exception_do (let: "v" := (alloc (zero_val ptrT)) in
    let: "$r0" := (alloc (struct.make switchConcrete [{
    }])) in
    do:  ("v" <-[#ptrT] "$r0");;;
    let: "x" := (alloc (zero_val switchInterface)) in
...
Definition testPointerAssignment : val :=
  rec: "testPointerAssignment" <> :=
    exception_do (let: "x" := (alloc (zero_val boolT)) in
    let: "$r0" := #true in
    do:  ("x" <-[#boolT] "$r0");;;
    return: (![#boolT] "x")).
...
Definition testAddressOfLocal : val :=
  rec: "testAddressOfLocal" <> :=
    ...
    let: "$r0" := #false in
    do:  ("x" <-[#boolT] "$r0");;;
    let: "xptr" := (alloc (zero_val ptrT)) in
    let: "$r0" := "x" in
    do:  ("xptr" <-[#ptrT] "$r0");;;
    let: "$r0" := #true in
