...
Definition ReadInt32ⁱᵐᵖˡ {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: "b",
    ...
    let: "$r0" := (let: "$a0" := (![go.SliceType go.byte] "b") in
    (FuncResolve primitive.UInt32Get [] #()) "$a0") in
    do:  ("i" <-[go.uint32] "$r0");;;
...
{|
  pkg_imported_pkgs := [code.github_com.goose_lang.primitive.pkg_id.primitive; code.github_com.goose_lang.std.pkg_id.std]
|}.
