...
Definition Enc__PutInt : val :=
  rec: "Enc__PutInt" "enc" "x" :=
    ...
    do:  "off" <-[uint64T] "$a0";;;
    do:  primitive.UInt64Put (let: "$s" := ![sliceT byteT] (struct.field_ref Enc "b" "enc") in
...
Definition Enc__PutInt32 : val :=
  rec: "Enc__PutInt32" "enc" "x" :=
    ...
    let: "$a0" := ![uint64T] (![ptrT] (struct.field_ref Enc "off" "enc")) in
    do:  "off" <-[uint64T] "$a0";;;
    do:  primitive.UInt32Put (let: "$s" := ![sliceT byteT] (struct.field_ref Enc "b" "enc") in
...
Definition Dec__GetInt : val :=
  rec: "Dec__GetInt" "dec" <> :=
    ...
    return: (primitive.UInt64Get (let: "$s" := ![sliceT byteT] (struct.field_ref Dec "b" "dec") in
...
Definition Dec__GetInt32 : val :=
  rec: "Dec__GetInt32" "dec" <> :=
    ...
    return: (primitive.UInt32Get (let: "$s" := ![sliceT byteT] (struct.field_ref Dec "b" "dec") in
     slice.slice byteT "$s" (![uint64T] "off") (slice.len "$s")));;;
    do:  #()).
...
Definition ReadInt : val :=
  rec: "ReadInt" "b" :=
    exception_do (let: "b" := ref_ty (sliceT byteT) "b" in
    let: "i" := ref_ty uint64T (zero_val uint64T) in
    let: "$a0" := primitive.UInt64Get (![sliceT byteT] "b") in
    do:  "i" <-[uint64T] "$a0";;;
...
Definition ReadInt32 : val :=
  rec: "ReadInt32" "b" :=
    ...
    let: "$a0" := primitive.UInt32Get (![sliceT byteT] "b") in
    do:  "i" <-[uint32T] "$a0";;;
...
Definition WriteInt : val :=
  rec: "WriteInt" "b" "i" :=
    ...
    do:  primitive.UInt64Put (let: "$s" := ![sliceT byteT] "b3" in
...
Definition WriteInt32 : val :=
  rec: "WriteInt32" "b" "i" :=
    ...
    do:  "b3" <-[sliceT byteT] "$a0";;;
    do:  primitive.UInt32Put (let: "$s" := ![sliceT byteT] "b3" in
