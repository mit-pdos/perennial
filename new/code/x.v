...
Module pkg_id.
Definition main : go_string := "x".

End pkg_id.
Export pkg_id.
Module main.

Definition Worker {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go.type := go.Named "x.Worker"%go [].

#[global] Opaque Worker.

Definition wordCount {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "x.wordCount"%go.

Definition main {ext : ffi_syntax} {go_gctx : GoGlobalContext} : go_string := "x.main"%go.
...
Definition mainⁱᵐᵖˡ {ext : ffi_syntax} {go_gctx : GoGlobalContext} : val :=
  λ: <>,
    ...
    do:  (let: "$a0" := #"word count: %d (expected %d)
    "%go in
    let: "$a1" := ((let: "$sl0" := (Convert go.int64 go.any (![go.int64] "got")) in
    let: "$sl1" := (Convert go.int64 go.any (![go.int64] "want")) in
...
{|
  pkg_imported_pkgs := [code.fmt.pkg_id.fmt; code.strings.pkg_id.strings; code.sync.pkg_id.sync; code.sync.atomic.pkg_id.atomic]
|}.
