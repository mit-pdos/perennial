...
Class IsPkgInit (pkg_name : go_string) :=
  {
    is_pkg_init_deps : PROP; (* autogenerated; includes [is_pkg_defined] *)
...
Definition is_pkg_init (pkg_name : go_string) `{!IsPkgInit pkg_name} : PROP :=
...
(** Maps [pkg_name] to a pure predicate that constrains [get_is_pkg_init] to
    ...
    dependencies. *)
Class GetIsPkgInitWf (pkg_name : go_string) :=
  {
    get_is_pkg_init_prop_def (get_is_pkg_init : go_string → PROP) : Prop
  }.
Notation get_is_pkg_init_prop := get_is_pkg_init_prop_def.
Global Arguments get_is_pkg_init_prop (pkg_name) {_} (get_is_pkg_init).

End package_init_and_defined.

#[global] Arguments IsPkgInit (_) (_) : clear implicits.
#[global] Hint Mode IsPkgInit + + : typeclass_instances.
#[global] Arguments GetIsPkgInitWf (_) (_) : clear implicits.
...
Local Ltac2 build_pkg_init_deps name :=
  Control.refine
    (fun () =>
       ...
       let prop_ty := Control.goal () in
       let rec build_iprop deps :=
         lazy_match! deps with
         | cons ?pkg ?deps =>
             let rest := build_iprop deps in
             constr:((is_pkg_init $pkg ∗ $rest : $prop_ty)%I)
         | nil => constr:(True%I : $prop_ty)
         | _ =>
...
Local Ltac2 build_is_pkg_init (is_pkg_init_def : preterm) :=
  Control.refine
    (fun () =>
       ...
       | [ |- IsPkgInit ?prop ?pkg_name ] =>
           constr:(@Build_IsPkgInit $prop $pkg_name ltac2:(build_pkg_init_deps pkg_name) $preterm:is_pkg_init_def)
  ...
  Control.refine
    (fun () =>
       ...
       | [ |- @GetIsPkgInitWf ?prop ?name] =>
           ...
           constr:(Build_GetIsPkgInitWf (PROP:=$prop) $name $p)
