...
Proof.
  ...
  - repeat setoid_rewrite bi.and_exist_r.
    ...
    iNamed "HΦ". iAssert (⌜ v = #v' ⌝ ∗ IsChan ch γ)%I with "[-]" as "[-> #?]".
...
Local Lemma wp_try_select_case_nonblocking cs Ψ :
  ...
  (match cs with
   | select_send_f t send_val send_chan send_handler =>
     ...
     ⌜ send_val = #v ⌝ ∗
     IsChan (V:=V) (t:=t) send_chan γ ∗
     NonBlockingSendAU send_chan v γ (WP #send_handler #() {{ Ψ }}) True
  | select_receive_f t recv_chan recv_handler =>
      ∃ V γ `(!IntoVal V) `(!IntoValTyped V t) `(!chanG Σ V),
     IsChan (V:=V) (t:=t) recv_chan γ ∗
     NonBlockingRecvAU recv_chan γ (λ (v: V) ok,
...
Proof.
  ...
  wp_call. rewrite [in (_ op)]to_val_unseal /=. destruct cs; wp_auto.
  - repeat setoid_rewrite bi.and_exist_r.
    iNamed "HΦ". iAssert (⌜ v = #a1 ⌝ ∗ IsChan ch a0)%I with "[-]" as "[-> #?]".
    { iLeft in "HΦ". iDestruct "HΦ" as "(-> & #? & Hau)". iFrame "#". done. }
    wp_apply (wp_TrySend with "[$]").
    iLeft. iSplit.
    + iLeft in "HΦ". iDestruct "HΦ" as "(_ & _ & Hau)".
    ...
    + wp_auto. iRight in "HΦ". done.
  - repeat setoid_rewrite bi.and_exist_r.
    iNamed "HΦ". iAssert (IsChan ch a0)%I with "[-]" as "#?".
    { iLeft in "HΦ". iDestruct "HΦ" as "(#? & Hau)". iFrame "#". }
    wp_apply (wp_TryReceive with "[$]").
    iLeft. iSplit.
    + iLeft in "HΦ". iDestruct "HΦ" as "[_ Hau]".
  ...
  ([∧ list] case ∈ cases,
     ...
     | select_send_f t send_val send_chan send_handler =>
         ∃ V γ (v : V) `(!IntoVal V) `(!chanG Σ V) `(!IntoValTyped V t),
             ⌜ send_val = #v ⌝ ∗
             IsChan (V:=V) (t:=t) send_chan γ ∗
             NonBlockingSendAU send_chan v γ (WP #send_handler #() {{ Φ }}) True
     | select_receive_f t recv_chan recv_handler =>
         ∃ V γ `(!IntoVal V) `(!IntoValTyped V t) `(!chanG Σ V),
             IsChan (V:=V) (t:=t) recv_chan γ ∗
             NonBlockingRecvAU recv_chan γ (λ (v: V) ok,
...
Local Lemma wp_try_select_case_nonblocking_alt cs Ψ Ψnotready :
  ...
  (match cs with
   | select_send_f t send_val send_chan send_handler =>
     ...
     ⌜ send_val = #v ⌝ ∗
     IsChan (V:=V) (t:=t) send_chan γ ∗
     NonBlockingSendAU_alt send_chan v γ (P -∗ WP #send_handler #() {{ Ψ }}) Ψnotready
  | select_receive_f t recv_chan recv_handler =>
      ∃ V γ `(!IntoVal V) `(!IntoValTyped V t) `(!chanG Σ V),
     IsChan (V:=V) (t:=t) recv_chan γ ∗
     NonBlockingRecvAU_alt recv_chan γ
  ...
  ([∗ list] case; Φnr ∈ cases; Φnrs,
     ...
     | select_send_f t send_val send_chan send_handler =>
         (∃ V γ (v : V) `(!IntoVal V) `(!chanG Σ V) `(!IntoValTyped V t),
             ⌜ send_val = #v ⌝ ∗
             IsChan (V:=V) (t:=t) send_chan γ ∗
             NonBlockingSendAU_alt send_chan v γ (P -∗ WP #send_handler #() {{ Ψ }})
               Φnr)
     | select_receive_f t recv_chan recv_handler =>
         (∃ V γ `(!IntoVal V) `(!IntoValTyped V t) `(!chanG Σ V),
             IsChan (V:=V) (t:=t) recv_chan γ ∗
  ...
  ([∗ list] case; Φnr ∈ cases; Φnrs,
     ...
     | select_send_f t send_val send_chan send_handler =>
         (∃ V γ (v : V) `(!IntoVal V) `(!chanG Σ V) `(!IntoValTyped V t),
             ⌜ send_val = #v ⌝ ∗
             IsChan (V:=V) (t:=t) send_chan γ ∗
             NonBlockingSendAU_alt send_chan v γ (P -∗ WP #send_handler #() {{ Φ }}) Φnr)
     | select_receive_f t recv_chan recv_handler =>
         (∃ V γ `(!IntoVal V) `(!IntoValTyped V t) `(!chanG Σ V),
             IsChan (V:=V) (t:=t) recv_chan γ ∗
             NonBlockingRecvAU_alt recv_chan γ
