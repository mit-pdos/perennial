...
Section goose_lang.
  ...
  Proof.
    ...
    all: repeat ltac2:(step ()).
    {
      ...
      move a after a0. subst.
      dependent induction a generalizing a0.
      { destruct a0; done. }
      dependent destruction a0; first done.
      simpl in Heq.
  ...
  Lemma wp_alloc stk E :
    {{{ True }}}
      mem.alloc t (#()) @ stk; E
    {{{ l, RET #l; l ↦ (default_val V) }}}.
  Proof.
    iIntros (Φ) "_ HΦ".
    rewrite mem.alloc_unseal.
    wp_call.
  ...
  Lemma wp_mem_store stk E l v v' :
    {{{ ▷ l ↦ v }}}
      (#l <-[t] #v')%V @ stk; E
    {{{ RET #(); l ↦ v' }}}.
  Proof using IntoValTyped0.
    ...
    - (* array *)
      rename a0 into a'.
      iInduction a as [|] "IH2" forall (l' a' Helems0 Hlen0).
      ...
      dependent destruction a'; first done.
      simpl.
      wp_pures.
      wp_apply ("IH" with "[] [] [$Hf]").
...
Section tac_lemmas.
  ...
  Lemma tac_wp_load {V t} `{!IntoVal V} `{!IntoValTyped V t}
    ...
    envs_entails Δ (WP (fill K (mem.load t #l)) @ s; E {{ Φ }}).
  Proof using Type*.
    ...
    - iDestruct "H" as "#H".
      ...
      unshelve wp_apply (wp_mem_load with "[$]"); first apply _.
      iIntros "?".
  ...
  Lemma tac_wp_store {V t} `{!IntoVal V} `{!IntoValTyped V t}
    ...
    envs_entails Δ (WP (fill K (mem.store t #l (Val #v'))) @ s; E {{ Φ }}).
  Proof.
    rewrite envs_entails_unseal => ?? HΦ.
    rewrite envs_simple_replace_sound // /=.
    iIntros "[H Henv]".
    iDestruct (points_to_acc with "H") as "[H Hclose]".
    unshelve wp_apply (wp_mem_store with "[$]"); first tc_solve.
  ...
  Lemma tac_wp_alloc
    ...
    K Δ stk E Φ :
    (∀ l, envs_entails Δ (l ↦ (default_val V) -∗ WP (fill K (Val #l)) @ stk; E {{ Φ }})) →
    envs_entails Δ (WP fill K (mem.alloc t #()) @ stk; E {{ Φ }}).
  Proof.
...
Ltac2 wp_load_visit e k :=
  Control.once_plus (fun () => Std.unify e '(mem.load _ (Val _)))
...
Ltac2 wp_store_visit e k :=
  Control.once_plus (fun () => (Std.unify e '(mem.store _ _ (Val _))))
         (fun _ => Control.zero Walk_expr_more);
...
Ltac2 wp_alloc_visit e k :=
  Control.once_plus (fun () => Std.unify e '(mem.alloc _ #()))
    (fun _ => Control.zero Walk_expr_more);
