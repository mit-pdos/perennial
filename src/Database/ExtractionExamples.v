...
Module HashTableBenchmarks.
  ...
  Definition ex1_setup : proc (HashTable uint64 ByteString) :=
    Data.newHashTable _ _.

  Definition ex1_write1 (t:HashTable uint64 ByteString)
             (k:uint64) (bs:ByteString) : proc unit :=
    Data.hashTableAlter t k (fun _ => Some bs).

  Definition ex1_write3 (t:HashTable uint64 ByteString)
             (k:uint64) (bs:ByteString) : proc unit :=
  ...
  Definition ex1_read (t:HashTable uint64 ByteString)
             (k:uint64) : proc (option ByteString) :=
    Data.hashTableLookup t k.

  Definition ex2_seq_reads (t:HashTable uint64 ByteString)
