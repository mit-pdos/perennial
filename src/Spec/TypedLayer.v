...
Record TypedLayer Op :=
  { OpState: Type;
    OpTy : Type;
    sem: Dynamics Op OpState;
    op_has_type : forall ty, Op ty -> OpTy -> Prop;
...
Definition State `(L: TypedLayer Op) := @Proc.State (L.(OpState)).
Coercion sem : TypedLayer >-> Dynamics.
...
Record LayerImpl C_Op Op :=
  { compile_op `(op: Op T1) : proc C_Op T2;
