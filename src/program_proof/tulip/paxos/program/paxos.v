...
Section repr.
  ...
  Definition own_paxos_leader_only
    (termc : u64) (log : list string) (lsnpeersP : loc) γ : iProp Σ :=
    ∃ (lsnpeers : gmap u64 u64),
      "Hps"        ∷ own_proposal γ (uint.nat termc) log ∗
      "Hlsnpeers"  ∷ own_map lsnpeersP (DfracOwn 1) lsnpeers ∗
      "#Hpfb"      ∷ prefix_base_ledger γ (uint.nat termc) log ∗
      "%Hlelenlog" ∷ ⌜map_Forall (λ _ i, (uint.nat i ≤ length log)%nat) lsnpeers⌝.

  Definition own_paxos_leader
    (paxos : loc) (termc : u64) (log : list string) (isleader : bool) γ : iProp Σ :=
    ∃ (lsnpeersP : loc),
      "HisleaderP" ∷ paxos ↦[Paxos :: "isleader"] #isleader ∗
      "HlsnpeersP" ∷ paxos ↦[Paxos :: "lsnpeers"] #lsnpeersP ∗
      "Honlyl"     ∷ (if isleader then own_paxos_leader_only termc log lsnpeersP γ else True).
