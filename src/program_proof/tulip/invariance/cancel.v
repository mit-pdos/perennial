...
Section inv.
  ...
  Proof.
    iIntros (Hhead) "Hwabt Hwrsexcl Htxnsys".
    do 2 iNamed "Htxnsys".
    (* Delete [tid] from [txns_abt_auth]. *)
    iDestruct (wabt_tid_elem_of with "Htidas Hwabt") as %Htid.
    iMod (wabt_tid_delete with "Hwabt Htidas") as "Htidas".
    ...
    iAssert (|==> own_txn_resm γ resm')%I with "[Hresm]" as "Hresm".
    { subst resm'.
      destruct Hresm as [Hnone | Hwabt]; last by rewrite insert_id.
      by iApply (txn_res_insert with "Hresm").
    }
    iMod "Hresm" as "Hresm".
    ...
    iMod (txn_oneshot_wrs_cancel with "Hwrsexcl") as "Hccl".
