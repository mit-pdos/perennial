...
Module RoApplyAsBackupArgs.
Section RoApplyAsBackupArgs.
Context `{!heapGS Σ}.
...
Definition has_encoding (encoded:list u8) (args:C) : Prop :=
  encoded = (u64_le args.(epoch)) ++ (u64_le args.(nextIndex)).

Definition own args_ptr args : iProp Σ :=
  "Hargs_epoch" ∷ args_ptr ↦[pb.RoApplyAsBackupArgs :: "epoch"] #args.(epoch) ∗
  "Hargs_index" ∷ args_ptr ↦[pb.RoApplyAsBackupArgs :: "nextIndex"] #args.(nextIndex)
  .

Definition own_ro args_ptr args : iProp Σ :=
  "#Hargs_epoch" ∷ readonly (args_ptr ↦[pb.RoApplyAsBackupArgs :: "epoch"] #args.(epoch)) ∗
  "#Hargs_index" ∷ readonly (args_ptr ↦[pb.RoApplyAsBackupArgs :: "nextIndex"] #args.(nextIndex))
.
...
Lemma wp_Encode (args_ptr:loc) (args:C) :
  {{{
    ...
    pb.EncodeRoApplyAsBackupArgs #args_ptr
  {{{
        enc enc_sl, RET (slice_val enc_sl);
        ⌜has_encoding enc args⌝ ∗
        is_slice enc_sl byteT 1 enc
...
Lemma wp_Decode enc enc_sl (args:C) :
  {{{
    ...
    pb.DecodeRoApplyAsBackupArgs (slice_val enc_sl)
  {{{
        args_ptr, RET #args_ptr; own args_ptr args
  }}}.
Proof.
