...
Lemma wp_MakeClerk2 hosts hosts_sl γ γconf :
  {{{
      ...
      "#Hhosts" ∷ ([∗ list] host ∈ hosts, is_pb_config_host host γ γconf) ∗
...
Proof.
  iIntros (Φ) "Hpre HΦ".
  iNamed "Hpre".
  wp_apply (config2_proof.wp_MakeClerk with "[]").
  {
    iFrame "#%".
  ...
  {
    destruct hosts.
    { exfalso. simpl in Hnonempty. lia. }
    rewrite big_sepL_cons.
    iDestruct "Hhosts" as "[H Hhosts]".
    iDestruct "H" as "[? ?]".
    iFrame "#".
  }
  iFrame "#".
