...
Lemma crash_heaps_match_heapmatch_latest γ logm crash_heaps :
    "Hmetactx" ∷ map_ctx γ.(txn_metaheap) 1 ∅ ∗
    "Hcrashheaps0" ∷ ([∗ map] a↦b ∈ (latest crash_heaps),
                      ∃ hb : heap_block, ⌜hb_latest_update hb = b⌝ ∗ mapsto (hG:=γ.(txn_walnames).(wal_heap_h)) a 1 hb) ∗
...
Theorem wpc_MkTxn (d:loc) dinit (γ:txn_names) logm k :
  ...
  {{{ is_txn_durable γ dinit ∗ txn_resources γ logm  ∨
      (∃ γ' logm',
       ⌜ txn_kinds γ' = txn_kinds γ ⌝ ∗ is_txn_durable γ' dinit ∗ txn_resources γ' logm') }}}.
Proof.
  ...
  - iSplit.
    { iLeft in "HΦ".
      ...
      - iDestruct "HP" as (ls1 γ'walnames Hcrashls12) "(Hdur' & Hres' & HP)".
        ...
        { admit. (* TODO: this should come from Hcrash_heaps0, but it's needed later for some reason *) }
