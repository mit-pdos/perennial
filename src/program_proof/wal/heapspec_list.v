...
Definition wal_heap_inv (γdur : wh_durable_gnames) (γeph : wh_ephem_gnames) (ls : log_state.t) : iProp Σ :=
  ∃ (crash_heaps : async (gmap u64 Block)),
    "%Hlast" ∷ ⌜ ∀ (a: u64), last_disk ls !! (uint.Z a) = latest crash_heaps !! a ⌝ ∗
...
Lemma wal_heap_inv_crashes_list_at_ids (σ : log_state.t) (heaps: list (gmap u64 Block)):
  ...
  ⌜ ∀ (i : nat) (d: gmap u64 Block), heaps !! i = Some d →
           (∀ a : u64, (disk_at_txn_id i σ !! uint.Z a) = d !! a)⌝.
Proof.
...
Lemma apply_upds_apply_upds_u64 bs d σ a :
  d !! uint.Z a = σ !! a →
  apply_upds bs d !! uint.Z a = apply_upds_u64 bs σ !! a.
Proof.
...
Proof using walheapG0.
  ...
  { admit. }
  intros. rewrite /ge.
  cut (uint.Z pos ≤ uint.Z pos')%Z; first lia.
  eauto.
Admitted.
...
Proof using walheapG0.
  ...
  { iIntros (σ σ' mb) "%Hwal_wf %Hrelation Hwalinv".
    ...
    destruct x.
    {
      simpl in *; monad_inv.
      simpl in *; monad_inv.
      destruct (last_disk σ !! uint.Z blkno) as [b|] eqn:Heq; last by monad_inv.
