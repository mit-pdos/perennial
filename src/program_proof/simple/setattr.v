...
Theorem wp_NFSPROC3_SETATTR γ (nfs : loc) (fh : u64) (fhslice : Slice.t) (sattr: SimpleNFS.sattr) (Q : SimpleNFS.res unit -> iProp Σ) dinit :
  ...
  {{{ v,
      RET v;
      ( ⌜ getField_f nfstypes.SETATTR3res "Status" v = #(I32 0) ⌝ ∗
        Q (SimpleNFS.OK tt) ) ∨
      ( ∃ stat,
        ⌜ getField_f nfstypes.SETATTR3res "Status" v = #(I32 stat) ⌝ ∗
        ⌜ stat ≠ 0 ⌝ ∗
        Q SimpleNFS.Err )
  }}}.
  ...
  destruct sattr_size; wp_if.
  {
      ...
      iIntros (wcount ok) "[Hjrnl_mem [(Hinode_mem & Hinode_enc & Hinode_data & %Hok) | Hok]]"; intuition subst.
      {
...

        {
          ...
          { iIntros "[[H _]|[H0 H1]]".
            ...
            assert (length state < uint.Z u)%Z by (revert Heqb; word).
            ...
            2: word.
            replace (length state + (uint.Z u - length state))%Z with (uint.Z u) by lia.
            replace (length state + (uint.Z u - length state))%Z with (uint.Z u) by lia.
            replace (length state + (uint.nat u - length state)) with (uint.nat u) by lia.
            replace (I64 (uint.nat u)) with (I64 (uint.Z u)) by word. iFrame.
            ...
            replace (Z.to_nat (uint.Z u - length state)) with (uint.nat u - length state) by lia.
            iFrame.
          }
    ...
    }
    {
      ...
      { iIntros "[[H _]|[H0 H1]]".
        ...
        { word. }
        rewrite -> firstn_length_le by word.
        replace (I64 (Z.of_nat (uint.nat u))) with u by word. iFrame.
      }
