...
Proof.
  ...
  replace (word.mul (word.mul inum 128) 8)%Z with (I64 (int.nat inum * 128 * 8)%nat).
  { iApply "HΦ". done. }
...
Proof.
  ...
  split; [ | word ].
  assert (i ≠ I64 0) as Hnot_0%(not_inj (f:=int.Z)) by congruence.
  assert (i ≠ I64 1) as Hnot_1%(not_inj (f:=int.Z)) by congruence.
