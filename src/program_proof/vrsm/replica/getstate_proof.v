...
Lemma wp_Clerk__GetState γ γsrv ck args_ptr (epoch_lb:u64) (epoch:u64) :
  ...
  {{{
        ...
        if (decide (err = I64 0)) then
            ...
            ⌜length (get_rwops opsfull) = uint.nat (I64 (length (get_rwops opsfull)))⌝
          else
...
Lemma is_StateMachine_acc_getstate sm own_StateMachine P :
  ...
  (∃ getstateFn,
    "#Hgetstate" ∷ readonly (sm ↦[StateMachine :: "GetStateAndSeal"] getstateFn) ∗
...
Lemma wp_Server__GetState γ γsrv s args_ptr args epoch_lb Φ Ψ :
  ...
  WP Server__GetState #s #args_ptr {{ Φ }}
  .
Proof.
