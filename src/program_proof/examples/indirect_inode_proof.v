...
Proof.
  ...
  all: iIntros (lref) "Hlock".
  {
    ...
    replace (int.Z (I64 (Z.of_nat (length σ.(inode.blocks))))) with (Z.of_nat (length σ.(inode.blocks))) in HnumDir by word.
    ...
    wp_apply (wp_SliceTake (length σ.(inode.blocks))).
    {
      ...
      replace (int.Z (I64 (Z.of_nat (length σ.(inode.blocks))))) with (Z.of_nat (length σ.(inode.blocks))) in H; word.
    }
      ...
      - iSplitL "Hdiraddrs"; unfold own_slice; rewrite /list.untype fmap_take//.
        {
          ...
          { len. }
          replace (int.nat (I64 (Z.of_nat (length σ.(inode.blocks))))) with (length σ.(inode.blocks)); auto.
          word.
        }
        {
          rewrite HnumInd0.
    ...
    { iExists _, _; iFrame.
      ...
      - iSplitL "Hdiraddrs"; unfold own_slice; rewrite /list.untype fmap_take//.
        ...
        }
        {
          ...
          by replace (int.nat (I64 (Z.of_nat ds.(impl_s.numInd)))) with (ds.(impl_s.numInd)) by word.
        }
    }
    iAssert (is_crash_lock inodeN k #lref (∃ σ, inode_linv l σ addr ∗ P σ) True) as "#Hcrash_lock".
    { admit. }
...
Proof using allocG0 allocN heapG0 inodeN stagedG0 Σ.
  ...
  + unfold slice_subslice.
    ...
    {
      by replace
        (I64 (int.Z (I64 (Z.of_nat (length (usedBlksList ++ usedIndBlks))))))
        with (I64 (Z.of_nat (int.nat direct_s.(Slice.sz) + int.nat indirect_s.(Slice.sz)))) by word.
    }
    iApply (own_slice_cap_skip with "Hcap").
    replace (int.Z (I64 (Z.of_nat (int.nat direct_s.(Slice.sz) + int.nat indirect_s.(Slice.sz)))))
    ...
    assert (Z.of_nat (length
          ...
          replicate
            ...
            (I64 0))) = int.Z s2.(Slice.sz)) as HrewriteMe2 by word.
    rewrite -HrewriteMe2.
    len.
  + iIntros "Halloced".
    iExists d, lref.
...
Proof.
  ...
  wp_op.
  wp_if_destruct;
    replace (int.Z (I64 (Z.of_nat (length σ.(inode.blocks))))) with (Z.of_nat (length σ.(inode.blocks))) in Heqb
  ...
  - wp_op.
    ...
    (* Is indirect block *)
    {
      ...
      assert ((indBlkAddrs ++ replicate (Z.to_nat ((indirectNumBlocks - length indBlkAddrs) `mod` indirectNumBlocks)) (I64 0)) !! int.nat offset = Some blkaddr) as HlookupBlkIndPadded. {
...
Theorem wp_padInts {Stk E} enc (n: u64) (encoded : list encodable) (off: Z):
  ...
  {{{ RET #()
;
    is_enc enc 4096 (encoded ++ (EncUInt64 <$> replicate (int.nat n) (I64 0))) (off-(8*int.Z n))
  }}}.
Proof.
  ...
  wp_apply (wp_forUpto (λ i, "%Hiupper_bound" ∷ ⌜int.Z i <= int.Z n⌝ ∗
                       "Henc" ∷ is_enc enc 4096 (encoded ++ (EncUInt64 <$> (replicate (int.nat i) (I64 0))))
                       (off - (8 * int.Z i)))%I _ _
                    0 n
            with "[] [Henc Hi] [-]").
  {
...
Theorem wp_Inode__mkHdr {Stk E} l (sz numInd : Z) allocedDirAddrs allocedIndAddrs direct_s indirect_s:
  ...
  {{{ s hdr, RET (slice_val s);
    is_block s 1 hdr ∗
    "%Hencoded" ∷ ⌜block_encodes hdr ([EncUInt64 sz] ++ (EncUInt64 <$> allocedDirAddrs) ++ (EncUInt64 <$> (replicate (int.nat (maxDirect - length allocedDirAddrs)) (I64 0)))
                                     ++ (EncUInt64 <$> allocedIndAddrs) ++ (EncUInt64 <$> (replicate (int.nat (maxIndirect - length allocedIndAddrs)) (I64 0)))
                                     ++ [EncUInt64 numInd])⌝ ∗
    "direct" ∷ l ↦[Inode :: "direct"] (slice_val direct_s) ∗
    "indirect" ∷ l ↦[Inode :: "indirect"] (slice_val indirect_s) ∗
    "size" ∷ l ↦[Inode :: "size"] #sz ∗
...
Proof.
  ...
  wp_apply (wp_padInts enc (I64 (500 - int.Z (direct_s.(Slice.sz))))
  ...
  iIntros "Henc".
  replace  (int.Z (I64 4096) - 8 - 8 * Z.of_nat (length allocedDirAddrs) -
              8 * int.Z (I64 (500 - int.Z direct_s.(Slice.sz))))
    with 88 by word.
  ...
  wp_apply (wp_padInts enc (I64 (10 - int.Z (indirect_s.(Slice.sz))))
               ((([EncUInt64 sz] ++ (EncUInt64 <$> allocedDirAddrs) ++
               (EncUInt64 <$> replicate (int.nat (500 - int.Z direct_s.(Slice.sz))) (I64 0))) ++
