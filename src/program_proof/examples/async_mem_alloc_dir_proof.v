...
Section goose.
  ...
  Definition dir_state (l alloc_l: loc) (inode_refs: list loc) : iProp Σ :=
    ∃ d (inodes_s: Slice.t),
      ...
      "#inodes_s" ∷ readonly (is_slice_small inodes_s (struct.ptrT async_mem_alloc_inode.Inode) 1 (inode_refs))
  .
  ...
  Lemma wpc_openInodes d s_inodes :
    ...
    {{{ inode_s inode_refs, RET (slice_val inode_s);
        is_slice_small inode_s (struct.ptrT async_mem_alloc_inode.Inode) 1 inode_refs ∗
        [∗ list] i↦inode_ref;s_inode ∈ inode_refs;s_inodes,
            pre_inode inode_ref (U64 (Z.of_nat i)) s_inode
    }}}
    {{{ ([∗ list] i↦s_inode ∈ s_inodes,
  ...
  Proof.
    ...
    wpc_apply (wpc_forUpto
               (λ n, ∃ (inode_s: Slice.t) (inode_refs: list loc),
                   ...
                   "Hinode_slice" ∷ is_slice inode_s (struct.ptrT async_mem_alloc_inode.Inode) 1 inode_refs ∗
  ...
  Theorem wpc_inodeUsedBlocks inode_s inode_refs s_inodes :
    {{{ "Hinode_s" ∷ is_slice_small inode_s (struct.ptrT async_mem_alloc_inode.Inode) 1 inode_refs ∗
