...
Section goose.
  ...
  Local Definition allocΨ (a: u64): iProp Σ := ∃ bd b, uint.Z a d↦[bd] b.
  ...
  Definition is_dir l (sz: Z) : iProp Σ :=
    ∃ (alloc_ref: loc) (inode_refs: list loc) γalloc γused γblocks,
      ...
      "#Hinodes" ∷ ([∗ list] i ↦ inode_ref ∈ inode_refs,
        is_inode inodeN inode_ref (Pinode γblocks γused i) (I64 (Z.of_nat i))) ∗
  ...
  Definition dir_cinv sz σ (post_crash: bool) : iProp Σ :=
    ...
    "Hinodes" ∷ (∃ s_inodes,
                   ...
                   "Hinode_cinv" ∷ (if post_crash then inode_cinv_postcrash (I64 (Z.of_nat i)) s_inode
                                   else inode_cinv_precrash (I64 (Z.of_nat i)) s_inode) ∗
  ...
  Definition pre_dir l (sz: Z) dir : iProp Σ :=
    ∃ alloc_ref inode_refs γblocks γused,
      ...
      "Hinodes" ∷ (∃ s_inodes,
                     ...
                     pre_inode inode_ref (I64 (Z.of_nat i)) s_inode ∗
    ...
    iSplitL "Hinodes Hinode_blocks Hinode_used".
    {
      ...
      + iAssert (⌜(n <= sz)%Z ∧ (sz < 2^64)%Z⌝)%I as "IHbound"; [iPureIntro; word|].
        ...
        ++ rewrite big_sepL_app. iFrame "Hinodes".
           ...
           replace (Z.of_nat n) with (uint.Z (I64 n)) by word.
           rewrite Hinode_len.
  ...
  Lemma wpc_openInodes d s_inodes :
    ...
    {{{ ([∗ list] i↦s_inode ∈ s_inodes,
          inode_cinv_postcrash (I64 (Z.of_nat i)) s_inode)
      }}}
    ...
    {{{ ([∗ list] i↦s_inode ∈ s_inodes,
          inode_cinv_precrash (I64 (Z.of_nat i)) s_inode) }}}.
  Proof.
    ...
    wpc_apply (wpc_forUpto
               (λ n, ∃ (inode_s: Slice.t) (inode_refs: list loc),
                   ...
                   "Hinode_cinvs" ∷ ([∗ list] i↦s_inode ∈ (drop (uint.nat n) s_inodes),
                                     inode_cinv_postcrash (uint.nat n+i) s_inode)
               )%I
              (λ n,
               "Hpre_inodes" ∷ ([∗ list] i↦s_inode ∈ take (uint.nat n) s_inodes,
                                inode_cinv_precrash i s_inode) ∗
               "Hinode_cinvs" ∷ ([∗ list] i↦s_inode ∈ drop (uint.nat n) s_inodes,
                      inode_cinv_precrash (uint.nat n + i) s_inode)
              )%I
    ...
    { iIntros (n Φ' Φc') "!> (inv&Haddr&%Hbound) HΦ".
      ...
      change (uint.Z (I64 5)) with (Z.of_nat num_inodes) in Hbound.
      ...
      { replace (I64 $ uint.nat n + 0) with n by word.
        iFrame. }
      iSplit.
    ...
    { (* loop crash condition implies overall crash condition *)
      ...
      - iApply (big_sepL_mono with "Hinode_cinvs").
        iIntros (???) "Hpre".
        change (uint.Z (I64 5)) with 5%Z in Hbound'.
        iExactEq "Hpre".
        f_equal; len.
