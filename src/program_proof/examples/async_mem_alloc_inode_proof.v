...
Definition is_inode_durable addr σ (addrs: list u64) : iProp Σ :=
  ∃ (hdr: Block),
    ...
    implemented those *)
    "Hdata" ∷ [∗ list] a;b ∈ addrs;σ.(inode.blocks), uint.Z a d↦[b] b
.
...
Theorem is_inode_durable_read addr σ addrs :
  is_inode_durable addr σ addrs -∗
    ∃ hdr,
      ...
      "Hhdr" ∷ uint.Z addr d↦[hdr] hdr ∗
      "Hdata" ∷ ([∗ list] a;b ∈ addrs;σ.(inode.blocks), uint.Z a d↦[b] b) ∗
      "Hdurable" ∷ □(uint.Z addr d↦[hdr] hdr -∗
                    ([∗ list] a;b ∈ addrs;σ.(inode.blocks), uint.Z a d↦[b] b) -∗
                   is_inode_durable addr σ addrs).
Proof.
  iNamed 1.
  iExists _; iFrame "∗ %".
...
Theorem init_inode addr :
  uint.Z addr d↦[block0] block0 -∗ inode_cinv_postcrash addr (inode.mk ∅ []).
Proof.
  iIntros "Hhdr".
  iExists [], block0.
...
Theorem wpc_Inode__UsedBlocks {l σ addr} :
  ...
  {{{ (s:Slice.t) (addrs: list u64), RET (slice_val s);
      own_slice s uint64T (DfracOwn 1) addrs ∗
      ⌜list_to_set addrs = σ.(inode.addrs)⌝ ∗
      (own_slice s uint64T (DfracOwn 1) addrs -∗ pre_inode l addr σ) ∧ inode_cinv_precrash addr σ }}}
  {{{ inode_cinv_precrash addr σ }}}.
Proof.
  iIntros (Φ Φc) "Hinode HΦ"; iNamed "Hinode".
...
Let Ψ (a: u64) := (∃ bd b, uint.Z a d↦[bd] b)%I.
...
Proof.
  ...
  - iDestruct "Hblock" as "[_ Hb]".
    ...
    iDeexHyp "Hb".
    iAssert (□ ∀ bd b0, uint.Z a d↦[bd] b0 ∗
                      (Φc) -∗
    ...
    { instantiate (1:=λ _, (∃ b', uint.Z a d↦[b'] b0)%I); simpl.
      iSplitL.
    ...
    iSplitR "Hda"; last first.
    { instantiate (1:=λ _, (uint.Z a d↦[b0] b0)%I); simpl.
      iSplitL.
      { iExact "Hda". }
      iModIntro.
      iIntros "Hda".
