...
Section goose.
  ...
  Definition EBlk (addr: u64) :=
   (∃ v n, "Ha" ∷ uint.Z addr d↦ v ∗ "%H0iseven" ∷ ⌜ Block_to_vals v !! O = Some #(I8 n) ∧ Z.even n ⌝)%I.

  Definition written_slice : list val :=
    <[uint.nat 0:=#(I8 4)]> (replicate (uint.nat 4096) (zero_val byteT)).

  Definition written_block : Block := list_to_vec (I8 4 :: replicate (uint.nat 4095) (I8 0)).
