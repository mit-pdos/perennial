...
Section goose.
  ...
  Theorem wpc_RepBlock__Read_triple (Q: Block → iProp Σ) (Qc: iProp Σ) {k} {k' l} addr (primary: bool) :
    (S k ≤ k')%nat →
  ...
  Theorem wpc_RepBlock__Write_triple (Q: iProp Σ) (Qc: iProp Σ) {k} l k' addr (s: Slice.t) q (b: Block) :
    (S k ≤ k')%nat →
    {{{ "Hrb" ∷ is_rblock (S k') l addr ∗
        "Hb" ∷ is_block s q b ∗
        "HQc" ∷ (Q -∗ <disc> ▷ Qc) ∗
