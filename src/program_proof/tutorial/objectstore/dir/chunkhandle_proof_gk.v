...
Definition own (args__v: val) (args__c: C) (dq: dfrac) : iProp Σ :=
  "%Hown_struct" ∷ ⌜ args__v = (#args__c.(addr), (#(str args__c.(contentHash)), #()))%V ⌝.


Definition to_val' (c : C) : val :=
  (#c.(addr), (#(str c.(contentHash)), #())).

Definition from_val' (v : val) : option C :=
  match v with
  | (#(LitInt addr), (#(LitString contentHash), #()))%V =>
    Some (mkC addr contentHash)
  | _ => None
  end.

#[global]
Instance chunkHandle_into_val : IntoVal C.
Proof.
  refine {|
    to_val := to_val';
    from_val := from_val';
  ...
  |}.
  intros v. 
  destruct v as [addr contentHash]; done.
Defined.

#[global]
Instance chunkHandle_into_val_for_type : IntoValForType C (struct.t chunkhandle_gk.S).
...
Lemma own_val_ty :
  ∀ (v : val) (x : C) (dq : dfrac), own v x dq -∗ ⌜val_ty v (struct.t chunkhandle_gk.S)⌝.
Proof.
  iIntros (???) "Hown".
...
Lemma wp_Encode (args__v : val) (args__c : C) (pre_sl : Slice.t) (prefix : list u8) (dq : dfrac):
  {{{
    ...
    chunkhandle_gk.Marshal (slice_val pre_sl) args__v
  {{{
...
Proof.
  ...
  iIntros (l__addr) "Haddr". wp_pures.
