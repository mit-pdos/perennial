...
Definition own (args__v: val) (args__c: C) (dq: dfrac) : iProp Σ :=
  "%Hown_struct" ∷ ⌜ args__v = (#args__c.(opId), (#(str args__c.(key)), #()))%V ⌝.
...
Definition from_val' (v : val) : option C :=
  match v with
  | (#(LitInt opId), (#(LitString key), #()))%V =>
    Some (mkC opId key)
  | _ => None
  end.

#[global]
Instance get_into_val : IntoVal C.
Proof.
  refine {|
    to_val := to_val';
    from_val := from_val';
  ...
  |}.
  intros v. 
  destruct v as [opId key]; done.
Defined.

#[global]
Instance get_into_val_for_type : IntoValForType C (struct.t get_gk.S).
...
Lemma own_val_ty :
  ∀ (v : val) (x : C) (dq : dfrac), own v x dq -∗ ⌜val_ty v (struct.t get_gk.S)⌝.
Proof.
  iIntros (???) "Hown".
...
Lemma wp_Encode (args__v : val) (args__c : C) (pre_sl : Slice.t) (prefix : list u8) (dq : dfrac):
  {{{
    ...
    get_gk.Marshal (slice_val pre_sl) args__v
  {{{
