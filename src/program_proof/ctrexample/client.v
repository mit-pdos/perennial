...
Proof.
  ...
  iAssert (∃ x, counter_lb γ x ∗ localBound_ptr ↦[uint64T] #(I64 x))%I with "[HlocalBound Hlb]" as "HH".
  {
  ...
  (* FIXME: why doesn't [$Hcl] work? *)
    instantiate (1:=(λ l, ∃ (x:nat), ⌜has_encoding l [EncUInt64 (I64 x)] ∧ localBound ≤ x⌝ ∗ counter_lb γ x)%I).
    iModIntro.
    iModIntro.
    simpl.
    iIntros (x) "Hctr".
