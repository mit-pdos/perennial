...
a smaller example of such a trace invariant can be found with the hash model
...
Definition init_state (s : state_ty) : Prop :=
  s = mk_hash_state ∅.
...
Lemma hashes_bij {t i a} :
  valid_trace t →
  t.(states) !! i = Some a →
  gset_bijective a.(hashes).
Proof.
...
Lemma hashes_mono {t i k a b} :
  valid_trace t →
  t.(states) !! i = Some a →
  t.(states) !! (i + k) = Some b →
  a.(hashes) ⊆ b.(hashes).
Proof.
  intros Hvalid Hlook_i.
  generalize dependent b.
  induction k.
  { replace (i + 0) with (i) in * by lia. naive_solver. }
  intros c Hlook_Sk.
  assert (∃ b, t.(states) !! (i + k) = Some b) as [b Hlook_k].
  { apply lookup_lt_is_Some. apply lookup_lt_Some in Hlook_Sk. lia. }
  specialize (IHk b Hlook_k).
  destruct Hvalid as [_ Hsteps].
  opose proof (Forall3_lookup_r _ _ _ _ _ _ Hsteps) as
    (prev_st & trans & Hlook_prev & Hlook_trans & Hstep); clear Hsteps.
  { replace (i + S k) with (S (i + k)) in * by lia. by rewrite lookup_drop. }
  apply lookup_take_Some in Hlook_prev as [? _].
  simplify_eq/=.
  etrans; [done|]; clear a Hlook_i IHk.
  rewrite /step in Hstep.
...
Lemma coll_resis {t i k h m0 m1} :
  ...
  m0 = m1.
Proof.
  intros Hvalid Hlook_l0 Hlook_l1.
  pose proof Hvalid as [_ Hsteps].
  opose proof (Forall3_lookup_m _ _ _ _ _ _ Hsteps) as
    (_ & s0 & _ & Hlook_s0 & _); [apply Hlook_l0|].
  opose proof (Forall3_lookup_m _ _ _ _ _ _ Hsteps) as
    (_ & s1 & _ & Hlook_s1 & _); [apply Hlook_l1|].
  rewrite lookup_drop in Hlook_s0.
  rewrite lookup_drop in Hlook_s1.

  (* use our helper lemmas about hashes. *)
  pose proof (hashes_incl Hvalid Hlook_l0 Hlook_s0) as Hincl0.
  pose proof (hashes_incl Hvalid Hlook_l1 Hlook_s1) as Hincl1.
  opose proof (hashes_mono Hvalid Hlook_s0 Hlook_s1) as Hsubset.
  pose proof (hashes_bij Hvalid Hlook_s1) as Hbij.
