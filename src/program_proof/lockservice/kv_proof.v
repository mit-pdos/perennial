...
Definition Get_Pre γ va : RPCValsC -> iProp Σ := (λ args, args.(W64_1) [[γ.(ks_kvMapGN)]]↦ va)%I.
Definition Get_Post γ va : RPCValsC -> u64 -> iProp Σ := λ args v, (⌜v = va⌝ ∗ args.(W64_1) [[γ.(ks_kvMapGN)]]↦ v)%I.

Definition Put_Pre γ : RPCValsC -> iProp Σ := (λ args, args.(W64_1) [[γ.(ks_kvMapGN)]]↦ _)%I.
...
Proof.
  ...
  iNamed "Hclerk".
  repeat wp_loadField.
  wp_apply (RPCClient__MakeRequest_spec _ _ _ {|U64_1:=_ ; W64_2:= _ |} γ.(ks_rpcGN) with "[] [Hpre Hcl]"); eauto.
  {
    iNamed "Hserver".
    iApply "Hgetspec".
  }
...
Proof.
  ...
  iNamed "Hclerk".
  repeat wp_loadField.
  wp_apply (RPCClient__MakeRequest_spec _ _ _ {| W64_1:= _; W64_2:=_ |} γ.(ks_rpcGN) with "[] [Hpre Hcl]"); eauto.
  {
