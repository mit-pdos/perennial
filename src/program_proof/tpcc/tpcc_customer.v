...
Definition customer_tbl_txnpt (τ : gname) (tplid : u64) (data : u64 * u64 * (list u8)) :=
  txnmaps_ptsto τ (I64 0) (u64_le tplid) (Value (u64_le data.1.1 ++ u64_le data.1.2 ++ data.2)).

Definition customer_tbl_dbpt (τ : gname) (tplid : u64) q (data : u64 * u64 * (list u8)) :=
  dbmaps_ptsto τ (I64 0) (u64_le tplid) q (Value (u64_le data.1.1 ++ u64_le data.1.2 ++ data.2)).
...
Definition customer_idx_txnpt (τ : gname) (cid cwid : u64) (tplid : u64) :=
  txnmaps_ptsto τ (I64 1) ((u64_le cid) ++ (u64_le cwid)) (Value (u64_le tplid)).

Definition customer_idx_txnpt_nil (τ : gname) (cid cwid : u64) :=
  txnmaps_ptsto τ (I64 1) ((u64_le cid) ++ (u64_le cwid)) Nil.

Definition customer_idx_dbpt (τ : gname) (cid cwid : u64) q (tplid : u64) :=
  dbmaps_ptsto τ (I64 1) ((u64_le cid) ++ (u64_le cwid)) q (Value (u64_le tplid)).

Definition customer_idx_dbpt_nil (τ : gname) (cid cwid : u64) q :=
  dbmaps_ptsto τ (I64 1) ((u64_le cid) ++ (u64_le cwid)) q Nil.
...
Proof.
  ...
  set m : u64 -> list u8 -> dbmaps :=
    λ (tplid : u64) (clast : list u8),
      {[ (I64 1, u64_le cid ++ u64_le cwid) := Value (u64_le tplid) ]} ∪
      {[ (I64 0, u64_le tplid) := Value (u64_le cid ++ u64_le cwid ++ clast) ]}.
  ...
  set Rc := (λ (r : dbmaps), ∃ (tplid : u64) (clast : list u8) (clastS : Slice.t),
                ⌜r !! (I64 1, u64_le cid ++ u64_le cwid) = Some (Value (u64_le tplid)) ∧
                 r !! (I64 0, u64_le tplid) = Some (Value (u64_le cid ++ u64_le cwid ++ clast))⌝ ∗
...
Proof.
  ...
  iIntros (cRef) "HcRef".
  wp_pures.
  set m : dbmaps := {[ (I64 1, u64_le cid ++ u64_le cwid) := Nil ]}.
