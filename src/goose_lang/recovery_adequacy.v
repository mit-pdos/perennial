  ...
  (∀ `{Hheap : !heapG Σ},
     ⊢ (ffi_start (heapG_ffiG) σ.(world) -∗ trace_frag σ.(trace) -∗ oracle_frag σ.(oracle) ={⊤}=∗
       ...
       □ (∀ hG (Hpf: @heapG_invG _ _ _ _ Hheap = @heapG_invG _ _ _ _ hG),
                     Φinv hG -∗ □ ∀ σ κ n, state_interp σ κ n -∗ |NC={⊤, ∅}=> ⌜ φinv σ ⌝) ∗
        wpr s k ⊤ e r (λ v, ⌜φ v⌝) Φinv (λ _ v, ⌜φr v⌝))) →
  recv_adequate (CS := goose_crash_lang) s e r σ (λ v _, φ v) (λ v _, φr v) φinv.
Proof.
  intros Hwp.
  eapply (wp_recv_adequacy_inv _ _ _ heap_namesO _ _ _ _ _ _ _ _ (λ Hinv Hc names, Φinv (heap_update_pre _ _ Hinv Hc (@pbundleT _ _ names)))).
  iIntros (???) "".
  ...
  iExists
    (λ t σ κs, let _ := heap_update Σ hG Hinv Hc (@pbundleT _ _ t) in
