...
(* a slight variant that can be used manually when [Q] is already persistent;
this is a little more convenient since you don't also have to eliminate the □
modality *)
Lemma update_into_persistent {M} (P Q : uPred M) {H: UpdateIntoPersistently P Q} :
  P ⊢ |==> Q.
Proof.
  rewrite (update_into_persistently P Q).
  iIntros ">#H". iFrame "H". done.
Qed.
...
Tactic Notation "iPersist" constr(H) "as" constr(H') := iPersist_hyp H H'.
