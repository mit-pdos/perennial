...
Section log.
  ...
  Instance log_semantics : ffi_semantics log_op log_model :=
    {| ffi_step := log_step;
       ffi_crash_step := fun s s' => relation.denote close s s' tt; |}.
End log.
...
Next Obligation.
  ...
  iFrame. iModIntro. by rewrite -own_op -Cinl_op -pair_op frac_op Qp.half_half agree_idemp.
Qed.
Next Obligation.
...
Instance log_spec_ext : spec_ffi_op := {| spec_ffi_op_field := log_op |}.
...
Instance log_spec_ty : ext_types (spec_ffi_op_field) := log_ty.
...
Lemma ghost_step_init_stuck E j K {HCTX: LanguageCtx K} σ g:
  ...
  j ⤇ K (ExternalOp (ext := @spec_ffi_op_field log_spec_ext) InitOp #()) -∗
...
Lemma ghost_step_open_stuck E j K {HCTX: LanguageCtx K} σ g:
  ...
  j ⤇ K (ExternalOp (ext := @spec_ffi_op_field log_spec_ext) OpenOp #()) -∗
  source_ctx (CS := spec_crash_lang) -∗
  source_state σ g -∗
  |NC={E}=> False.
Proof.
...
Lemma log_closed_init_false vs E j K {HCTX: LanguageCtx K}:
  ...
  log_closed_frag -∗
  log_frag vs -∗
  j ⤇ K (ExternalOp (ext := @spec_ffi_op_field log_spec_ext) InitOp #()) -∗ |NC={E}=>
  False.
Proof.
...
Proof.
  ...
  - iMod (ghost_step_lifting with "Hj Hctx H") as "(Hj&H&_)".
    { apply base_prim_step_trans. simpl. econstructor.
...
Lemma log_init_open_false E j K {HCTX: LanguageCtx K} j' K' {HCTX': LanguageCtx K'}:
  ...
  spec_ctx -∗
  j ⤇ K (ExternalOp (ext := @spec_ffi_op_field log_spec_ext) InitOp #()) -∗
  j' ⤇ K' (ExternalOp (ext := @spec_ffi_op_field log_spec_ext) OpenOp #()) -∗ |NC={E}=>
  False.
Proof.
  iIntros (?) "(#Hctx&#Hstate) Hj Hj'".
  iInv "Hstate" as (σ g) "(>H&Hinterp)" "Hclo".
  ...
  - iMod (ghost_step_stuck with "Hj' Hctx H") as "[]".
    { eapply stuck_ExternalOp; first (by eauto).
      apply base_irreducible_not_atomically; [ by inversion 1 | ].
      intros ??????. by repeat (inv_base_step; simpl in H3; repeat monad_inv).
    }
    { solve_ndisj. }
  - iMod (ghost_step_init_stuck with "Hj [$] [$]") as "[]".
    { solve_ndisj. }
...
Lemma ghost_step_log_reset E j K {HCTX: LanguageCtx K} l vs:
  ...
  log_open l -∗
  log_frag vs -∗
  j ⤇ K (ExternalOp (ext := @spec_ffi_op_field log_spec_ext) ResetOp #l)
  -∗ |NC={E}=>
  j ⤇ K #()%V ∗log_frag [].
Proof.
  ...
  iMod (ghost_step_lifting with "Hj Hctx H") as "(Hj&H&_)".
  { apply base_prim_step_trans. repeat (eauto || monad_simpl || rewrite Heq || econstructor || simpl). }
  { solve_ndisj. }
  simpl. rewrite Heq.
  iDestruct "Hffi" as "(Huninit_auth&Hvals_auth)".
