...
Class perennialG (Λ : language) (CS: crash_semantics Λ) (T: ofeT) (Σ : gFunctors) := PerennialG {
  perennial_irisG :> ∀ (H: invG Σ) (Hcrash: crashG Σ), pbundleG T Σ → irisG Λ Σ;
  perennial_invG: ∀ H1 H2 t, @iris_invG _ _ (perennial_irisG H1 H2 t) = H1;
  perennial_crashG: ∀ H1 H2 t, @iris_crashG _ _ (perennial_irisG H1 H2 t) = H2
}.
...
Proof.
  rewrite /wpr_pre=> n wp wp' Hwp H1inv H2crash t E1 e1 rec Φ Φinv Φc.
  apply wpc_ne; eauto;
...
Definition wpr_def `{!perennialG Λ CS T Σ} (s : stuckness) k :
  ...
  (invG Σ → crashG Σ → pbundleG T Σ → iProp Σ) →
  (invG Σ → crashG Σ → pbundleG T Σ → val Λ → iProp Σ) → iProp Σ := fixpoint (wpr_pre s k).
...
Lemma wpr_strong_mono s k Hi Hc t E e rec Φ Ψ Φinv Ψinv Φr Ψr :
  wpr s k Hi Hc t E e rec Φ Φinv Φr -∗
      (∀ v, Φ v ==∗ Ψ v) ∧ <bdisc> ((∀ Hi Hc t, Φinv Hi Hc t -∗ Ψinv Hi Hc t) ∧
                                    (∀ Hi Hc t v, Φr Hi Hc t v ==∗ Ψr Hi Hc t v)) -∗
