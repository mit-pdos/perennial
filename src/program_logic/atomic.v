...
Notation "'{{{' P } } } '<<{' ∀∀ x1 .. xn , Pa '}>>' e @ s ; k ; E1 ; E2 <<{ Qa '}>>' {{{ z1 .. zn , 'RET' pat ; Q } } }" :=
  (□ ∀ Φ Φc,
      P -∗
      <disc> ▷ Φc (* crash condition before lin.point *) ∧ 
        ▷ (∀ x1, .. (∀ xn, Pa ={⊤}=∗ Qa ∗
          (<disc> ▷ Φc (* crash condition after lin.point *) ∧
           ∀ z1, .. (∀ zn, Q -∗ Φ pat%V) .. )) .. ) -∗
      WPC e @ s; k; E1; E2 {{ Φ }} {{ Φc }})%I
    (at level 20, x1 closed binder, xn closed binder, z1 closed binder, zn closed binder,
     format "'[hv' {{{  P  } } }  '/'  <<{  ∀∀  x1  ..  xn ,  Pa }>>  '/  ' e  '/' @  s ; k ;  E1 ; E2 '/' <<{ Qa }>> '/' {{{  z1  ..  zn ,  RET  pat ;  Q  } } } ']'") : bi_scope.

Notation "'{{{' P } } } '<<{' ∀∀ x1 .. xn , Pa '}>>' e @ s ; k ; E1 ; E2 <<{ Qa '}>>' {{{ 'RET' pat ; Q } } }" :=
  (□ ∀ Φ Φc,
      P -∗
      <disc> ▷ Φc (* crash condition before lin.point *) ∧ 
        ▷ (∀ x1, .. (∀ xn, Pa ={⊤}=∗ Qa ∗
    ...
    (at level 20, x1 closed binder, xn closed binder,
     format "'[hv' {{{  P  } } }  '/'  <<{  ∀∀  x1  ..  xn ,  Pa }>>  '/  ' e  '/' @  s ; k ;  E1 ; E2 '/' <<{ Qa }>> '/' {{{  RET  pat ;  Q  } } } ']'") : bi_scope.
